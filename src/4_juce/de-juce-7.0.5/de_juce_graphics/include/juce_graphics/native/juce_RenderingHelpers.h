...
namespace EdgeTableFillers
{
    ...
    struct TransformedImageFill
    {
        TransformedImageFill (const Image::BitmapData& dest, const Image::BitmapData& src,
                              const AffineTransform& transform, int alpha, GraphicsResamplingQuality q)
            : interpolator (transform,
                            q != Graphics_lowResamplingQuality ? 0.5f : 0.0f,
                            q != Graphics_lowResamplingQuality ? -128 : 0),
              destData (dest),
              srcData (src),
              extraAlpha (alpha + 1),
              quality (q),
    ...
    private:
        ...
        void generate (PixelType* dest, int x, int numPixels) noexcept
        {
            ...
            do
            {
                ...
                if (quality != Graphics_lowResamplingQuality)
                {
        ...
        const GraphicsResamplingQuality quality;
        const int maxX, maxY;
        int currentY;
        DestPixelType* linePixels;
    ...
    void renderImageTransformed (Iterator& iter, const Image::BitmapData& destData, const Image::BitmapData& srcData,
                                 int alpha, const AffineTransform& transform, GraphicsResamplingQuality quality, bool tiledFill)
    {
...
struct ClipRegions
{
    struct Base  : public SingleThreadedReferenceCountedObject
    {
        ...
        virtual Ptr clipToImageAlpha (const Image&, const AffineTransform&, GraphicsResamplingQuality) = 0;
        ...
        virtual void renderImageTransformed (SavedStateType&, const Image&, int alpha, const AffineTransform&, GraphicsResamplingQuality, bool tiledFill) const = 0;
    ...
    struct EdgeTableRegion  : public Base
    {
        ...
        Ptr clipToImageAlpha (const Image& image, const AffineTransform& transform, GraphicsResamplingQuality quality) override
        {
            ...
            if (transform.isOnlyTranslation())
            {
                ...
                if (quality == Graphics_lowResamplingQuality || ((tx | ty) & 224) == 0)
                {
        ...
        void renderImageTransformed (SavedStateType& state, const Image& src, int alpha, const AffineTransform& transform, GraphicsResamplingQuality quality, bool tiledFill) const override
        {
    ...
    private:
        ...
        void transformedClipImage (const Image::BitmapData& srcData, const AffineTransform& transform, GraphicsResamplingQuality quality, const SrcPixelType*)
        {
...
public:
    ...
    SavedStateBase (Rectangle<int> initialClip)
        : clip (new RectangleListRegionType (initialClip)),
          interpolationQuality (Graphics_mediumResamplingQuality), transparencyLayerAlpha (1.0f)
    {
    }
    ...
    void renderImage (const Image& sourceImage, const AffineTransform& trans, const BaseRegionType* tiledFillClipRegion)
    {
        ...
        if (isOnlyTranslationAllowingError (t, 0.002f))
        {
            ...
            if (interpolationQuality == Graphics_lowResamplingQuality || ((tx | ty) & 224) == 0)
            {
                tx = ((tx + 128) >> 8);
                ty = ((ty + 128) >> 8);
    ...
    FillType fillType;
    GraphicsResamplingQuality interpolationQuality;
...
public:
    ...
    void renderImageTransformed (IteratorType& iter, const Image& src, int alpha, const AffineTransform& trans, GraphicsResamplingQuality quality, bool tiledFill) const
    {
...
public:
    ...
    void setInterpolationQuality (GraphicsResamplingQuality quality) override  { stack->interpolationQuality = quality; }
