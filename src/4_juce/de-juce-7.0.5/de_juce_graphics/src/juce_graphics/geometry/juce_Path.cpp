...
//static 
bool isPathMarker (float value, float marker) // noexcept
{
    return value == marker;
}
...
bool Path::isEmpty() const noexcept
{
    for (auto i = data.begin(), e = data.end(); i != e; ++i)
    {
        ...
        if (isPathMarker (type, moveMarker))
        {
            i += 2;
        }
        else if (isPathMarker (type, lineMarker)
                 || isPathMarker (type, quadMarker)
                 || isPathMarker (type, cubicMarker))
        {
            return false;
        }
    }
...
{
    if (! (data.isEmpty() || isPathMarker (data.getLast(), closeSubPathMarker)))
...
Point<float> Path::getCurrentPosition() const
{
    ...
    if (isPathMarker (*i, closeSubPathMarker))
    {
        while (i != data.begin())
        {
            if (isPathMarker (*--i, moveMarker))
            {
                i += 2;
                break;
            }
...
void Path::addPath (const Path& other)
{
    ...
    for (int i = 0; i < other.data.size();)
    {
        ...
        else if (isPathMarker (type, quadMarker))
        {
            quadraticTo (d[i], d[i + 1], d[i + 2], d[i + 3]);
            i += 4;
        }
        else if (isPathMarker (type, cubicMarker))
        {
            cubicTo (d[i], d[i + 1], d[i + 2], d[i + 3], d[i + 4], d[i + 5]);
            i += 6;
        }
        else if (isPathMarker (type, closeSubPathMarker))
        {
            closeSubPath();
        }
        else
                    ...
                    const AffineTransform& transformToApply)
{
    ...
    for (int i = 0; i < other.data.size();)
    {
        ...
        if (isPathMarker (type, closeSubPathMarker))
        {
            closeSubPath();
        }
        else
...
Path Path::createPathWithRoundedCorners (const float cornerRadius) const
{
    ...
    while (n < data.size())
    {
        ...
        if (isPathMarker (type, moveMarker))
        {
            ...
            lastWasLine = false;
            firstWasLine = (isPathMarker (elements[n], lineMarker));
        }
        else if (isPathMarker (type, lineMarker) || isPathMarker (type, closeSubPathMarker))
        {
            float startX = 0, startY = 0, joinX = 0, joinY = 0, endX, endY;

            if (isPathMarker (type, lineMarker))
            {
                endX = elements[n++];
                endY = elements[n++];
...
String Path::toString() const
{
    ...
    for (int i = 0; i < data.size();)
    {
        ...
        {
            jassert (isPathMarker (type, closeSubPathMarker));
            markerChar = 'z';
        }

        if (! isPathMarker (type, lastMarker))
        {
            if (s.getDataSize() != 0)
                s << ' ';
