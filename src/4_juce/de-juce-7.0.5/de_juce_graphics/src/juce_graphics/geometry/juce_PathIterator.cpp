...
bool PathFlatteningIterator::isLastInSubpath() const noexcept
{
    return stackPos == stackBase.get()
             && (source == path.data.end() || isPathMarker (*source, Path::moveMarker));
}
...
bool PathFlatteningIterator::next()
{
    ...
    for (;;)
    {
        ...
        if (stackPos == stackBase.get())
        {
            ...
            if (! isPathMarker (type, Path::closeSubPathMarker))
            {
                x2 = *source++;
                y2 = *source++;

                if (isPathMarker (type, Path::quadMarker))
                {
                    x3 = *source++;
                    y3 = *source++;
                ...
                else if (isPathMarker (type, Path::cubicMarker))
                {
                    x3 = *source++;
                    y3 = *source++;
                    x4 = *source++;
        ...
        if (isPathMarker (type, Path::lineMarker))
        {
            ++subPathIndex;
        ...
        else if (isPathMarker (type, Path::closeSubPathMarker))
        {
        ...
        {
            jassert (isPathMarker (type, Path::moveMarker));
