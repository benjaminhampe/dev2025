...
std::pair<const uint8_t*, int> getFactoryPresetBinaryData(const std::string& p_preset) {
    switch(hash(p_preset.c_str())) {
    case hash("Arp Charger [RM]"):
        return std::make_pair(BinaryData::Arp_Charger_RM_odin,
                              BinaryData::Arp_Charger_RM_odinSize);
    ...
    default:
        // JUCE_DBG("ERROR: Illegal Lookup in FactoryPresetBinaryMapping.h PresetName: " + p_preset);
