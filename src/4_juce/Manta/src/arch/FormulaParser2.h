...
namespace fx
{
   ...
   inline unsigned int getDigit(Char chr) noexcept
   {
      return chr - '0';
   }
   ...
   inline String toString(ParserErrorType p)
   {
      switch (p)
      {
   ...
   inline int getAssociativity(Operator op)
   {
      switch (op)
      {
      case Operator::Plus:
   ...
   inline int getNumArguments(Operator op) noexcept
   {
      switch (op)
      {
      case Operator::Plus:
   ...
   inline Operator getOperator(const String& str)
   {
      if (str == "+")
   ...
   inline String toString(Operator o)
   {
      switch (o)
      {
      case Operator::Plus:
   ...
   inline String getOperator(const String& txt, int& i)
   {
   ...
   inline Func getFunc(Operator o)
   {
      switch (o)
      {
      case Operator::Asinh:
   ...
   inline Func2 getFunc2(Operator o)
   {
      switch (o)
      {
      case Operator::Plus:
   ...
   inline String toString(const Token& t)
   {
      switch (t.type)
      {
         ...
         func = [this, postfix](float x)
         {
            ...
            for (const auto& p : postfix)
            {
               ...
               case Token::Type::X:
                  ...
                  break;
               case Token::Type::Operator:
                  if (int(stack.size()) < p.numArguments)
                  {
