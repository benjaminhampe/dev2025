...
// ============================================================================
Shell::Shell( LiveSkin & skin, Track* parent )
// ============================================================================
   : QWidget( parent )
   , m_skin( skin )
   , m_track( parent )
...
void Shell::timerEvent( QTimerEvent* event )
{
...
// =========================================
void
Shell::enterEvent( QEnterEvent* event )
{
   m_isHovered = true;
   update();
...
void
Shell::showEvent(QShowEvent* event )
{
    DE_DEBUG("")
   startUpdateTimer();
...
void
Shell::hideEvent(QHideEvent* event )
{
   DE_DEBUG("")
   stopUpdateTimer();
...
void
Shell::on_btn_enabled( bool enabled )
{
...
void
Shell::on_btn_more( bool visible )
{
}

void
Shell::on_btn_editor( bool visible )
{
...
void Shell::setExtraMoreVisible( bool visible )
{
...
bool Shell::openPlugin( de::audio::PluginInfo const & pluginInfo )
{
   closePlugin();
   ...
   // Shell needs path/directory of itself
   {
...
void
Shell::setInputSignal( int i, de::audio::IDspChainElement* input )
{
   m_inputSignal = input;
}

void
Shell::clearInputSignals()
{
   m_inputSignal = nullptr;
}

void
Shell::sendMidi( uint8_t byte1, uint8_t data1, uint8_t data2  )
{
   if ( !isSynth() )
   {
      return;
...
void
Shell::aboutToStart( uint32_t dstFrames, uint32_t dstChannels, uint32_t dstRate )
{
...
Shell::readSamples( double pts, float* dst, uint32_t dstFrames, uint32_t dstChannels, uint32_t dstRate )
{
   ...
   if ( !dst )
   {
      throw std::runtime_error("No dst audio dsp buffer in Shell::readSamples()!");
   }
...
//uint64_t
//Shell::getSamplePos() const { return m_framePos; }
//uint32_t
//Shell::getSampleRate() const { return m_sampleRate; }
//uint64_t
//Shell::getBlockSize() const { return m_bufferFrames; }
//uint64_t
//Shell::getChannelCount() const { return m_channelCount; }

//bool
//Shell::isSynth() const { return getFlags(effFlagsIsSynth); }
intptr_t
Shell::dispatcher( int32_t opcode, int32_t index, intptr_t value, void *ptr, float opt ) const
{
//   if ( !m_isLoaded )
//   {
...
// This function is called from refillCallback() which is running in audio thread.
//float**
//Shell::processAudio( uint64_t frameCount, uint64_t & outputFrameCount )
//{
...
// static
VstIntPtr
Shell::hostCallback_static( AEffect* effect, VstInt32 opcode, VstInt32 index, VstIntPtr value, void *ptr, float opt )
{
   if ( effect && effect->user )
   {
      auto me = static_cast< Shell* >( effect->user );
...
VstIntPtr
Shell::hostCallback(VstInt32 opcode, VstInt32 index, VstIntPtr value, void* ptr, float)
{
   switch(opcode)
   {
      default:                                break;
