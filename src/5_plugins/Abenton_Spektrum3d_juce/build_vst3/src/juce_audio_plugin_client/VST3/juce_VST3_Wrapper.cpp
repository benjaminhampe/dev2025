...
public:
          ...
          host (h)
    {
        ...
        // TODO:
        comPluginInstance = VSTComSmartPtr<JuceAudioProcessor> { new JuceAudioProcessor(pluginInstance.get()) };
    ...
    //==============================================================================
    // TODO: bool getCurrentPosition (CurrentPositionInfo& info) override
    Optional<PositionInfo> getPosition() const override
    {
        PositionInfo info;
        auto timeInSamples = jmax ((juce::int64) 0, processContext.projectTimeSamples);
        info.setTimeInSamples( timeInSamples );
        info.setTimeInSeconds( static_cast<double> (timeInSamples) / processContext.sampleRate );
        info.setBpm( jmax (1.0, processContext.tempo) );
        TimeSignature ts;
        ts.numerator = jmax (1, (int) processContext.timeSigNumerator);
        ts.denominator = jmax (1, (int) processContext.timeSigDenominator);
        info.setTimeSignature(ts);
        info.setPpqPositionOfLastBarStart( processContext.barPositionMusic );
        info.setPpqPosition( processContext.projectTimeMusic );
        LoopPoints lp;
        lp.ppqStart = processContext.cycleStartMusic;
        lp.ppqEnd = processContext.cycleEndMusic;
        ...
        if (info.getFrameRate())
        info.setEditOriginTime( (double) processContext.smpteOffsetSubframes / (80.0 * info.getFrameRate()->getEffectiveRate()) );
            ...
            if (auto* bus = pluginInstance->getBus (dir == Vst::kInput, index))
            {
                ...
                info.busType = [&]
                {
                    ...
                    if (dir == Vst::kInput)
                    {
                        if (isFirstBus)
                        {
                            if (auto* extensions = dynamic_cast<VST3ClientExtensions*> (pluginInstance.get()))
