...
#else
    m_dsp.setInputSampleRate(48000);
#endif
}

bool TimeStretchProcessor::isBusesLayoutSupported (const juce::AudioProcessor::BusesLayout& layouts) const
{
    return layouts.getMainInputChannelSet()  == juce::AudioChannelSet::stereo()
    && layouts.getMainOutputChannelSet() == juce::AudioChannelSet::stereo();
}

void TimeStretchProcessor::prepareToPlay (double sampleRate, int samplesPerBlock)
{
#if 0
...
#else
    m_dsp.setInputSampleRate(sampleRate);
    m_dsp.setup(samplesPerBlock,2,sampleRate);
    m_dspIn.resize(samplesPerBlock * 2);
    m_dspOut.resize(samplesPerBlock * 2);
#endif

    double a = *apvts.getRawParameterValue("tempo");
    double b = *apvts.getRawParameterValue("rate");
    double c = *apvts.getRawParameterValue("pitch");
    int d = (int)*apvts.getRawParameterValue("sequence");
    int e = (int)*apvts.getRawParameterValue("seek");
    int f = (int)*apvts.getRawParameterValue("overlap");
    int g = (int)*apvts.getRawParameterValue("aa");

    DE_BENNI("tempo = ",a)
    DE_BENNI("rate = ",b)
    DE_BENNI("pitch = ",c)
    DE_BENNI("sequenceMs = ",d)
    DE_BENNI("seekWinMs = ",e)
    DE_BENNI("overlapMs = ",f)
    DE_BENNI("aaFilterLen = ",g)
}

void TimeStretchProcessor::processBlock (juce::AudioBuffer<float>& buffer,
                                       juce::MidiBuffer&)
{
    ...
    // === APPLY PARAMETERS ===
    //m_dsp.m_realTimeParams.virtualTempo = *apvts.getRawParameterValue("tempo");
    // m_dsp.m_realTimeParams.virtualRate = *apvts.getRawParameterValue("rate");
    //m_dsp.m_realTimeParams.virtualPitch = *apvts.getRawParameterValue("pitch") / 12.0f;

    m_dsp.m_realTimeParams.sequenceMs = (int)*apvts.getRawParameterValue("sequence");
    m_dsp.m_realTimeParams.seekWinMs = (int)*apvts.getRawParameterValue("seek");
    m_dsp.m_realTimeParams.overlapMs = (int)*apvts.getRawParameterValue("overlap");
    // m_dsp.m_realTimeParams.aaFilterLen = (int)*apvts.getRawParameterValue("aa");

    // === INTERLEAVE-L ===
    {
        const float* __restrict__ src = buffer.getReadPointer(0);
              float* __restrict__ dst = m_dspIn.data();
    ...
    // === INTERLEAVE-R ===
    {
        const float* __restrict__ src = buffer.getReadPointer(1);
              float* __restrict__ dst = m_dspIn.data() + 1;
    ...
    m_dsp.process(m_dspIn.data(),L,R);

#endif
}

void TimeStretchProcessor::getStateInformation (juce::MemoryBlock& destData)
{
    auto xml = apvts.copyState().createXml();
    copyXmlToBinary(*xml, destData);
}

void TimeStretchProcessor::setStateInformation (const void* data, int sizeInBytes)
{
...
// PARAMETER LAYOUT
// ============================================================================
TimeStretchProcessor::APVTS::ParameterLayout TimeStretchProcessor::createLayout()
{
...
juce::AudioProcessorEditor* TimeStretchProcessor::createEditor()
{
    return new TimeStretchEditor(*this);
}
...
{
    return new TimeStretchProcessor();
}
...
bool TimeStretchProcessor::isBusesLayoutSupported (const BusesLayout& layouts) const
{
    return layouts.getMainInputChannelSet()  == juce::AudioChannelSet::stereo()
    && layouts.getMainOutputChannelSet() == juce::AudioChannelSet::stereo();
}
...
TimeStretchProcessor::APVTS::ParameterLayout
TimeStretchProcessor::createLayout()
{
    std::vector<std::unique_ptr<juce::RangedAudioParameter>> p;
