...
de::FileInfos DropList::getFileInfosA() const
{
...
DropList::DropList(int X, int Y, int W, int H, const char* L)
...
void
DropList::addRow(std::string uri)
{
    if (uri.size() > 4096)
    {
    ...
    uri = de::FileSystem::makePosixPath(uri);
...
void DropList::emit_onListChanged()
{
...
// Update progress
void DropList::set_progress(int row, float p)
{
...
// Custom row height
// -----------------------------
int DropList::item_height(void*) const
{
    //return textsize() + 8;
    return 24 * Fl::screen_scale(0);
}

void DropList::sortRows()
{
...
// Custom row drawing
// -----------------------------
void DropList::item_draw(void* v, int X, int Y, int W, int H) const
{
