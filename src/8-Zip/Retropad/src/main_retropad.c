...
static void HandleCommand(HWND hwnd, WPARAM wParam, LPARAM lParam) {
    ...
    case IDM_EDIT_GOTO:
        if (g_app.wordWrap) {
            ...
            DialogBoxW(g_hInst, MAKEINTRESOURCEW(IDD_GOTO), hwnd, GoToDlgProc);
        }
        break;
...
int APIENTRY wWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPWSTR lpCmdLine, int nCmdShow) {
    ...
    wc.hIcon = LoadIconW(hInstance, MAKEINTRESOURCEW(IDI_RETROPAD));
    wc.hIconSm = wc.hIcon;
    wc.hCursor = LoadCursorW(NULL, MAKEINTRESOURCEW(IDC_IBEAM));
    wc.hbrBackground = (HBRUSH)(COLOR_WINDOW + 1);
    wc.lpszClassName = L"RETROPAD_WINDOW";
    wc.lpszMenuName = MAKEINTRESOURCEW(IDC_RETROPAD);
    ...
    HACCEL accel = LoadAcceleratorsW(hInstance, MAKEINTRESOURCEW(IDC_RETROPAD));
...
{
    // 1) CRT liefert ANSI-argv → wir brauchen UTF‑16
    int wargc = 0;
    LPWSTR* wargv = CommandLineToArgvW(GetCommandLineW(), &wargc);

    // 2) lpCmdLine nachbauen (alles hinter dem Programmname)
    LPWSTR lpCmdLine = GetCommandLineW();
    int nCmdShow = SW_SHOWDEFAULT;

    // 3) WinMain-Parameter erzeugen
    HINSTANCE hInstance     = GetModuleHandleW(NULL);
    HINSTANCE hPrevInstance = NULL;

    // 4) Aufruf deiner echten WinMain-Implementierung
    int ret = wWinMain(hInstance, hPrevInstance, lpCmdLine, nCmdShow);
