...
void fast_sin7_avx512(const float* __restrict input, float* __restrict output) {
...
void fast_sin9_avx512(const float* __restrict input, float* __restrict output) {
    __m512 x  = _mm512_loadu_ps(input);
    __m512 x2 = _mm512_mul_ps(x, x);         // x²
    __m512 x3 = _mm512_mul_ps(x2, x);        // x³
    __m512 x5 = _mm512_mul_ps(x3, x2);       // x⁵
...
{
    baseFreq = 440.0 * pow(2.0, (note - 69) / 12.0);  // MIDI to Hz
...
void Synth::noteOff(int channel, int note, int velocity)
{
    ...
    if (note == currentNote) {
        baseFreq = 0.0f;  // silence the oscillator
        currentNote = -1;
    }
...
void Synth::pitchBend(int channel, int bendValue)
{
    ...
    if (currentNote >= 0) {
        baseFreq = 440.0f * pow(2.0, (currentNote - 69 + bendSemis) / 12.0);
    }
}
