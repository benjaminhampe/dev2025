...
#include <commctrl.h>	// Common controls unit
#include <commdlg.h>	// Common dialogs unit
...
15Apr16 LdB
--------------------------------------------------------------------------*/
void DrawGLScene(GLDATABASE* db)
{
...
int makeAbsolutePath(const TCHAR* filename, TCHAR* absolutePath, DWORD bufferSize)
{
    DWORD result = GetFullPathName(
        filename,        // input: relative or absolute filename
        bufferSize,      // size of output buffer
        absolutePath,    // output: absolute path
        NULL             // optional: pointer to file part
    );

    if (result == 0 || result > bufferSize) {
        // failed or buffer too small
        return 0;
    }
    return 1;
}
...
GLuint BMP2GLTexture(TCHAR* fileName, HWND Wnd, GLDATABASE* db)
{
  ...
  if (makeAbsolutePath(fileName, absolute, MAX_PATH))
  {
    _tprintf(_T("Absolute path: %s\n"), absolute);
  }
  else
  {
    _tprintf(_T("Failed to resolve path\n"));
  }
  ...
  hBMP = (HBITMAP) LoadImage(
    ...
    0,
    0,
    LR_CREATEDIBSECTION | LR_LOADFROMFILE); // Load the bitmap from file
...
LRESULT CALLBACK OpenGLDemoHandler (HWND Wnd, UINT Msg, WPARAM wParam, LPARAM lParam)
{
  switch (Msg)
  {
    ...
    { // WM_CREATE MESSAGE
      ...
      db->glTexture = BMP2GLTexture(_T("media\\OpenGLAppDemo\\Dragons.bmp"), Wnd, db);
    ...
    { // WM_PAINT MESSAGE
      ...
      wglMakeCurrent(Ps.hdc, db->Rc); // Make our render context current
      DrawGLScene(db); // Draw the OpenGL scene
    ...
    { // WM_TIMER MESSAGE
      GLDATABASE* db = (GLDATABASE*) GetProp(Wnd, DATABASE_PROPERTY);// Fetch the data base
      db->xrot += .1f; // Inc x rotation
      db->yrot += .15f; // Inc y rotation
    ...
    default:
    {
      return DefWindowProc(Wnd, Msg, wParam, lParam); // Default handler
    }
  }
  return 0;
}
