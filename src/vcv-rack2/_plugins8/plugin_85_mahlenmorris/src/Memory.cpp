          ...
          while (module_buffer_queue->tasks.pop(task) && !shutdown) {
            switch (task->type) {
              case BufferTask::REPLACE_AUDIO: {
                RK_WARN("There should not be a REPLACE_AUDIO task on the module_buffer_queue!");
                delete task;
              }
              break;
        ...
        if (prepare_buffer_queue->tasks.size() > 0) {
          ...
          while (prepare_buffer_queue->tasks.pop(task) && !shutdown) {
            switch (task->type) {
              ...
              break;
              case BufferTask::SAVE_FILE: {
                RK_WARN("There should not be a SAVE_FILE task on the prepare_buffer_queue!");
                delete task;
              }
              break;
            }
        ...
        if (buffer->replacements.queue.size() > 0) {
          ...
          while (buffer->replacements.queue.pop(task) && !shutdown) {
            switch (task->type) {
              ...
              break;
              case BufferTask::SAVE_FILE: {
                RK_WARN("There should not be a SAVE_FILE task on the 'replacements' queue!");
                delete task;
              }
              break;
            }
        ...
        while (module_file_queue->tasks.pop(task) && !shutdown) {
          switch (task->type) {
            ...
            case PrepareTask::SAVE_FILE: {
              RK_WARN("There should not be a SAVE_FILE task on the module_file_queue!");
              delete task;
            }
            break;
          }
    ...
    if (!buffer_initialized) {
      ...
      if (buffer_module_queue.tasks.size() > 0) {
        ...
        while (buffer_module_queue.tasks.pop(task)) {
          switch (task->type) {
            case PrepareTask::LOAD_FILE: {
              RK_WARN("There should not be a LOAD_FILE task on the buffer_module_queue!");
              delete task;
            }
            break;

            case PrepareTask::SAVE_FILE: {
              RK_WARN("There should not be a SAVE_FILE task on the buffer_module_queue!");
              delete task;
            }
            break;
            ...
            case PrepareTask::MAKE_BLANK: {
              RK_WARN("There should not be a MAKE_BLANK task on the buffer_module_queue!");
              delete task;
            }
            break;
          }
      ...
      if (widget_module_queue.tasks.size() > 0) {
        ...
        while (widget_module_queue.tasks.pop(task)) {
          switch (task->type) {
            ...
            case PrepareTask::MAKE_BLANK: {
              RK_WARN("There should not be a MAKE_BLANK task on the widget_module_queue!");
              delete task;
            }
            break;
          }
...
struct MemoryCV : Memory {
  MemoryCV() : Memory() {
    ...
    if (buffer) {  // Pretty convinced this is always the case.
      ...
      RK_WARN("MemoryCV(): buffer is null!");
    }
  }
};
