...
struct XTModule : public rack::Module, public SurgeStorage::ErrorListener
{
    ...
    void showBuildInfo()
    {
        RK_INFO("[SurgeXTRack] Instance: Module=%s BuildInfo=%s", getName().c_str(),
             getBuildInfo().c_str());
    }
    ...
    void guaranteeRackUserWavetablesDir()
    {
        ...
        {
            RK_INFO("Failed to create FS Dir: %s", e.what());
        }
    }
    ...
    {
        RK_WARN("Surge Reported an Error");
        RK_WARN("%s", title.c_str());
        RK_WARN("%s", msg.c_str());
    }
