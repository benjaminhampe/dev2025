...
struct RACK_DLL_API Vec {
    ...
    */
    ...
    Vec clamp(const Rect& bound) const;
    Vec clampSafe(const Rect& bound) const;
    Vec crossfade(Vec b, float p) {
        return this->plus(b.minus(*this).mult(p));
    }

    // Method aliases
    bool isEqual(const Vec& b) const {
        return equals(b);
    }
};
...
struct RACK_DLL_API Rect {
    ...
    */
    static Rect fromMinMax(const Vec& a, const Vec& b) {
        return Rect(a, b.minus(a));
    }
    /** Constructs a Rect from any two opposite corners.
    */
    static Rect fromCorners(const Vec& a, const Vec& b) {
    ...
    */
    ...
    Rect clamp(const Rect& bound) const {
        Rect r;
#if 0
...
#else // Benni
        const float x2 = bound.pos.x + std::abs(bound.size.x);
        const float y2 = bound.pos.y + std::abs(bound.size.y);
        r.pos.x = math::clamp(pos.x, bound.pos.x, x2);
        r.pos.y = math::clamp(pos.y, bound.pos.y, y2);
        r.size.x = math::clamp(pos.x + size.x, bound.pos.x, x2) - r.pos.x;
        r.size.y = math::clamp(pos.y + size.y, bound.pos.y, y2) - r.pos.y;
#endif
        return r;
    }
    /** Nudges the position to fix inside a bounding box. */
    Rect nudge(const Rect& bound) const {
        Rect r;
        r.size = size;
    ...
    Rect expand(const Rect& b) const {
        Rect r;
    ...
    /** Expands each corner. */
    Rect grow(const Vec& delta) const {
        Rect r;
    ...
    Rect shrink(const Vec& delta) const {
        Rect r;
    ...
    /** Returns `pos + size * p` */
    Vec interpolate(const Vec& p) {
        return pos.plus(size.mult(p));
    }

    // Method aliases
    bool isContaining(const Vec& v) const {
        return contains(v);
    }
...
inline Vec Vec::clamp(const Rect& bound) const {
    return Vec(
...
inline Vec Vec::clampSafe(const Rect& bound) const {
#if 0
    return Vec(
...
#else // Benni
    return Vec(
        std::clamp(x, bound.pos.x, bound.pos.x + std::abs(bound.size.x)),
        std::clamp(y, bound.pos.y, bound.pos.y + std::abs(bound.size.y))
    );
#endif
}
