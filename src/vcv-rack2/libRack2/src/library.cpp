...
void RACK_DLL_CALL checkAppUpdate() {
    ...
    if (!resJ) {
        RK_WARN("Request for version failed");
        return;
    }
...
bool RACK_DLL_CALL isAppUpdateAvailable() {
    return (appVersion != "");
}


bool RACK_DLL_CALL isLoggedIn() {
    return settings::token != "";
}


void RACK_DLL_CALL logIn(std::string email, std::string password) {
...
void RACK_DLL_CALL checkUpdates() {
    if (!updateMutex.try_lock())
        return;
    DEFER({updateMutex.unlock();});
    ...
    if (!userResJ) {
        RK_WARN("Request for user account failed");
    ...
    if (userErrorJ) {
        ...
        RK_WARN("Request for user account error: %s", userError.c_str());
        // Unset token
        settings::token = "";
        refreshRequested = true;
        return;
    ...
    if (!manifestsResJ) {
        RK_WARN("Request for library manifests failed");
    ...
    if (!modulesResJ) {
        RK_WARN("Request for user's modules failed");
    ...
    json_object_foreach(pluginsJ, modulesKey, modulesJ) {
        ...
        if (!versionJ) {
            // RK_WARN("Plugin %s has no version in manifest", pluginSlug.c_str());
            continue;
        }
...
void RACK_DLL_CALL syncUpdate(std::string slug) {
    if (!updateMutex.try_lock())
        return;
    DEFER({updateMutex.unlock();});
    ...
    RK_INFO("Downloading plugin %s v%s for %s-%s", slug.c_str(), update.version.c_str(), APP_OS.c_str(), APP_CPU.c_str());
    ...
    if (!network::requestDownload(downloadUrl, packagePath, &updateProgress, getTokenCookies())) {
        RK_WARN("Plugin %s download was unsuccessful", slug.c_str());
        return;
    }
...
void RACK_DLL_CALL syncUpdates() {
    if (settings::token.empty())
        return;
