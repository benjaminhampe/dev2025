...
struct EngineWorker {
	...
	void start() {
		if (running) {
			RK_WARN("Engine worker already started");
			return;
		}
		running = true;
		...
		err = pthread_create(&thread, NULL, [](void* p) -> void* {
			...
			// 	RK_DEBUG("EngineWorker %d thread launched with policy %d priority %d", that->id, policy, param.sched_priority);
			// }
		...
		if (err) {
			RK_WARN("EngineWorker %d thread could not be started: %s", id, strerror(err));
		}
	}
...
void Engine::fromJson(json_t* rootJ) {
	...
	json_array_foreach(modulesJ, moduleIndex, moduleJ) {
		...
		try {
			...
			RK_WARN("Cannot load model: %s", e.what());
			continue;
		}
		...
		try {
			...
			RK_WARN("Cannot load module: %s", e.what());
			delete module;
			continue;
		}
	...
	json_array_foreach(cablesJ, cableIndex, cableJ) {
		...
		try {
			...
			RK_WARN("Cannot load cable: %s", e.what());
			delete cable;
			continue;
		}
	}
