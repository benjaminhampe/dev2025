...
struct RtAudioDriver : audio::Driver {
    ...
    RtAudioDriver(RtAudio::Api api, std::string name) {
        ...
        RK_INFO("Creating RtAudio %s driver", name.c_str());
        rtAudio = new RtAudio(api, [](RtAudioErrorType type, const std::string& errorText) {
            RK_WARN("RtAudio error %d: %s", type, errorText.c_str());
        });
...
struct RtAudioDevice : audio::Device {
    ...
    RtAudioDevice(RtAudioDriver* driver, int deviceId) {
        ...
        RK_INFO("Creating RtAudio %s device", driver->getName().c_str());
    ...
    void openStream() {
        ...
        RK_INFO("Opening RtAudio %s device %d: %s (%d in, %d out, %d sample rate, %d block size)", driver->getName().c_str(), deviceId, deviceInfo.name.c_str(), inputParameters.nChannels, outputParameters.nChannels, closestSampleRate, blockSize);
        ...
        try {
            RK_INFO("Starting RtAudio %s device %d", driver->getName().c_str(), deviceId);
    ...
    void closeStream() {
        if (rtAudio->isStreamRunning()) {
            RK_INFO("Stopping RtAudio %s device %d", driver->getName().c_str(), deviceId);
            rtAudio->stopStream();
        }
        if (rtAudio->isStreamOpen()) {
            RK_INFO("Closing RtAudio %s device %d", driver->getName().c_str(), deviceId);
            rtAudio->closeStream();
        }
