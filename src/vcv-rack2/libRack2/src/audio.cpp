...
void Port::setDriverId(int driverId) {
    ...
    try {
        ...
        RK_WARN("Audio port could not get default device ID: %s", e.what());
    }
}
...
std::string Port::getDriverName() {
    ...
    try {
        ...
        RK_WARN("Audio port could not get driver name: %s", e.what());
        return "";
    }
}
...
std::vector<int> Port::getDeviceIds() {
    ...
    try {
        ...
        RK_WARN("Audio port could not get device IDs: %s", e.what());
        return {};
    }
}
...
void Port::setDeviceId(int deviceId) {
    ...
    if (this->deviceId >= 0) {
        try {
            ...
            RK_WARN("Audio port could not unsubscribe from device: %s", e.what());
        }
    }
    device = NULL;
    this->deviceId = -1;
    ...
    if (deviceId >= 0) {
        try {
            ...
            RK_WARN("Audio port could not subscribe to device: %s", e.what());
        }
    }
}
...
int Port::getDeviceNumInputs(int deviceId) {
    ...
    try {
        ...
        RK_WARN("Audio port could not get device number of inputs: %s", e.what());
        return 0;
    }
}
...
int Port::getDeviceNumOutputs(int deviceId) {
    ...
    try {
        ...
        RK_WARN("Audio port could not get device number of outputs: %s", e.what());
        return 0;
    }
}
...
std::string Port::getDeviceName(int deviceId) {
    ...
    try {
        ...
        RK_WARN("Audio port could not get device name: %s", e.what());
        return 0;
    }
}
...
std::set<float> Port::getSampleRates() {
    ...
    try {
        ...
        RK_WARN("Audio port could not get device sample rates: %s", e.what());
        return {};
    }
}
...
float Port::getSampleRate() {
    ...
    try {
        ...
        RK_WARN("Audio port could not get device sample rate: %s", e.what());
        return 0;
    }
}
...
void Port::setSampleRate(float sampleRate) {
    ...
    try {
        ...
        RK_WARN("Audio port could not set device sample rate: %s", e.what());
    }
}
...
std::set<int> Port::getBlockSizes() {
    ...
    try {
        ...
        RK_WARN("Audio port could not get device block sizes: %s", e.what());
        return {};
    }
}
...
int Port::getBlockSize() {
    ...
    try {
        ...
        RK_WARN("Audio port could not get device block size: %s", e.what());
        return 0;
    }
}
...
void Port::setBlockSize(int blockSize) {
    ...
    try {
        ...
        RK_WARN("Audio port could not set device block size: %s", e.what());
    }
}
...
void RACK_DLL_CALL addDriver(int driverId, Driver* driver) {
    assert(driver);
    assert(driverId != -1);
    drivers.push_back(std::make_pair(driverId, driver));
}

std::vector<int> RACK_DLL_CALL getDriverIds() {
...
Driver* RACK_DLL_CALL getDriver(int driverId) {
    if (driverId == -1)
        return NULL;
