...
static void unarchiveToDirectory(const std::string& archivePath, const std::vector<uint8_t>* archiveData, const std::string& dirPathStr) {
    ...
    for (;;) {
        ...
        // RK_DEBUG("entryPath: %s", entryPath.generic_u8string().c_str());
...
void RACK_DLL_CALL openBrowser(const std::string& url) {
    ...
    RK_INFO("Opening browser URL %s", url.c_str());
...
void RACK_DLL_CALL openDirectory(const std::string& path) {
    ...
    RK_INFO("Opening directory %s", path.c_str());
