...
static void modelDbInit() {
	...
	for (plugin::Plugin* plugin : plugin::plugins) {
		...
		for (plugin::Model* model : plugin->models) {
			...
			// RK_DEBUG("%s; %s; %s; %s; %s", fields[0].c_str(), fields[1].c_str(), fields[2].c_str(), fields[3].c_str(), fields[4].c_str());
	...
	void refresh() {
		...
		if (search.empty()) {
			...
			for (auto& result : results) {
				...
				// RK_DEBUG("%s %s\t\t%f", result.key->plugin->slug.c_str(), result.key->slug.c_str(), result.score);
			}
			// Sort by score
			sortModels([&](ModelBox* m) {
			...
			if (e.isKeyCommand(GLFW_KEY_F2, RACK_MOD_CTRL | GLFW_MOD_SHIFT | GLFW_MOD_ALT)) {
				...
				for (widget::Widget* w : modelContainer->children) {
					...
					if (!mb->visible)
						continue;
					count++;
					RK_DEBUG("Dumping into rack (%d): %s/%s", count, mb->model->plugin->slug.c_str(), mb->model->slug.c_str());
