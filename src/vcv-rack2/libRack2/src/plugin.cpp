...
Plugin* RACK_DLL_CALL getPlugin(const std::string& pluginSlug) {
    if (pluginSlug.empty())
        return NULL;
...
Plugin* RACK_DLL_CALL getPluginFallback(const std::string& pluginSlug) {
    if (pluginSlug.empty())
        return NULL;
...
Model* RACK_DLL_CALL getModel(const std::string& pluginSlug, const std::string& modelSlug) {
...
Model* RACK_DLL_CALL getModelFallback(const std::string& pluginSlug, const std::string& modelSlug) {
    if (pluginSlug.empty() || modelSlug.empty())
        return NULL;
...
Model* RACK_DLL_CALL modelFromJson(json_t* moduleJ) {
    // Get slugs
...
bool RACK_DLL_CALL isSlugValid(const std::string& slug) {
    for (char c : slug) {
...
std::string RACK_DLL_CALL normalizeSlug(const std::string& slug) {
    std::string s;
    for (char c : slug) {
        if (!(std::isalnum(c) || c == '-' || c == '_'))
            continue;
