...
json_t* RACK_DLL_CALL requestJson(Method method, const std::string& url, json_t* dataJ, const CookieMap& cookies) {
    std::string urlS = url;
...
bool RACK_DLL_CALL requestDownload(const std::string& url, const std::string& filename, float* progress, const CookieMap& cookies) {
    CURL* curl = createCurl();
...
std::string RACK_DLL_CALL encodeUrl(const std::string& s) {
    CURL* curl = createCurl();
...
std::string RACK_DLL_CALL urlPath(const std::string& url) {
    CURLU* curl = curl_url();
