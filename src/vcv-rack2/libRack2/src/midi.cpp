...
std::vector<int> Input::getDeviceIds() {
    ...
    try {
        ...
        RK_WARN("MIDI port could not get input device IDs: %s", e.what());
        return {};
    }
}
...
int Input::getDefaultDeviceId() {
    ...
    try {
        ...
        RK_WARN("MIDI port get default input device ID: %s", e.what());
        return -1;
    }
}
...
void Input::setDeviceId(int deviceId) {
    ...
    if (driver && this->deviceId >= 0) {
        try {
            ...
            RK_WARN("MIDI port could not unsubscribe from input: %s", e.what());
        }
    }
    ...
    if (driver && deviceId >= 0) {
        try {
            ...
            RK_WARN("MIDI port could not subscribe to input: %s", e.what());
        }
    }
}
...
std::string Input::getDeviceName(int deviceId) {
    ...
    try {
        ...
        RK_WARN("MIDI port could not get input device name: %s", e.what());
        return "";
    }
}
...
std::vector<int> Output::getDeviceIds() {
    ...
    try {
        ...
        RK_WARN("MIDI port could not get output device IDs: %s", e.what());
        return {};
    }
}
...
void Output::setDeviceId(int deviceId) {
    ...
    if (driver && this->deviceId >= 0) {
        try {
            ...
            RK_WARN("MIDI port could not unsubscribe from output: %s", e.what());
        }
    }
    ...
    if (driver && deviceId >= 0) {
        try {
            ...
            RK_WARN("MIDI port could not subscribe to output: %s", e.what());
        }
    }
}
...
int Output::getDefaultDeviceId() {
    ...
    try {
        ...
        RK_WARN("MIDI port get default output device ID: %s", e.what());
        return -1;
    }
}
...
std::string Output::getDeviceName(int deviceId) {
    ...
    try {
        ...
        RK_WARN("MIDI port could not get output device name: %s", e.what());
        return "";
    }
}
...
void Output::sendMessage(const Message& message) {
    ...
    // RK_DEBUG("sendMessage %02x %02x %02x", msg.cmd, msg.data1, msg.data2);
    try {
        outputDevice->sendMessage(msg);
    }
    catch (Exception& e) {
        // Don't log error because it could flood the log.
        // RK_WARN("MIDI port could not be sent MIDI message: %s", e.what());
