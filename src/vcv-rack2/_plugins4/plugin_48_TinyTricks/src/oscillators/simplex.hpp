...
struct SimplexOscillator {
  ...
  void setPitch(float pitch) {
    freq = rack::dsp::FREQ_C4 * powf(2.0f, pitch);
  }
  ...
  float getNormalizedOsc(float detaillevel, float x, float y, float z, float scale) {
    ...
    if (tick % 256 == 0) {
      auto result = std::minmax_element(std::begin(buffer), std::end(buffer));
      min = *result.first;
      max = *result.second;
    }
    tick++;

    value = std::clamp(rack::math::rescale(value, min, max, -1.f, 1.f), -1.f, 1.f);
    return value * 5.f;;
