...
json_t* requestJson(Method method, const std::string& url, json_t* dataJ, const CookieMap& cookies) {
	...
	// Perform request
	RK2_INFO("Requesting JSON %s %s", methodNames[method].c_str(), urlS.c_str());
	...
	if (res != CURLE_OK) {
		RK2_WARN("Could not request %s: %s", urlS.c_str(), curl_easy_strerror(res));
		return NULL;
	}
...
bool requestDownload(const std::string& url, const std::string& filename, float* progress, const CookieMap& cookies) {
	...
	RK2_INFO("Requesting download %s", url.c_str());
	CURLcode res = curl_easy_perform(curl);
	curl_easy_cleanup(curl);
	...
	if (res != CURLE_OK) {
		...
		RK2_WARN("Could not download %s: %s", url.c_str(), curl_easy_strerror(res));
		return false;
	}
#endif
	return true;
