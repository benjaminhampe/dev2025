...
static void fatalSignalHandler(int sig) {
	...
	RK2_FATAL("Fatal signal %d. Stack trace:\n%s", sig, stackTrace.c_str());
...
int main(int argc, char* argv[]) {
	...
	// Log environment
	RK2_INFO("%s", appInfo.c_str());
	RK2_INFO("%s", system::getOperatingSystemInfo().c_str());
	std::string argsList;
	for (int i = 0; i < argc; i++) {
		argsList += argv[i];
		argsList += " ";
	}
	RK2_INFO("Args: %s", argsList.c_str());
	if (settings::devMode)
		RK2_INFO("Development mode");
	RK2_INFO("System directory: %s", asset::systemDir.c_str());
	RK2_INFO("User directory: %s", asset::userDir.c_str());
#if defined ARCH_MAC
	RK2_INFO("Bundle path: %s", asset::bundlePath.c_str());
#endif
	RK2_INFO("System time: %s", string::formatTimeISO(system::getUnixTime()).c_str());
	...
	RK2_INFO("Initializing network");
	network::init();
	RK2_INFO("Initializing audio");
	audio::init();
	rtaudioInit();
#if defined ARCH_MAC
...
#endif
	RK2_INFO("Initializing MIDI");
	midi::init();
	rtmidiInit();
	keyboard::init();
	gamepad::init();
	midiloopback::init();
	RK2_INFO("Initializing plugins");
	plugin::init();
	RK2_INFO("Initializing browser");
	app::browserInit();
	RK2_INFO("Initializing library");
	library::init();
	if (!settings::headless) {
		RK2_INFO("Initializing UI");
		ui::init();
		RK2_INFO("Initializing window");
		window::init();
	}

	// Initialize context
	contextSet(new Context);
	RK2_INFO("Creating MIDI loopback");
	APP->midiLoopbackContext = new midiloopback::Context;
	RK2_INFO("Creating engine");
	APP->engine = new engine::Engine;
	RK2_INFO("Creating history state");
	APP->history = new history::State;
	RK2_INFO("Creating event state");
	APP->event = new widget::EventState;
	RK2_INFO("Creating scene");
	APP->scene = new app::Scene;
	APP->event->rootWidget = APP->scene;
	RK2_INFO("Creating patch manager");
	APP->patch = new patch::Manager;
	if (!settings::headless) {
		RK2_INFO("Creating window");
	...
	if (settings::headless) {
		...
		RK2_INFO("Taking screenshots of all modules at %gx zoom", screenshotZoom);
		APP->window->screenshotModules(asset::user("screenshots"), screenshotZoom);
	}
	else {
		RK2_INFO("Running window");
		APP->window->run();
		RK2_INFO("Stopped window");

		// RK2_INFO("Destroying window");
		// delete APP->window;
		// APP->window = NULL;
		// RK2_INFO("Re-creating window");
		// APP->window = new window::Window;
		// APP->window->run();
	}

	// Destroy context
	RK2_INFO("Deleting context");
	delete APP;
	contextSet(NULL);
	if (!settings::headless) {
		settings::save();
	...
	if (!settings::headless) {
		RK2_INFO("Destroying window");
		window::destroy();
		RK2_INFO("Destroying UI");
		ui::destroy();
	}
	RK2_INFO("Destroying library");
	library::destroy();
	RK2_INFO("Destroying MIDI");
	midi::destroy();
	RK2_INFO("Destroying audio");
	audio::destroy();
	RK2_INFO("Destroying plugins");
	plugin::destroy();
	RK2_INFO("Destroying network");
	network::destroy();
	settings::destroy();
	RK2_INFO("Destroying logger");
	logger::destroy();
