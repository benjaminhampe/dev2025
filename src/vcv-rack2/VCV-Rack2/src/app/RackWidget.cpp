...
struct PlugContainer : widget::TransparentWidget {
	...
	void drawLayer(const DrawArgs& args, int layer) override {
		if (layer == 2) {
			...
			nvgSave(args.vg);
#if 1
            nvgFillColor(args.vg, color::WHITE);       // For fill operations
            nvgStrokeColor(args.vg, color::WHITE);     // For stroke operations
            nvgGlobalAlpha(args.vg, 1.0f);   // If you want to apply global transparency
...
#if 1
    nvgFillColor(args.vg, nvgRGBAf(b, b, b, 1));       // For fill operations
    nvgStrokeColor(args.vg, nvgRGBAf(b, b, b, 1));     // For stroke operations
    nvgGlobalAlpha(args.vg, 1.0f);   // If you want to apply global transparency
#else
...
void RackWidget::mergeJson(json_t* rootJ) {
	...
	json_array_foreach(modulesJ, moduleIndex, moduleJ) {
		...
		if (!mw) {
			RK2_WARN("Cannot find ModuleWidget %lld", (long long) id);
			continue;
		}
	...
	json_array_foreach(cablesJ, cableIndex, cableJ) {
		...
		if (!cw) {
			RK2_WARN("Cannot find CableWidget %lld", (long long) id);
			continue;
		}
...
void RackWidget::fromJson(json_t* rootJ) {
	...
	json_array_foreach(modulesJ, moduleIndex, moduleJ) {
		...
		if (!module) {
			RK2_WARN("Cannot find Module %lld", (long long) id);
			continue;
		}

		// Create ModuleWidget
		RK2_INFO("Creating module widget %s", module->model->getFullName().c_str());
	...
	json_array_foreach(cablesJ, cableIndex, cableJ) {
		...
		if (!cable) {
			RK2_WARN("Cannot find Cable %lld", (long long) id);
			continue;
		}
	...
	if (cablesJ) {
		...
		json_array_foreach(cablesJ, cableIndex, cableJ) {
			...
			try {
				...
				RK2_WARN("Cannot paste cable: %s", e.what());
				delete cable;
				continue;
			}
...
void RackWidget::pasteClipboardAction() {
	...
	if (!json) {
		RK2_WARN("Could not get text from clipboard.");
		return;
	}
	...
	if (!rootJ) {
		RK2_WARN("JSON parsing error at %s %d:%d %s", error.source, error.line, error.column, error.text);
		return;
	}
...
void RackWidget::loadSelection(std::string path) {
	...
	RK2_INFO("Loading selection %s", path.c_str());
...
void RackWidget::saveSelection(std::string path) {
	RK2_INFO("Saving selection %s", path.c_str());
