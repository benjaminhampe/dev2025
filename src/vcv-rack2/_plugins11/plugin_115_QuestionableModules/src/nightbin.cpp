...
struct NightbinButton : ui::Button {
            ...
            if (json_t* array = json_object_get(json, "assets")) {
                ...
                json_array_foreach(array, key, value) {
                    if (json_t* url = json_object_get(value, "browser_download_url")) {
                        ...
                        RK_INFO("[QuestionableModules::NightBin] Found download %s-%s %s", info.os.c_str(), info.arch.c_str(), download.c_str());

                        RK_INFO("[QuestionableModules::NightBin] looking for %s-%s", APP_OS.c_str(), APP_CPU.c_str());
                        if (info.arch == APP_CPU && info.os == APP_OS) {
                            RK_INFO("[QuestionableModules::NightBin] Found correct dl for arch %s", info.version.c_str());
    ...
    void downloadUpdate(QRemotePluginInfo info) {
        ...
        RK_INFO("Downloading %s to %s", info.name.c_str(), packagePath.c_str());
        if (!q::network::requestDownload(info.dlURL, packagePath, &progress, headers, cookies)) {
            RK_WARN("Download failed :(");
        } else {
    ...
    QRemotePluginInfo getPluginRemoteInfo(Plugin* plugin) {
        ...
        RK_INFO("checking for builds at: %s", api.c_str());
        if (!api.size()) {
            RK_WARN("Failed to get api string for module: %s, sourceURL: %s", plugin->name.c_str(), plugin->sourceUrl.c_str());
        ...
        if (!request) {
            RK_WARN("Request for github release info failed");
        ...
        if (json_t* msg = json_object_get(request, "message")) {
            ...
            if (message.find("API rate limit exceeded") != std::string::npos) {
                RK_WARN("Request for github rate limited, consider setting your gitPersonalAccessToken");
...
struct NightBinWidget : QuestionableWidget {
    ...
    void setupMenuBar() {
        ...
        } else RK_WARN("Unable to add to racks menubar, could not find ui::SequentialLayout reference.");
    }
