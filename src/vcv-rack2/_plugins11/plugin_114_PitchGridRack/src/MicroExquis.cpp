...
struct MicroExquis : Module {
    ...
    MicroExquis() {
        ...
        if (MTS_CanRegisterMaster()){
            ...
            RK_INFO("MTS ESP Master registered");
        }else{
            bool has_ipc = MTS_HasIPC();
            int num_clients = MTS_GetNumClients();
            RK_INFO("MTS ESP Master not registered. IPC: %d, Num Clients: %d", has_ipc, num_clients);
            if (has_ipc && num_clients == 0){
                MTS_Reinitialize();
                RK_INFO("MTS ESP Reinitialized");
                if (MTS_CanRegisterMaster()){
                    MTS_RegisterMaster();
                    is_mts_esp_master = true;
                    RK_INFO("MTS ESP Master registered");
        ...
        if (exquis.connected){
            RK_INFO("Exquis connected. Initializing...");
            exquis.initialize();
            RK_INFO("Exquis initialized.");
        }
        else{
            RK_INFO("Exquis not found.");
        }
    ...
    ~MicroExquis(){
        if (exquis.connected){
            ...
            RK_INFO("Exquis disconnected");
        }
        if (is_mts_esp_master){
            MTS_DeregisterMaster();
            RK_INFO("MTS ESP Master unregistered");
        }
    ...
    void dataFromJson(json_t* rootJ) override {
        ...
        if (scaleJ && json_is_object(scaleJ)){
            ...
            if (
                ...
                RK_INFO("Scale loaded: %d %d %d", x, y, mode);
            }
        }
        ...
        if (layoutJ && json_is_object(layoutJ)){
            ...
            if (
                ...
                RK_INFO("Layout loaded: %d %d %d %d %d %d", baseX, baseY, interval1X, interval1Y, interval2X, interval2Y);
            }
        }
        exquis.showAllOctavesLayer();
    }
