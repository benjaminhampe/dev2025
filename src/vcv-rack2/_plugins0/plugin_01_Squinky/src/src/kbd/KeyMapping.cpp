...
KeyMapping::KeyMapping(const std::string& configPath)
{
    ...
    if (bindings) {
        if (json_is_array(bindings)) {
            ...
            RK_INFO("bindings is not an array");
            return;
        }
    } else {
        RK_INFO("bindings not found at root");
        return;
    }
    ...
    if (ignoreCase) {
        if (!json_is_array(ignoreCase)) {
            RK_INFO("ignoreCase is not an array");
            return;
        }
        size_t index;
        json_t* value;
...
Actions::action KeyMapping::parseAction(Actions& actions, json_t* binding)
{
    ...
    if (!keyJ) {
        RK_WARN("binding does not have action field: %s\n", json_dumps(keyJ, 0));
        return nullptr;
    }
    if (!json_is_string(keyJ)) {
        RK_WARN("binding action is not a string: %s\n", json_dumps(keyJ, 0));
        return nullptr;
    }
