...
SqKeyPtr SqKey::parse(json_t* binding)
{
    ...
    if (!keyJ) {
        RK_WARN("Binding does not have key field");
        return nullptr;
    }
    if (!json_is_string(keyJ)) {
        RK_WARN("Binding key is not a string: %s", json_dumps(keyJ, 0));
        return nullptr;
    }
    ...
    while (getline(f, s, '+')) {
        if (s == "ctrl") {
            ...
            RK_WARN("can't parse key fragment %s of %s\n", s.c_str(), keyString.c_str());
            return nullptr;
        }
    }
    if (key == 0) {
        RK_WARN("binding does not have valid key: %s\n", keyString.c_str());
        return nullptr;
    }
