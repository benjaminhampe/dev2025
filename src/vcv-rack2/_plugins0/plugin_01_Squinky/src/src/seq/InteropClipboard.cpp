    ...
    MidiSelectionModelPtr sel)
{
    ...
    if (!clipTrack) {
        RK_WARN("get exit early for incompatible data");
        return PasteData();
    }
...
MidiTrackPtr  InteropClipboard::fromJsonStringToTrack(const std::string& json, MidiLockPtr lock)
{
    ...
    if (!clipboardJson) {
        RK_WARN("no sequence data found at root");
        return nullptr;
    }
    ...
    if (notesJson && jsonLength && json_is_number(jsonLength)) {
        ...
        if (!notesJson) RK_WARN("notes missing from sequence clip");
        if (!jsonLength) RK_WARN("length missing from sequence clip");
        return nullptr;
    }
...
MidiTrackPtr InteropClipboard::fromJsonToTrack(MidiLockPtr lock, json_t *notesJson, float length )
{
    ...
    if (!(json_is_array(notesJson))) {
        RK_WARN("clipboard: notes is not an array");
        return nullptr;
    }
...
MidiEventPtr InteropClipboard::fromJsonEvent(json_t *data)
{
    ...
    if (!typeJson) {
        RK_WARN("clipboard: event has no type");
        return event;
    }
...
MidiNoteEventPtr InteropClipboard::fromJsonNoteEvent(json_t *data)
{
    ...
    if (!json_is_number(pitchJson)) {
        RK_WARN("clipboard: note.pitch is not a number");
        return nullptr;
    }
    if (!json_is_number(durationJson)) {
        RK_WARN("clipboard: note.length is not a number");
        return nullptr;
    }
    if (!json_is_number(startJson)) {
        RK_WARN("clipboard: note.start is not a number");
        return nullptr;
    }
    ...
    if (note->duration < 0) {
        RK_WARN("clipboard: note.length < 0");
        return nullptr;
    }

    if (note->startTime < 0) {
        RK_WARN("clipboard: note.start < 0");
        return nullptr;
    }
    return note;
}
