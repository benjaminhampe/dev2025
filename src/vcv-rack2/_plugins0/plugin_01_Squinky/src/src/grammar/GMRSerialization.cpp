...
StochasticProductionRuleEntryPtr GMRSerialization::readRuleEntry(json_t* entryJson) {
    ...
    if (!prob) {
        RK_WARN("rule has no probability");
        return nullptr;
    }

    if (!json_is_number(prob)) {
        RK_WARN("probability not a number");
        return nullptr;
    }
    ...
    if (!rhs) {
        RK_WARN("rule rhs not present");
        return nullptr;
    }
    if (!json_is_array(rhs)) {
        RK_WARN("rhs is not an array");
        return nullptr;
    }
    ...
    json_array_foreach(rhs, index, prod) {
        if (!json_is_string(prod)) {
            RK_WARN("production not a string");
            return nullptr;
        }
...
StochasticProductionRulePtr GMRSerialization::readRule(json_t* r) {
    ...
    if (!lhs) {
        RK_WARN("rule has no lhs");
        return nullptr;
    }
    if (!json_is_string(lhs)) {
        RK_WARN("rule lhs not string");
        return nullptr;
    }
    ...
    if (!lhsNote.isValid()) {
        RK_WARN("can't convert %s into note", sLhs.c_str());
        return nullptr;
    }
    ...
    json_array_foreach(ents, index, entry) {
        if (!json_is_object(entry)) {
            RK_WARN("production entry not an object");
            return nullptr;
        }
...
StochasticGrammarPtr GMRSerialization::readGrammar(json_t* theJson) {
    ...
    if (!rules) {
        RK_WARN("Json grammar has no rules");
        return nullptr;
    }
    if (!json_is_array(rules)) {
        RK_WARN("rules is not an array");
        return nullptr;
    }
