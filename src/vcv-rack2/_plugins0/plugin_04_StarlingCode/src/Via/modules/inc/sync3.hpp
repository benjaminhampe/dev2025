...
#endif
#ifdef BUILD_VIRTUAL
    struct Sync3Scale * scales = nullptr;

    uint32_t * numerators = nullptr;
    uint32_t * denominators = nullptr;
    uint32_t * dividedPhases = nullptr;
...
//			phaseModTracker = phaseModTracker3 + phaseModIncrement2 * playbackPosition;
    ...
    ViaSync3(std::string binPath) : sync3UI(*this) {
        ...
        numerators = scales[0].numerators;
        denominators = scales[0].denominators;
        dividedPhases = scales[0].dividedPhases;
        keys = scales[0].keys;
        #endif
