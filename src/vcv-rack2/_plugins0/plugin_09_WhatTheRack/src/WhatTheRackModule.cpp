...
void SpawnModel(Model* model, bool randomize = false) {
  RK_INFO("WhatTheRack will spawn a %s/%s module.", model->plugin->slug.c_str(), model->slug.c_str());
  Module *module = model->createModule();
  ModuleWidget *moduleWidget = model->createModuleWidget(module);
  if (!moduleWidget) {
    RK_WARN("WhatTheRack was unable to spawn a %s/%s module.", model->plugin->slug.c_str(), model->slug.c_str());
    return;
  }
  ...
  RK_INFO("WhatTheRack successfully spawned a %s/%s module.", model->plugin->slug.c_str(), model->slug.c_str());
}
