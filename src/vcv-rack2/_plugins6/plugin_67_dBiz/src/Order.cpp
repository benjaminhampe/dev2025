...
///////////////////////////////////
  ...
  Order()
  {
    ...
    for (int i = 0; i < 4; i++)
    {
        configParam(G1_ATT_PARAM + i, -5.0, 5.0, 0, rack::string::f("Group A%d Att",i+1));
        configParam(G2_ATT_PARAM + i, -5.0, 5.0, 0, rack::string::f("Group B%d Att",i+1));
        configParam(G3_ATT_PARAM + i, -5.0, 5.0, 0, rack::string::f("Group C%d Att",i+1));
        configParam(G4_ATT_PARAM + i, -5.0, 5.0, 0, rack::string::f("Group D%d Att",i+1));

        configOutput(G1_ATT_OUTPUT+i, rack::string::f("Group A%d",i+1));
        configOutput(G2_ATT_OUTPUT+i, rack::string::f("Group B%d",i+1));
        configOutput(G3_ATT_OUTPUT+i, rack::string::f("Group C%d",i+1));
        configOutput(G4_ATT_OUTPUT+i, rack::string::f("Group D%d",i+1));
    }
    //onReset();
