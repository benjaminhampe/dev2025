...
struct FourSeq : Module {
    ...
    FourSeq()
    {
     config(NUM_PARAMS, NUM_INPUTS, NUM_OUTPUTS, NUM_LIGHTS);
        ...
        for(int i=0; i<4;i++)
        {
            configParam(GATEA_PARAM + i, 0.0, 1.0, 0.0, rack::string::f("Gate_A %d", i + 1));
            configParam(GATEB_PARAM + i, 0.0, 1.0, 0.0, rack::string::f("Gate_B %d", i + 1));
            configParam(SEQA_PARAM+i, -3.0,3.0, 0.0,rack::string::f("Freq_A %d", i + 1));
            configParam(SEQB_PARAM+i, -3.0,3.0, 0.0,rack::string::f("Freq_B %d", i + 1));
            configInput(CVA_INPUT+i,rack::string::f("Freq_A %d mod" ,i + 1));
            configInput(CVB_INPUT+i,rack::string::f("Freq_B %d mod", i + 1));
        }
