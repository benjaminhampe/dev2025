...
struct PerfMixerWidget : ModuleWidget {
  ...
  void appendContextMenu(Menu *menu) override {
    ...
    PerfMixer *module = dynamic_cast<PerfMixer*>(this->module);
    assert(module);
...
PerfMixerWidget(PerfMixer *module){
  ...
  addParam(createParam<MicroBlu>(Vec(right_column+7.5, 160 ), module, PerfMixer::AUX_S2_PARAM));
    ...
    for (int i = 0; i < 8; i++)
    {
      ...
      addParam(createParam<Trim>(Vec(column_1 + column_spacing * i + 3, top_row + row_spacing * 6 + top - 12), module, PerfMixer::PAN_PARAM + i));
...
//Screw
  ...
  addOutput(createOutput<PJ301MLPort>(Vec(right_column +5 , 60),    module, PerfMixer::MIX_OUTPUT_L));
	addOutput(createOutput<PJ301MRPort>(Vec(right_column +32 , 60 ),  module, PerfMixer::MIX_OUTPUT_R));

  addOutput(createOutput<PJ301MLPort>(Vec(right_column + 35, 100 ),  module, PerfMixer::SEND_1_L_OUTPUT));
  addOutput(createOutput<PJ301MRPort>(Vec(right_column + 35, 126 ),  module, PerfMixer::SEND_1_R_OUTPUT));

  addOutput(createOutput<PJ301MLPort>(Vec(right_column + 35, 160 ),  module, PerfMixer::SEND_2_L_OUTPUT));
  addOutput(createOutput<PJ301MRPort>(Vec(right_column + 35, 186 ),  module, PerfMixer::SEND_2_R_OUTPUT));

  addInput(createInput<PJ301MLPort>(Vec(right_column + 35, 225 ), module, PerfMixer::RETURN_1_L_INPUT));
  addInput(createInput<PJ301MRPort>(Vec(right_column + 35, 251 ), module, PerfMixer::RETURN_1_R_INPUT));

  addInput(createInput<PJ301MLPort>(Vec(right_column + 35, 285 ), module, PerfMixer::RETURN_2_L_INPUT));
  addInput(createInput<PJ301MRPort>(Vec(right_column + 35, 311 ), module, PerfMixer::RETURN_2_R_INPUT));
}
