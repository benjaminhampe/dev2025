...
struct Meander : Module
{
    ...
    {
        if (doDebug) RK_DEBUG("userPlaysCirclePositionHarmony(%d)", circle_position);
        if (doDebug) RK_DEBUG("circle_position=%d", circle_position);
        ...
        if (doDebug) RK_DEBUG("root_key_note=%d %s", root_key_note, note_desig[root_key_note%12]);
        ...
        if (doDebug) RK_DEBUG("circle_chord_type=%d", circle_chord_type);
        int num_chord_members=chord_type_num_notes[circle_chord_type];
        if (doDebug) RK_DEBUG("num_chord_members=%d", num_chord_members);
        ...
        for (int j=0;j<num_chord_members;++j)
        {
            ...
            if (doDebug) RK_DEBUG("  current_chord_note=%d %s", current_chord_note, note_desig[current_chord_note%12]);
    ...
    {
        if (doDebug) RK_DEBUG("doHarmony");
        if (doDebug) RK_DEBUG("doHarmony() theActiveHarmonyType.min_steps=%d, theActiveHarmonyType.max_steps=%d", theActiveHarmonyType.min_steps, theActiveHarmonyType.max_steps );
        ...
        if (doDebug) RK_DEBUG("theHarmonyTypes[%d].num_harmony_steps=%d", harmony_type, theActiveHarmonyType.num_harmony_steps);
        ...
        if ((harmony_type==31)||(harmony_type==42)||(harmony_type==43)||(harmony_type==44)||(harmony_type==45)||(harmony_type==46)||(harmony_type==47)||(harmony_type==48))  // Markov chains
        {
            ...
            {
                float rnd = rack::random::uniform();
                if (doDebug) RK_DEBUG("rnd=%.2f",rnd);
                ...
                else
                {
                    ...
                    if (doDebug) RK_DEBUG("Markov Probabilities:");
                    for (int i=1; i<8; ++i)  // skip first array index since this is 1 based
                    {
                        if (harmony_type==31)
                        {
                            if (doDebug) RK_DEBUG("i=%d: p=%.2f b=%.2f t=%.2f", i, MarkovProgressionTransitionMatrixBach1[theMeanderState.theHarmonyParms.last_circle_step+1][i], probabilityTargetBottom[i], probabilityTargetTop[i]);
                        }
                        else
                        if (harmony_type==42)
                        {
                            if (doDebug) RK_DEBUG("i=%d: p=%.2f b=%.2f t=%.2f", i, MarkovProgressionTransitionMatrixBach2[theMeanderState.theHarmonyParms.last_circle_step+1][i], probabilityTargetBottom[i], probabilityTargetTop[i]);
                        }
                        else
                        if (harmony_type==43)
                        {
                            if (doDebug) RK_DEBUG("i=%d: p=%.2f b=%.2f t=%.2f", i, MarkovProgressionTransitionMatrixMozart1[theMeanderState.theHarmonyParms.last_circle_step+1][i], probabilityTargetBottom[i], probabilityTargetTop[i]);
                        }
                        else
                        if (harmony_type==44)
                        {
                            if (doDebug) RK_DEBUG("i=%d: p=%.2f b=%.2f t=%.2f", i, MarkovProgressionTransitionMatrixMozart2[theMeanderState.theHarmonyParms.last_circle_step+1][i], probabilityTargetBottom[i], probabilityTargetTop[i]);
                        }
                        else
                        if (harmony_type==45)
                        {
                            if (doDebug) RK_DEBUG("i=%d: p=%.2f b=%.2f t=%.2f", i, MarkovProgressionTransitionMatrixPalestrina1[theMeanderState.theHarmonyParms.last_circle_step+1][i], probabilityTargetBottom[i], probabilityTargetTop[i]);
                        }
                        else
                        if (harmony_type==46)
                        {
                            if (doDebug) RK_DEBUG("i=%d: p=%.2f b=%.2f t=%.2f", i, MarkovProgressionTransitionMatrixBeethoven1[theMeanderState.theHarmonyParms.last_circle_step+1][i], probabilityTargetBottom[i], probabilityTargetTop[i]);
                        }
                        else
                        if (harmony_type==47)
                        {
                            if (doDebug) RK_DEBUG("i=%d: p=%.2f b=%.2f t=%.2f", i, MarkovProgressionTransitionMatrixTraditional1[theMeanderState.theHarmonyParms.last_circle_step+1][i], probabilityTargetBottom[i], probabilityTargetTop[i]);
                        }
                        else
                        if (harmony_type==48)
                        {
                            if (doDebug) RK_DEBUG("i=%d: p=%.2f b=%.2f t=%.2f", i, MarkovProgressionTransitionMatrix_I_IV_V[theMeanderState.theHarmonyParms.last_circle_step+1][i], probabilityTargetBottom[i], probabilityTargetTop[i]);
                        }

                        if ((rnd>probabilityTargetBottom[i])&&(rnd<= probabilityTargetTop[i]))
                        {
                            step=i-1;
                            if (doDebug) RK_DEBUG("step=%d", step);
                        }
                    }
        ...
        if (doDebug) RK_DEBUG("degreeStep=%d", degreeStep);
        ...
        for (int i=0; i<7; ++i)
        {
            ...
            if (i==7)
            {
               if (doDebug) RK_DEBUG("  warning circleposition could not be found 2");
            }
        }
        ...
        if (doDebug) RK_DEBUG("current_circle_position=%d root=%d %s", current_circle_position, circle_of_fifths[current_circle_position], note_desig[circle_of_fifths[current_circle_position]]);
        if (doDebug) RK_DEBUG("theCircleOf5ths.Circle5ths[current_circle_position].chordType=%d", theCircleOf5ths.Circle5ths[current_circle_position].chordType);
        ...
        if (doDebug) RK_DEBUG("step_chord_type=%d", step_chord_type);
        ...
        if (doDebug) RK_DEBUG("theMeanderState.last_harmony_chord_root_note=%d %s", theMeanderState.last_harmony_chord_root_note, note_desig[theMeanderState.last_harmony_chord_root_note%MAX_NOTES]);

        if (doDebug) RK_DEBUG("1st 3 step_chord_notes=%d %s, %d %s, %d %s", step_chord_notes[step][0], note_desig[step_chord_notes[step][0]%MAX_NOTES], step_chord_notes[step][1], note_desig[step_chord_notes[step][1]%MAX_NOTES], step_chord_notes[step][2], note_desig[step_chord_notes[step][2]%MAX_NOTES]);

        bool tonicFound=false;
        for (int j=0;j<num_chord_members;++j)
        {
                if (doDebug) RK_DEBUG("num_step_chord_notes[%d]=%d", step, num_step_chord_notes[step]);
                ...
                if (doDebug) RK_DEBUG("current_chord_notes[%d]=%d %s", j, current_chord_notes[j], note_desig[current_chord_notes[j]%MAX_NOTES]);

                int note_to_play=current_chord_notes[j]-12;  // drop it an octave to get target octave right
                if (doDebug) RK_DEBUG("    h_note_to_play=%d %s", note_to_play, note_desig[note_to_play%MAX_NOTES]);
    ...
    void doMelody()
    {
        if (doDebug) RK_DEBUG("doMelody()");

        outputs[OUT_MELODY_VOLUME_OUTPUT].setVoltage(theMeanderState.theMelodyParms.volume);
        clock_t current_cpu_t= clock();  // cpu clock ticks since program began
        double current_cpu_time_double= (double)(current_cpu_t) / (double)CLOCKS_PER_SEC;

        if (doDebug) RK_DEBUG("Melody: Time=%.3lf",  (double)current_cpu_time_double);
        ...
        if (doDebug) RK_DEBUG("    melody note_to_play=%d %s", note_to_play, note_desig[note_to_play%MAX_NOTES]);
    ...
    void doArp()
    {
        ...
        if (doDebug) RK_DEBUG("theMeanderState.theMelodyParms.last_chord_note_index=%d", theMeanderState.theMelodyParms.last_chord_note_index);
        if (doDebug) RK_DEBUG("num_step_chord_notes[%d]=%d", theMeanderState.theMelodyParms.last_step, num_step_chord_notes[theMeanderState.theMelodyParms.last_step]);
        ...
        if (theMeanderState.theArpParms.scaler) // use root_key_notes rather than step_chord_notes.  This is slower since scale note index has to be looked up
        {
            if (false)  // old brute force search from beginning
            {
                for (int x=0; x<num_root_key_notes[root_key]; ++x)
                {
                    if (root_key_notes[root_key][x]==theMeanderState.theMelodyParms.last[0].note)
                    {
                        ...
                        if (doDebug) RK_DEBUG("note fount at index=%d root_key_notes[root_key][x]=%d", x, root_key_notes[root_key][x]);
                        break;
                    }
                }
            }
            ...
            if (true)  // new // BSP search  .
            {
                ...
                if (doDebug) RK_DEBUG("BSP  note_to_search_for=%d",  note_to_search_for);
                int num_to_search=num_root_key_notes[root_key];
                if (doDebug) RK_DEBUG("BSP num_to_search=%d", num_to_search);
                ...
                while (pass<8)
                {
                    if (doDebug) RK_DEBUG("start_search_index=%d end_search_index=%d", start_search_index, end_search_index);
                    partition_index=(end_search_index+start_search_index)/2;
                    if (doDebug) RK_DEBUG("BSP start_search_index=%d end_search_index=%d partition_index=%d", start_search_index, end_search_index, partition_index);
                    if ( note_to_search_for>root_key_notes[root_key][partition_index])
                    {
                        start_search_index=partition_index;
                        if (doDebug) RK_DEBUG(">BSP root_key_notes[root_key][partition_index]=%d", root_key_notes[root_key][partition_index]);
                    }
                    else
                    if ( note_to_search_for<root_key_notes[root_key][partition_index])
                    {
                    ...
                    {
                        /* we found it */
                        if (doDebug) RK_DEBUG("value %d found at index %d", root_key_notes[root_key][partition_index], partition_index);
                        pass=8;
                        break;
                    }
                    ++pass;
    ...
    void doBass()
    {
        ...
        if (theMeanderState.theBassParms.enabled)
        {
            ...
            if (doDebug) RK_DEBUG("    bass note to play=%d %s", theMeanderState.last_harmony_chord_root_note, note_desig[theMeanderState.last_harmony_chord_root_note%MAX_NOTES]);
        ...
        if ((step_button_trig.process(params[BUTTON_PROG_STEP_PARAM].getValue() || (  inputs[IN_PROG_STEP_EXT_CV].isConnected()  &&  (inputs[IN_PROG_STEP_EXT_CV].getVoltage() > 0.)))))
        {
            ...
            for (int j=0; j<7; ++j)
            {
                if  (theCircleOf5ths.theDegreeSemiCircle.degreeElements[j].Degree==degreeStep)
                {
                    ...
                    if (doDebug) RK_DEBUG("harmony step edit-pt2 current_circle_position=%d", current_circle_position);
                    break;
                }
            }
...
//
        ...
        for (int i=0; i<12; ++i)
        {
            if (CircleStepToggles[i].process(params[BUTTON_CIRCLESTEP_C_PARAM+i].getValue()))  // circle button clicked
            {
                ...
                if (doDebug) RK_DEBUG("harmony step edit-pt3 current_circle_position=%d", current_circle_position);
                ...
                for (int j=0; j<7; ++j)
                {
                    if  (theCircleOf5ths.theDegreeSemiCircle.degreeElements[j].CircleIndex==current_circle_position)
                    {
                        ...
                        if (doDebug) RK_DEBUG("harmony step edit-pt4 theDegree=%d", theDegree);
                        if ((theDegree>=1)&&(theDegree<=7))
                        {
                            if (theMeanderState.theHarmonyParms.pending_step_edit)
                            {
                                if (doDebug) RK_DEBUG("harmony step edit-pt5 theMeanderState.theHarmonyParms.pending_step_edit=%d", theMeanderState.theHarmonyParms.pending_step_edit);
                                if (doDebug) RK_DEBUG("harmony step edit-pt6 theDegree=%d found", theDegree);
        ...
        if (!running)
        {
            for (int i=0; i<theActiveHarmonyType.num_harmony_steps; ++i)
            {
                ...
                {
                    if (doDebug) RK_DEBUG("harmony step edit-pt1 step=%d clicked", i);
                    int selectedStep=i;
        ...
        if (  (inputs[IN_HARMONY_CIRCLE_DEGREE_GATE_EXT_CV].isConnected()) && (inputs[IN_HARMONY_CIRCLE_DEGREE_EXT_CV].isConnected()) )
        {
            ...
            else  // keyboard  C-B
            {
                    ...
                    if ( (degreeChanged) || (circleDegree!=inportStates[IN_HARMONY_CIRCLE_DEGREE_EXT_CV].lastValue))
                    {
                        ...
                        if (circleDegree>=0)
                            ...
                            circleDegree=-(float)std::fmod(std::fabs(circleDegree), 1.0f);
                        degreeChanged=true;
                        if (doDebug) RK_DEBUG("IN_HARMONY_CIRCLE_DEGREE_EXT_CV circleDegree=%f", circleDegree);
                        if (circleDegree>=0)
                        {
            ...
            if ((degreeChanged)&&(!skipStep))
            {
                ...
                if (doDebug) RK_DEBUG("IN_HARMONY_CIRCLE_DEGREE_EXT_CV=%d", (int)theMeanderState.circleDegree);
            //	RK_DEBUG("IN_HARMONY_CIRCLE_DEGREE_EXT_CV=%d", (int)theMeanderState.circleDegree);
                ...
                if (doDebug) RK_DEBUG("userPlaysCirclePositionHarmony()");
                if (theMeanderState.theBassParms.enabled)
                    doBass();
        ...
        if (lowFreqClock.process())
        {
            ...
            {
                tempo = fvalue;
                if (doDebug) RK_DEBUG("tempo changed to %d", (int)tempo);
            }
            ...
            if ((fvalue=std::round(params[CONTROL_ROOT_KEY_PARAM].getValue()))!=circle_root_key)
            {
                ...
                if (doDebug) RK_DEBUG("root_key changed to %d = %s", root_key, note_desig[root_key]);
                for (int i=0; i<12; ++i)
            ...
            {
                mode = fvalue;
                if (doDebug) RK_DEBUG("mode changed to %d", mode);
                circleChanged=true;
            }
                        ...
                        switch (i)
                        {
                            ...
                            case IN_HARMONYPRESETS_EXT_CV:
                                if (fvalue>=0.01)
                                {
                                    ...
                                    {
                                        if (doDebug) RK_DEBUG("getVoltage harmony type=%d", (int)newValue);
                                    ...
                                    if (!(gateValue==scaleDegree))
                                    {
                                        ...
                                        if ( (degreeChanged) || (scaleDegree!=inportStates[IN_MELODY_SCALE_DEGREE_EXT_CV].lastValue))
                                        {
                                            ...
                                            if (scaleDegree>=0)
                                                ...
                                                scaleDegree=-(float)std::fmod(std::fabs(scaleDegree), 1.0f);
                                            degreeChanged=true;
                                            if (doDebug) RK_DEBUG("IN_MELODY_SCALE_DEGREE_EXT_CV scaleDegree=%f", scaleDegree);
                                            if (scaleDegree>=0)
                                            {
            ...
            {
                if (doDebug) RK_DEBUG(" getValue harmony type=%d", (int)fvalue);
            ...
            {
                if (doDebug) RK_DEBUG("theActiveHarmonyType.min_steps=%d, theActiveHarmonyType.max_steps=%d", theActiveHarmonyType.min_steps, theActiveHarmonyType.max_steps );
                if (doDebug) RK_DEBUG("theActiveHarmonyType.num_harmony_steps changed to %d %s", (int)fvalue, "test");  // need actual descriptor
            ...
            if (circleChanged)
            {
                if (doDebug) RK_DEBUG("circleChanged");

                notate_mode_as_signature_root_key=((root_key-(mode_natural_roots[mode_root_key_signature_offset[mode]]))+12)%12;
                if (doDebug) RK_DEBUG("notate_mode_as_signature_root_key=%d", notate_mode_as_signature_root_key);
    ...
    Meander()
    {
        ...
        if (doDebug) RK_DEBUG("");  // clear debug log file
...
struct MeanderWidget : ModuleWidget
{
    ...
    struct CircleOf5thsDisplay : TransparentWidget
    {
        ...
        {
            if (doDebug) RK_DEBUG("DrawCircle5ths()");
            ...
            for (int i=0; i<MAX_CIRCLE_STATIONS; ++i)
            {
                    ...
                    if (doDebug) RK_DEBUG("radialDirection= %.3f %.3f", theCircleOf5ths.Circle5ths[i].radialDirection.x, theCircleOf5ths.Circle5ths[i].radialDirection.y);
        ...
        {
            if (doDebug) RK_DEBUG("DrawDegreesSemicircle()");
            int chord_type=0;
            ...
            for (int i=0; i<MAX_HARMONIC_DEGREES; ++i)
            {
                // draw degree annulus sector
                    ...
                    if (doDebug) RK_DEBUG("radialDirection= %.3f %.3f", theCircleOf5ths.theDegreeSemiCircle.degreeElements[i].radialDirection.x, theCircleOf5ths.theDegreeSemiCircle.degreeElements[i].radialDirection.y);
        ...
        void drawGrid(const DrawArgs &args)
        {
                ...
                if (doDebug) RK_DEBUG("panel size (WxH) in px=%.2f x %.2f", panelWidth, panelHeight);  // panel size (WxH) in px=1198.82 x 380.91
        ...
        //	pos=Vec(beginEdge+10, beginTop+45);  // had to change for some unknown reason
            ...
            if (globalsInitialized)  // global fully initialized if Module!=NULL
            {
                ...
                for (int i=0; ((i<bar_note_count)&&(i<256)); ++i)
                {
                    ...
                    if (doDebug) RK_DEBUG("display_note=%d %s", display_note, note_desig[display_note%12]);
                    ...
                    if (strstr(note_desig[display_note%12],"B"))
                        scale_note=6;
                    if (doDebug) RK_DEBUG("scale_note=%d", scale_note%12);

                    int octave=(display_note/12)-2;
                    if (doDebug) RK_DEBUG("octave=%d", octave);

                    display_note_position = 108.0-(octave*21.0)-(scale_note*3.0)-7.5;
                    if (doDebug) RK_DEBUG("display_note_position=%d", (int)display_note_position);
        ...
        //	drawGrid(args);  // here after all updates are completed so grid is on top
            ...
            if (doDebug) RK_DEBUG("UpdatePanel()-end");
        }  // end UpdatePanel()
    ...
    {
        if (doDebug) RK_DEBUG("MeanderWidget()");
    ...
    void step() override   // note, this is a widget step() which is not deprecated and is a GUI call.  This advances UI by one "frame"
    {
        ...
        if ((module != NULL)&&(module->instanceRunning))
        {
            ...
            {
            //	RK_DEBUG("cwIn!==NULL cableID=%d", cwIn->cable->id);
                ...
                for (CableWidget* cwOut : APP->scene->rack->getCablesOnPort(outPortWidgets[Meander::OUT_CLOCK_BAR_OUTPUT]))
                {
                    ...
                    {
                    //	RK_DEBUG("cwOut!==NULL cableID=%d STEP in port connected to OUT_CLOCK_BAR_OUTPUT port", cwOut->cable->id);
                ...
                for (CableWidget* cwOut : APP->scene->rack->getCablesOnPort(outPortWidgets[Meander::OUT_CLOCK_BEAT_OUTPUT]))
                {
                    if (cwOut->cable->id == cwIn->cable->id)
                    {
                    //	RK_DEBUG("cwOut!==NULL cableID=%d STEP in port connected to OUT_CLOCK_BEAT_OUTPUT port", cwOut->cable->id);
                ...
                for (CableWidget* cwOut : APP->scene->rack->getCablesOnPort(outPortWidgets[Meander::OUT_CLOCK_BEATX2_OUTPUT]))
                {
                    if (cwOut->cable->id == cwIn->cable->id)
                    {
                    //	RK_DEBUG("cwOut!==NULL cableID=%d STEP in port connected to OUT_CLOCK_BEATX2_OUTPUT port", cwOut->cable->id);
                ...
                for (CableWidget* cwOut : APP->scene->rack->getCablesOnPort(outPortWidgets[Meander::OUT_CLOCK_BEATX4_OUTPUT]))
                {
                    if (cwOut->cable->id == cwIn->cable->id)
                    {
                    //	RK_DEBUG("cwOut!==NULL cableID=%d STEP in port connected to 	OUT_CLOCK_BEATX4_OUTPUT port", cwOut->cable->id);
                ...
                for (CableWidget* cwOut : APP->scene->rack->getCablesOnPort(outPortWidgets[Meander::OUT_CLOCK_BEATX8_OUTPUT]))
                {
                    if (cwOut->cable->id == cwIn->cable->id)
                    {
                    //	RK_DEBUG("cwOut!==NULL cableID=%d STEP in port connected to OUT_CLOCK_BEATX8_OUTPUT port", cwOut->cable->id);
