...
void SubLightButton::draw(const DrawArgs &args) {
    nvgSave(args.vg);
...
void SubLightButton::drawLayer(const DrawArgs &args, int layer) {
    if (layer == 1) {
        float value = 0.0f;
        if (getParamQuantity()) {
            value = getParamQuantity()->getValue();
...
void SubLightButton::drawLight(const DrawArgs &args, bool enabled) {
...
void SubLightButton::drawHalo(const DrawArgs &args) {
    // Halo
