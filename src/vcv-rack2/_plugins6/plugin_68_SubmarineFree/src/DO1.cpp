...
// https://sashat.me/2017/01/11/list-of-20-simple-distinct-colors/
    ...
    void loadJson() {
        ...
        if (!settings) {
            RK_WARN("Submarine Free DO-1xx: JSON parsing error at %s %d:%d %s", error.source, error.line, error.column, error.text);
            return;
        }
    ...
    struct PLTruthTable : MenuEntry {
        ...
        PLTruthTable(std::string tableValue) {
            ...
            if (entries.size() != (inputs + others) * rows) {
                RK_WARN("Invalid Truth Table: %s", tableValue.c_str());
            }
        }
        void draw(const DrawArgs &args) override {
