...
struct WM101 : SizeableModuleWidget, WM_Base {
    ...
    void loadSettings() {
        ...
        fclose(file);
        if (!rootJ) {
            RK_WARN("Submarine Free WM-101: JSON parsing error at %s %d:%d %s", error.source, error.line, error.column, error.text);
            return;
        }
...
struct WM102 : SchemeModuleWidget, WM_Base {
    ...
    void loadCollectionFromDisk(std::string pathC) override {
        ...
        fclose(file);
        if (!rootJ) {
            RK_WARN("Submarine Free WM-102: JSON parsing error at %s %d:%d %s", error.source, error.line, error.column, error.text);
            return;
        }
        ColorCollectionButton *cb = loadCollectionFromJson(rootJ);
        if (cb->colors.size()) {
