...
struct LowFrequencyOscillator {
    ...
    float tri(float phaseOffset) {
        float newPhase = 4.0f * fabsf(phase + phaseOffset -0.75f - roundf(phase + phaseOffset - 0.75f));
        return -1.0f + newPhase;
    }
    float saw(float phaseOffset) {
        return 2.0f * (phase + phaseOffset - roundf(phase + phaseOffset));
    }
    ...
    float light() {
        return sinf( float(2.0*M_PI) * phase);
    }
};
