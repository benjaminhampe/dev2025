...
struct CellGrid : rack::FramebufferWidget {
  ...
  void draw(const DrawArgs &args) override {
    ...
    void createContextMenu() {
        ...
        menu->addChild(
            rack::construct<rack::MenuLabel>(
                &rack::MenuLabel::text, gridName.c_str()));
  ...
  struct DrawShapeMenuItem : rack::MenuItem {
  ...
  struct ChangeShapeMenuItem : rack::MenuItem {
    OneDimensionalCells *cells;
    int flipDirection = 0;
...
struct CellBarGrid : rack::FramebufferWidget {
  OneDimensionalCells *cells = nullptr;
  float initX = 0;
  float initY = 0;
  float dragX = 0;
...
struct DisplayBarGrid : rack::FramebufferWidget {
  float *graph= nullptr;
  ...
  void draw(const DrawArgs &args) override {
    if (graph) {
      ...
      for (uint16_t y = 0; y < height; y++) {
        uint16_t x = std::clamp(graph[y] * width,0.0f,width);
        nvgBeginPath(args.vg);
