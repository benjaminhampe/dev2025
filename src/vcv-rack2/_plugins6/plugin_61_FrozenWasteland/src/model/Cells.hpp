...
struct OneDimensionalCells : Cells {
  ...
  void changeShape(int flipDirection, int shiftDirection,float reductionAmount) {
    if(flipDirection==-1) {
      ...
      for(uint16_t i=0;i<height;i++) {
          cells[i] = std::clamp(static_cast<float>(cells[i] + (totalRange * .25)),lowRange,highRange); // shift right by 25%
      }
    } else if (shiftDirection==1) {
      for(uint16_t i=0;i<height;i++) {
          cells[i] = std::clamp(static_cast<float>(cells[i] - (totalRange * .25)),lowRange,highRange); // shift left by 25%
      }
    } else {
      for(uint16_t i=0;i<height;i++) {
