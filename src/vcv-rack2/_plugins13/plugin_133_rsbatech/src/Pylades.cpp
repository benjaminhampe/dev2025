...
struct OscOutput {
   ...
   void sendModuleList(Iterator begin, Iterator end, int numMappedModules) {
        ...
        if (moduleRef.sending) {
            ...
            RK_WARN("Cannot send module list whilst not connected");
        }
   }
...
private:
    ...
    void receiverPower() {
        if (receiving) {
            ...
            if (receiving) RK_INFO("Started OSC Receiver on port: %i", port);
        } else {
            oscReceiver.stop();
        }
    }
    ...
    void senderPower() {
        if (sending) {
            ...
            if (sending) {
                RK_INFO("Started OSC Sender on port: %i", port);
                oscResendFeedback();
            }
        } else {
            oscSender.stop();
    ...
    bool processOscMessage(TheModularMind::OscMessage msg) {
        ...
        if (address == OSCMSG_FADER) {
            ...
            RK_WARN("Discarding unknown OSC message. OSC message had address: %s and %i args", msg.getAddress().c_str(), (int)msg.getNumArgs());
            return false;
        };
   ...
   /**
    ...
    void exportPluginMidiMap_action(char* path, std::string pluginSlug) {
        ...
        if (!file) {
            RK_WARN("Could not create file %s", path);
            return;
        }
        DEFER({
            fclose(file);
    ...
    void dataFromJson(json_t* rootJ) override {
        ...
        if (midiMapLibraryFilenameJ) {
            ...
            RK_WARN("No midimap library filename set");
        }
    ...
    void createMappingLibraryFromFactory() {
        ...
        if (loadMidiMapFromFactoryLibraryFile()) {
            ...
            RK_INFO("Factory library loaded, saving to preset library %s", defaultMidiMapLibraryFilename.c_str() );
            if (!system::exists(midiMapLibraryFilename)) {
    ...
    bool loadMidiMapFromFactoryLibraryFile() {
        ...
        if (!system::exists(factoryLibraryFilename)) {
            RK_WARN("Factory library file %s does not exist, skipping", factoryLibraryFilename.c_str());
            return false;
        }

        FILE* file = fopen(factoryLibraryFilename.c_str(), "r");
        if (!file) {
            RK_WARN("Could not load factory library file %s, skipping", factoryLibraryFilename.c_str());
            return false;
        }
        DEFER({
            fclose(file);
        ...
        if (!libraryJ) {
            RK_WARN("Factory library file is not a valid JSON file. Parsing error at %s %d:%d %s, skipping", error.source, error.line, error.column, error.text);
            return false;
        }

        if (loadMidiMapFromLibrary(libraryJ)) {
            RK_INFO("Loaded factory library file %s", factoryLibraryFilename.c_str());
            return true;
        } else {
            return false;
        }
    ...
    bool readMappingLibraryFile(std::string filename) {
        ...
        if (!file) {
            RK_WARN("Could not load mapping library file %s", filename.c_str());
            return false;
        }
        DEFER({
            fclose(file);
        ...
        if (!libraryJ) {
            RK_WARN("File is not a valid JSON file. Parsing error at %s %d:%d %s", error.source, error.line, error.column, error.text);
            return false;
        }
    ...
    bool saveMappingLibraryFile(std::string filename) {
        ...
        RK_INFO ("Saving mapping library to %s", filename.c_str());
        json_t* rootJ = json_object();
        DEFER({
            json_decref(rootJ);
        });
        ...
        if (!file) {
            RK_WARN("Could not open mapping library file for writing %s", filename.c_str());
            return false;
        }
        DEFER({
            fclose(file);
