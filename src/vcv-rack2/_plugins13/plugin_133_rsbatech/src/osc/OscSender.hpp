   ...
   public:
    ...
    bool start(std::string &host, int port) {
        ...
        try {
            ...
            if (!name.address) {
                RK_WARN("Bad hostname: %s", host.c_str());
                return false;
            }
            ...
            RK_WARN("OscSender couldn't start with %s:%i because of: %s", host.c_str(), port, e.what());
            if (socket != nullptr) {
                delete socket;
                socket = nullptr;
            }
    ...
    void appendMessage(const OscMessage &message, osc::OutboundPacketStream &outputStream) {
        ...
        for (size_t i = 0; i < message.getNumArgs(); ++i) {
            ...
            case osc::STRING_TYPE_TAG:
                ...
                break;
            default:
                RK_WARN("OscSender.appendMessage(), Unimplemented type?: %i", (int)message.getArgType(i));
                break;
            }
        }
