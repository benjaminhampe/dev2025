   ...
   public:
    ...
    bool start(int port) {
        ...
        try {
            ...
            RK_WARN("OscReceiver couldn't create receiver on port %i, %s", port, e.what());
            if (socket != nullptr) {
                delete socket;
                socket = nullptr;
            }
    ...
    void listenerProcess() {
        while (listenSocket) {
            try {
                ...
                RK_WARN("OscReceiver error: %s", e.what());
            }
        }
    }
        ...
        if (!queue.full()) {
            ...
            for (auto arg = receivedMessage.ArgumentsBegin(); arg != receivedMessage.ArgumentsEnd(); ++arg) {
                if (arg->IsInt32()) {
                    ...
                    RK_WARN("OscReceiver ProcessMessage(): argument in message %s is an unknown type %d", receivedMessage.AddressPattern(), arg->TypeTag());
                    break;
                }
            }
            queue.push(msg);
