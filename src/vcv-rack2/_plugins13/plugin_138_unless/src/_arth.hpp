...
struct Filer{
  ...
  void save(const std::string filename, json_t *rootJ){
    if (rootJ) {
      ...
      if (!file){
        RK_WARN("Cannot open '%s' to write", filename.c_str());
        return;
      }
      json_dumpf(rootJ, file, JSON_INDENT(2) | JSON_REAL_PRECISION(9));
      json_decref(rootJ);
      fclose(file);
      RK_INFO("Saving art to %s", filename.c_str());
    }else{
      RK_WARN("json to %s is empty", filename.c_str());
    }
  }
  ...
  json_t *load(const std::string filename){
    ...
    if(file){
      ...
      RK_WARN("no art file at %s", filename.c_str());
    }
    return rootJ;
  }
  void check(){}
...
struct Helper{
  ...
  static json_t *vec_to_json(rack::Vec v){
  ...
  static rack::Vec vec_from_json(json_t *rootJ){
    // return Vec(json_number_value(json_object_get(rootJ, "x")), json_number_value(json_object_get(rootJ, "y")));
    return rack::Vec(json_number_value(json_array_get(rootJ, 0)), json_number_value(json_array_get(rootJ, 1)));
  }
  ...
  static json_t *rect_to_json(rack::Rect r){
  ...
  static rack::Rect cornered_rect(rack::Rect r){
    return rack::Rect(r.pos.minus(r.size.mult(0.5f)), r.size);
  }
};
...
struct EditorLayer : _less::Widget {
  ...
  virtual void click(rack::Vec pos, int button){};
  virtual void drag(rack::Vec delta){};
  virtual void undo(){};
};
...
struct ArtData{
  ...
  static ArtData load(std::string slug){
    ...
    if(art_json){
      ...
      RK_INFO("Loaded ART %s.art", slug.c_str());
...
struct Art : ArtData{
  ...
  void widget(TWidget *w, bool add = true){
    if(widget_count < (int)layout.widgets.size()){
      ...
      RK_WARN("widget [%d] does not exist, adding new...", widget_count);
