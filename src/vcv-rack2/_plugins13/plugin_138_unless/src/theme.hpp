...
struct ThemeFile{
  ...
  void save(const std::string filename, json_t *rootJ){
    if (rootJ) {
      ...
      if (!file){
        RK_WARN("[ unless ] cannot open '%s' to write\n", filename.c_str());
        return;
      }
      json_dumpf(rootJ, file, JSON_INDENT(2) | JSON_REAL_PRECISION(9));
      json_decref(rootJ);
      fclose(file);
      RK_INFO("[ unless ] saving to %s\n", filename.c_str());
    }else{
      RK_INFO("[ unless ] json to %s is empty\n", filename.c_str());
    }
  }
    ...
    if(!loaded){
      if(!rack::system::isDirectory(rack::asset::user("unlessgames"))){
        ...
        if(!rack::system::isFile(file_path())){
          ...
          RK_INFO("[ unless ] loading color theme from ./unlessgames/theme.json");
          ...
          if(!version || json_integer_value(version) < theme->version){
            RK_INFO("[ unless ] theme created with previous version");
            generate(theme);
            return;
          }
          ...
          if(rootJ){
            ...
            RK_WARN("problem loading file from ./unlessgames/theme.json");
            return;
          }
        }
      }
