...
// =======================================================================
{
    ...
    static std::string
    bin2hpp( std::string sym )
    {
        if (sym.empty())
        {
            DE_WARN("Got empty sym name")
            return {};
        }

        std::ostringstream o;
        o << "extern const unsigned int " << sym << "_len;\n";
        o << "extern const unsigned char " << sym << "[];\n";
        return o.str();
    }
    ...
    bin2cpp( const de::Blob& bytes, std::string sym, uint32_t bytesPerLine = 256 )
    {
        ...
        std::ostringstream o;
        o << "const unsigned int " << sym << "_len = " << bytes.size() << ";\n";
        o << "const unsigned char " << sym << "[] = \n";
        o << "{\n\t";
    ...
    static void convertFiles(
        ...
        std::string inputDirectory, bool bRecursive = false)
    {
        outputFileNameH = de::FileSystem::makeAbsolute(outputFileNameH);
        inputDirectory = de::FileSystem::makeAbsolute(inputDirectory);
        DE_DEBUG("outputFileNameH = ",outputFileNameH)
        DE_DEBUG("inputDirectory = ",inputDirectory)
        ...
        DE_DEBUG("outputFileNameDir = ",outputFileNameDir)
        ...
        for (const auto& inputFileName : inputFileNames)
        {
            ...
            bin.binName = de::FileSystem::fileName( inputFileName, inputDirectory );
            DE_DEBUG("inputFileName[",i,"] = ", inputFileName, ", binName = ", bin.binName)
