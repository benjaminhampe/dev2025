...
struct BORDL : BidooModule {
	...
	BORDL() {
		...
		configOutput(ACC_OUTPUT, "Accent");
		...
		for (int i = 0; i < 8; i++) {
			...
			configParam(TRIG_PITCHRND_PARAM + i, 0.0f, 1.0f, 0.0f, "Pitch randomize");
			configParam(TRIG_ACCENT_PARAM + i, 0.0f, 10.0f, 0.0f,"Accent");
			configParam(TRIG_RNDACCENT_PARAM + i, 0.0f, 1.0f, 0.0f, "Accent randomize");
			configParam(TRIG_GATEPROB_PARAM + i, 0.0f, 1.0f,  1.0f, "Step probability");
		}
	}
