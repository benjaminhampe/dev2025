...
void wtFrame::loadSample(size_t sCount, bool interpolate, float *wav) {
  if (interpolate) {
    for(size_t i=0;i<FS;i++) {
      ...
      sample[i]=rack::math::rescale(pos,index,index+1,*(wav+index),*(wav+index+1));
    }
  }
  else {
    for(size_t i=0;i<FS;i++) {
  ...
  if (nFrames>1) {
    ...
    for (size_t i=0; i<fs-1; i++) {
      for (size_t j=1; j<fCount+1; j++) {
        ...
        for(size_t k=0; k<FS; k++) {
          frames[index].sample[k]=rack::math::rescale(j,0,fCount+1,frames[i*(fCount+1)].sample[k],frames[(i+1)*(fCount+1)].sample[k]);
        }
  ...
  if (nFrames>1) {
    ...
    for (size_t i=0; i<fs-1; i++) {
      for (size_t j=1; j<fCount+1; j++) {
        size_t index = i*(fCount+1) + j;
        for(size_t k=0; k<FS2; k++) {
          frames[index].magnitude[k]=rack::math::rescale(j,0,fCount+1,frames[i*(fCount+1)].magnitude[k],frames[(i+1)*(fCount+1)].magnitude[k]);
          frames[index].phase[k]=rack::math::rescale(j,0,fCount+1,frames[i*(fCount+1)].phase[k],frames[(i+1)*(fCount+1)].phase[k]);
        }
