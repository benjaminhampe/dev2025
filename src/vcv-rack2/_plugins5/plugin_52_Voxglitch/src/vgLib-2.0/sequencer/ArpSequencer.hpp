...
struct ArpSequencer
{
    ...
    std::vector<float> note_voltages;
    ...
    void step()
    {
        ...
        if (playback_mode == FORWARD || playback_mode == MANUAL)
        {
        ...
        else if (playback_mode == PINGPONG)
        {
            ...
            // Check if sequence_playback_position is out of bounds and correct if so
            sequence_playback_position = clamp(sequence_playback_position, 0, (int) indicies_vec.size() - 1);
        }
    ...
    float getNote()
    {
        if (indicies_vec.empty() || note_voltages.empty()) return 0.0;
        ...
        if(playback_mode != MANUAL)
        {
            ...
            // return note_voltages[sorted_indicies[sequence_playback_position]];
            return note_voltages[getIndex()];
        }
        else
        {
            // return note_voltages[indicies_vec[sequence_playback_position]];
            return note_voltages[getIndex()];
        }
    }
    ...
    int getIndex()
    {
        ...
        if(playback_mode != MANUAL)
        {
            ...
            return (sorted_indicies[sequence_playback_position]);
        }
        else
        {
    ...
    void setNumberOfNotes(int number_of_notes)
    {
        if ((int) note_voltages.size() != number_of_notes)
            note_voltages.resize(number_of_notes);

        // max_length = number_of_notes;
    }

    void setNoteAtIndex(int index, float voltage)
    {
        // Double check that the index is within bounds
        if (index < 0 || index >= (int) note_voltages.size())
            return;

        note_voltages[index] = voltage;
    }
