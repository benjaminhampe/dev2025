    ...
    void process(const ProcessArgs &args) override
    {
        ...
        if (frozen == false)
        {
            ...
            for (unsigned int i = 0; i < NUMBER_OF_SEQUENCERS; i++)
            {
                ...
                {
                    if (gate_sequencers[i].getValue())
                        outputs[POLY_CV_OUTPUT].setVoltage(output, i);
                }
                else
                {
                    outputs[POLY_CV_OUTPUT].setVoltage(output, i);
        ...
        else // IF FROZEN
        {
            ...
            for (unsigned int i = 0; i < NUMBER_OF_SEQUENCERS; i++)
            {
                ...
                float output = this->getScaledOutput(i);
                outputs[POLY_CV_OUTPUT].setVoltage(output);
            }
    ...
    void set_sequencer_lengths()
    {
        for (unsigned int i = 0; i < NUMBER_OF_SEQUENCERS; i++)
        {
            if (i < (unsigned int)inputs[POLY_LENGTH_INPUT].getChannels()) // If there's an input controlling this sequencer
            {
                ...
                voltage_sequencers[i].setWindowEnd((int)length - 1);
                gate_sequencers[i].setWindowEnd((int)length - 1);
            }
        }
    }
};
