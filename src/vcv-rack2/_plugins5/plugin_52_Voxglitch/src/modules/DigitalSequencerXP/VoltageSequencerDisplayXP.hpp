struct VoltageSequencerDisplayXP : SequencerDisplay
{
        ...
        if (layer == 1)
        {
            ...
            if (module)
            {
                ...
                for (unsigned int i = 0; i < MAX_SEQUENCER_STEPS; i++)
                {
                    ...
                    if ((int)i < (module->selected_voltage_sequencer->getWindowEnd() + 1))
                    {
                    ...
                    {
                        bar_color = current_step_highlight_color; // Highlight current step
                    }
                    else if ((int)i < (module->selected_voltage_sequencer->getWindowEnd() + 1))
                    {
    ...
    void drawTooltip(NVGcontext *vg)
    {
        ...
        if (draw_tooltip_index > 26)
            x = x - bar_width - TOOLTIP_WIDTH - (x_offset * 2) - BAR_HORIZONTAL_PADDING;
    ...
    void startControlSequence(Vec mouse_position)
    {
        ...
        module->selected_gate_sequencer->setWindowEnd(clicked_column);
        module->selected_voltage_sequencer->setWindowEnd(clicked_column);
    }
    ...
    void dragControlSequence(Vec mouse_position)
    {
        if (module)
        {
            ...
            module->selected_gate_sequencer->setWindowEnd(drag_column);
            module->selected_voltage_sequencer->setWindowEnd(drag_column);
        }
    }
    ...
    void onHoverKey(const event::HoverKey &e) override
    {
        ...
        if (keypressUp(e))
        {
            ...
            if (value > DRAW_AREA_HEIGHT)
                value = DRAW_AREA_HEIGHT;
