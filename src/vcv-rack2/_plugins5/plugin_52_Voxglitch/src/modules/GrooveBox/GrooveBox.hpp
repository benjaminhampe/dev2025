...
struct GrooveBox : VoxglitchSamplerModule
{
    ...
    // Add pulse generators for copy and paste buttons
    dsp::PulseGenerator copyPulse;
    dsp::PulseGenerator pastePulse;
    ...
    enum LightIds
    {
        ENUMS(STEP_LIGHTS, NUMBER_OF_STEPS),
        ENUMS(MEMORY_SLOT_LIGHTS, NUMBER_OF_MEMORY_SLOTS),
        COPY_LIGHT,
        PASTE_LIGHT,
        NUM_LIGHTS
    };
    ...
    */
    ...
    void dataFromJson(json_t *json_root) override
    {
        ...
        if (selected_color_theme_json)
            LCDColorScheme::selected_color_scheme = json_integer_value(selected_color_theme_json);

        json_t *selected_memory_index_json = json_object_get(json_root, "selected_memory_index");
        if (selected_memory_index_json)
            this->switchMemory(json_integer_value(selected_memory_index_json));
    ...
    */
    ...
    void process(const ProcessArgs &args) override
    {
        ...
        {
            bool isActive = selected_track->getValue(i);
            bool isCurrent = (playback_step == (unsigned int)i);

            // Set brightness based on step state
            float brightness = isActive ? (isCurrent ? 1.0f : 0.5f) : 0.0f;
            lights[STEP_LIGHTS + i].setBrightness(brightness);
        }

        // Update memory slot lights
        for (int i = 0; i < NUMBER_OF_MEMORY_SLOTS; i++) {
            lights[MEMORY_SLOT_LIGHTS + i].setBrightness(i == (int)memory_slot_index ? 1.0f : 0.0f);
        }

        // Button lights
        bool copyGate = copyPulse.process(args.sampleTime);
        lights[COPY_LIGHT].setSmoothBrightness(copyGate, args.sampleTime);

        bool pasteGate = pastePulse.process(args.sampleTime);
        lights[PASTE_LIGHT].setSmoothBrightness(pasteGate, args.sampleTime);
    }
