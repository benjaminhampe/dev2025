...
struct TrackLabelDisplay : TransparentWidget
{
    ...
    void draw_track_label(std::string label, NVGcontext *vg, NVGcolor backgroundColor)
    {
        ...
        // Only draw the text if the label is not empty
    ...
    void drawLayer(const DrawArgs &args, int layer) override
    {
        if (layer == 1)
        {
            ...
            if (module)
            {
                ...
                {
                    backgroundColor = LCDColorScheme::getLightColor();
                }
                else
                {
                    backgroundColor = LCDColorScheme::getDarkColor();
                }

                std::string to_display = module->sample_players[track_number].getFilename();

                // Always draw the background
                draw_track_label(to_display, vg, backgroundColor);
            }
            else
            {
                // In library view, highlight only the first track
                ...
                draw_track_label(PLACEHOLDER_TRACK_NAMES[track_number], vg, backgroundColor);
            }
