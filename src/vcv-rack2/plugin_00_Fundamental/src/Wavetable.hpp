...
struct Wavetable {
    ...
    */
    ...
    void interpolate() {
        ...
        octaves = rack::math::log2(waveLen) - 1;
        ...
        rack::dsp::RealFFT inFFT(waveLen);

        float* outF = new float[2 * waveLen * quality]();
        rack::dsp::RealFFT outFFT(waveLen * quality);
    ...
    void load(std::string path) {
        ...
        if (ext == ".wav") {
            ...
            drwav wav;
#if defined ARCH_WIN
            if (!drwav_init_file_w(&wav, rack::string::UTF8toUTF16(path).c_str(), NULL))
#else
            ...
            if (ext == ".f32") {
                ...
                rack::dsp::convert((const float*) data.data(), samples.data(), len);
            }
            else if (ext == ".s8" || ext == ".i8") {
                size_t len = data.size() / sizeof(int8_t);
                samples.resize(len);
                rack::dsp::convert((const int8_t*) data.data(), samples.data(), len);
            }
            else if (ext == ".s16" || ext == ".i16") {
                size_t len = data.size() / sizeof(int16_t);
                samples.resize(len);
                rack::dsp::convert((const int16_t*) data.data(), samples.data(), len);
            }
            else if (ext == ".s24" || ext == ".i24") {
                size_t len = data.size() / sizeof(rack::dsp::Int24);
                samples.resize(len);
                rack::dsp::convert((const rack::dsp::Int24*) data.data(), samples.data(), len);
            }
            else if (ext == ".s32" || ext == ".i32") {
                size_t len = data.size() / sizeof(int32_t);
                samples.resize(len);
                rack::dsp::convert((const int32_t*) data.data(), samples.data(), len);
            }
            else {
                return;
            }
    ...
    void loadDialog() {
        ...
        std::free(pathC);
        wavetableDir = rack::system::getDirectory(path);

        load(path);
        filename = rack::system::getFilename(path);
    }
    ...
    void saveDialog() const {
        ...
        std::string path = pathC;
        if (rack::system::getExtension(path) != ".wav") {
            path += ".wav";
        }
        wavetableDir = rack::system::getDirectory(path);
    ...
    void appendContextMenu(rack::Menu* menu) {
        menu->addChild(rack::createMenuItem("Initialize wavetable", "",
            [=]() {reset();}
        ));

        menu->addChild(rack::createMenuItem("Load wavetable", "",
            [=]() {loadDialog();}
        ));

        menu->addChild(rack::createMenuItem("Save wavetable", "",
            [=]() {saveDialog();}
        ));
        ...
        for (int i = sizeOffset; i <= 14; i++) {
            sizeLabels.push_back(rack::string::f("%d", 1 << i));
        }
        menu->addChild(rack::createIndexSubmenuItem("Wave points", sizeLabels,
            [=]() {return rack::math::log2(waveLen) - sizeOffset;},
...
struct WTDisplay : rack::LedDisplay {
    TModule* module;
    ...
    void drawLayer(const DrawArgs& args, int layer) override {
        ...
        if (layer == 1) {
            ...
            if (!font)
            {
                auto fontPath = rack::asset::system("res/fonts/ShareTechMono-Regular.ttf");
            ...
            nvgFillColor(args.vg, rack::SCHEME_YELLOW);
            ...
            rack::Vec scopePos = rack::Vec(0.0, 13.0);
            rack::Rect scopeRect = rack::Rect(scopePos, box.size - scopePos);
            scopeRect = scopeRect.shrink(rack::Vec(4, 5));
            ...
            for (size_t i = 0; i <= wavetable.waveLen; i += iSkip) {
                ...
                if (posF > 0.f) {
                    ...
                    wave = rack::crossfade(wave0, wave1, posF);
                }
                else {
                    wave = wave0;
                }
            ...
            nvgStrokeColor(args.vg, rack::SCHEME_YELLOW);
            nvgStroke(args.vg);
        }
    ...
    void onPathDrop(const PathDropEvent& e) override {
        ...
        module->wavetable.filename = rack::system::getFilename(path);
        e.consume(this);
    }
};
