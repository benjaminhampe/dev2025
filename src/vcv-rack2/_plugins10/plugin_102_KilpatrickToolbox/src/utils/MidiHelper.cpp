...
//                    RK_DEBUG("clearing devices so they will be reattached");
                    if((int)inputs.size() > slot && inputs[slot].deviceId != -1) {
//                        RK_DEBUG("clearing input slot: %d", slot);
                        inputs[slot].setDeviceId(-1);
                    }
                    if((int)outputs.size() > slot && outputs[slot].deviceId != -1) {
//                        RK_DEBUG("clearing output slot: %d", slot);
...
//                RK_DEBUG("trying to restore input - slot: %d - name: %s", slot, inputNames[slot].c_str());
...
//                RK_DEBUG("trying to restore output - slot: %d - name: %s", slot, outputNames[slot].c_str());
...
//            RK_DEBUG("recalling input - slot: %d - name: %s", slot, inputNames[slot].c_str());
        }
    }
...
//            RK_DEBUG("recalling output - slot: %d - name: %s", slot, outputNames[slot].c_str());
        }
    }
}
...
void MidiHelper::printMessage(const midi::Message& msg) {
    RK_DEBUG("MIDI RX - len: %d - st: 0x%02x - d0: 0x%02x - d1: 0x%02x",
