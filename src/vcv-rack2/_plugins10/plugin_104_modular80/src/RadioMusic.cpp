	...
	for (std::string &entry : entries) {
		if (system::isDirectory(entry)) {
			...
			if (banks.size() > MAX_NUM_BANKS) {
				RK_WARN("Max number of banks reached. Ignoring subdirectories.");
				return;
			}

			if (scanDepth++ > MAX_DIR_DEPTH) {
				RK_WARN("Directory has too many subdirectories: %s", entry.c_str());
				continue;
			};
...
bool load(const std::string &path) override {
	...
	if (wav) {
		...
		if (rawSamples) {
			...
			fclose(wav);
			if (samplesRead != fsize/(int)bytesPerSample) { RK_WARN("Failed to read entire file"); }
			...
			RK_FATAL("Failed to allocate memory");
		}

		free(rawSamples);

	} else {
		RK_FATAL("Failed to load file: %s", filePath.c_str());
	}
...
void RadioMusic::threadedScan() {
	if (audioPoolLocation.empty()) {
		RK_WARN("No root directory defined. Scan failed.");
		showError = true;
		return;
	}
...
void RadioMusic::threadedLoad() {
	if (scanner.banks.empty()) {
		RK_WARN("No banks available. Failed to load audio files.");
		showError = true;
		return;
	}
	...
	for (auto &f : files) {
		...
		if (drwav_init_file(&wav, f.c_str(), nullptr)) {
			...
			if (drwav_uninit(&wav) != DRWAV_SUCCESS) {
				RK_FATAL("Failed to uninitialize object %s", f.c_str());
			}
		...
		if (object->load(f)) {
			...
			if ((tmpObjectPool->memoryUsage + memory) < MAX_BANK_SIZE) {
				...
				RK_WARN("Bank memory limit of %ld Bytes exceeded. Aborting loading of audio objects.", (long int)MAX_BANK_SIZE);
				showError = true;
				break;
			}
		} else {
			RK_WARN("Failed to load object %s", f.c_str());
			showError = true;
		}
	}
...
void RadioMusic::removeAudioPoolFromPatchStorage() {
	...
	if (system::exists(audiopool)) {
		if (!system::removeRecursively(audiopool)) {
			RK_WARN("Failed to remove audiopool: %s", audiopool.c_str());
			showError = true;
		}
	}
}
...
void RadioMusic::saveCurrentBankToPatchStorage() {
	...
	if (system::exists(audiopool)) {
		if (!system::removeRecursively(audiopool)) {
			RK_WARN("Failed to remove existing audiopool: %s", audiopool.c_str());
			showError = true;
			return;
		}
	}
	...
	if (!system::createDirectory(audiopool)) {
		RK_WARN("Creating audiopool failed: %s", audiopool.c_str());
		showError = true;
		return;
	};
	...
	for (auto& f : scanner.banks[currentBank]) {
		if (!system::copy(f, audiopool)) {
			RK_WARN("Failed to copy file: %s", f.c_str());
			showError = true;
		}
	}
