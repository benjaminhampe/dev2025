...
struct TestLowstepper : Module {
	...
	void process(const ProcessArgs& args) override {
		if(lfoA == nullptr) {
			RK_INFO("Initializing lowstepper with sample rate of %fhz", args.sampleRate);
		...
		if(morphCv > 5 || morphCv < -5) {
			RK_INFO("morph cv is out of the range +/-5V %f", morphCv);
			morphCv = 0;
		}
		...
		if(seconds >= 0.5) {
			seconds = 0;
			RK_INFO("morphPot: %f", morphPot);
			RK_INFO("morphCv: %f", morphCv);
			RK_INFO("inputA.morph: %f", inputA.morph);
		}*/
