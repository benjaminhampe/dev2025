...
struct AddSynth : Module {
  ...
  bool load(const std::string &path) {
    RK_INFO("Loading addsynth config file %s",path.c_str());
    ...
    if(!rootJ) {
      RK_WARN("%s",string::f("Addsynth file has invalid JSON at %d:%d %s",error.line,error.column,error.text).c_str());
      return false;
    }
    try {
      parseJson(rootJ);
  ...
  AddSynth() {
    if(!load(asset::plugin(pluginInstance,"res/addsynth.json"))) {
      RK_INFO("user addsynth file %s does not exist or failed to load. using default_addsynth.json ....","res/addsynth.json");
