...
struct JTKeys : Module {
  ...
  bool load(const std::string &path) {
    RK_INFO("Loading scale file %s",path.c_str());
    ...
    if(!rootJ) {
      RK_WARN("%s",string::f("Scales file has invalid JSON at %d:%d %s",error.line,error.column,error.text).c_str());
      return false;
    }
    try {
      parseJson(rootJ);
    } catch(Exception &e) {
      RK_WARN("%s",e.msg.c_str());
      return false;
    }
    json_decref(rootJ);
    return true;
  ...
  JTKeys() {
    if(!load(asset::plugin(pluginInstance,"res/scales_31.json"))) {
      RK_INFO("user scale file %s does not exist or failed to load. using default_scales ....","res/scales_31.json");
  ...
  void updateKey(int key) {
    ...
    //RK_INFO("update %d -- %d - %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d",key, keyOn,on[0],on[1],on[2],on[3],on[4],on[5],on[6],on[7],on[8],on[9],on[10],on[11],on[12],on[13],on[14],on[15]);
  }
