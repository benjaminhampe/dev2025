// Moog style Saw oscillator by Mockba the Borg

#include "a_plugin.hpp"
#include "MockbaModular.hpp"

struct MaugSaw : Module {
	enum ParamIds {
		_LFO_PARAM,
		_FREQ_PARAM,
		_FINE_PARAM,
...
void MaugSaw::onReset() {
	onAdd();
}

void MaugSaw::process(const ProcessArgs& args) {
...
struct MaugSawWidget : ModuleWidget {
	MaugSawWidget(MaugSaw* module) {
		setModule(module);
		setPanel(APP->window->loadSvg(asset::plugin(pluginInstance, BGCOLOR)));
		SvgWidget* panel = createWidget<SvgWidget>(Vec(0, 0));
		panel->setSvg(APP->window->loadSvg(asset::plugin(pluginInstance, "res/MaugSaw.svg")));
		addChild(panel);
		...
		// Knobs
		addParam(createParamCentered<_Hsw>(mm2px(Vec(5.1, 68.0)), module, MaugSaw::_LFO_PARAM));
		addParam(createParamCentered<_Knob>(mm2px(Vec(5.1, 79.0)), module, MaugSaw::_FREQ_PARAM));
		addParam(createParamCentered<_Knob>(mm2px(Vec(5.1, 90.0)), module, MaugSaw::_FINE_PARAM));

		// Inputs
		addInput(createInputCentered<_Port>(mm2px(Vec(5.1, 101.0)), module, MaugSaw::_MODF_INPUT));

		// Outputs
		addOutput(createOutputCentered<_Port>(mm2px(Vec(5.1, 112.0)), module, MaugSaw::_WAVE_OUTPUT));
	}
};

Model* modelMaugSaw = createModel<MaugSaw, MaugSawWidget>("MaugSaw");