...
void CZSquare::onReset() {
	onAdd();
}

void CZSquare::process(const ProcessArgs& args) {
...
struct CZSquareWidget : ModuleWidget {
	CZSquareWidget(CZSquare* module) {
		...
		// Knobs
		addParam(createParamCentered<_Hsw>(mm2px(Vec(5.1, 46.0)), module, CZSquare::_LFO_PARAM));
		addParam(createParamCentered<_Knob>(mm2px(Vec(5.1, 57.0)), module, CZSquare::_FREQ_PARAM));
		addParam(createParamCentered<_Knob>(mm2px(Vec(5.1, 68.0)), module, CZSquare::_FINE_PARAM));
		addParam(createParamCentered<_Knob>(mm2px(Vec(5.1, 90.0)), module, CZSquare::_SHAPE_PARAM));

		// Inputs
		addInput(createInputCentered<_Port>(mm2px(Vec(5.1, 79.0)), module, CZSquare::_MODF_INPUT));
		addInput(createInputCentered<_Port>(mm2px(Vec(5.1, 101.0)), module, CZSquare::_MODS_INPUT));

		// Outputs
		addOutput(createOutputCentered<_Port>(mm2px(Vec(5.1, 112.0)), module, CZSquare::_WAVE_OUTPUT));
	}
};

Model* modelCZSquare = createModel<CZSquare, CZSquareWidget>("CZSquare");