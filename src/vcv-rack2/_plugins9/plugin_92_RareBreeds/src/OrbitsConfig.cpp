...
float OrbitsConfig::rFindFloatAttribute(std::string &content, std::string attribute, size_t search)
{
    ...
    {
        RK_WARN("Unable to find attribute \"%s\"", attribute.c_str());
        return 0.0f;
    }
    ...
    {
        RK_WARN("Unable to find content for attribute \"%s\"", attribute.c_str());
        return 0.0f;
    }
...
Vec OrbitsConfig::getPos(std::string component)
{
    ...
    if(search == std::string::npos)
    {
        RK_WARN("Unable to find inkscape:label=\"components\"");
    ...
    if(search == std::string::npos)
    {
        RK_WARN("Unable to find inkscape:label=\"%s\"", component.c_str());
        return mm2px(Vec(0.f, 0.f));
    }
