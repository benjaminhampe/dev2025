...
void oscCVWidget::step()
{
    ...
    try
    {
        ...
        else if (thisModule->oscShowConfigurationScreen)
        {
            ...
            if (thisModule->expToRenumber != NULL)
            {
                try
                {
                    ...
                    RK_DEBUG("Renumbering expansion module...");
#endif
                ...
                {
                    RK_WARN("Could not renumber the expander.\n%s", expansionEx.what());
                }
            ...
            {
#if OSCSV_DEBUG_EXPANDERS
                RK_DEBUG("Expander Index Changed - Old (%d), New (%d) - ColumnIx = %d", showConfigIndex, thisModule->expCurrentEditExpanderIx, this->showConfigColumnIndex);
#endif
...
#if OSCSV_DEBUG_EXPANDERS
                RK_DEBUG("Expander Index Changed - (New %d) Setting ColumnIx = 0", thisModule->expCurrentEditExpanderIx);
#endif
                // Now set to page 0
                thisModule->setExpansionEditPageCol(0);
                this->showConfigColumnIndex = 0;
                ...
                this->showConfigColumnIndex != thisModule->expCurrentEditPageCol)
            {
                ...
                //reloadPageOnly = true;
#if OSCSV_DEBUG_EXPANDERS
                RK_DEBUG("Expander ColumnIx Changed - Expander %d, Old ColumnIx = %d, New ColumnIx = %d", showConfigIndex, showConfigColumnIndex, thisModule->expCurrentEditPageCol);
#endif
        ...
        if (loadModuleToConfig)// (thisModule->oscConfigTrigger.process(thisModule->params[oscCV::ParamIds::OSC_SHOW_CONF_PARAM].getValue()))
        {
            ...
            {
                RK_DEBUG("*LOADING*: MASTER");
            }
            else
            {
                RK_DEBUG("*LOADING*: Expander (%s) %d - COLUMN %d", thisModule->expCurrentEditExpanderIx < 0 ? "Input" : "Output",
            ...
            for (int c = 0; c < thisModule->numberChannels; c++) {
                ...
                if (thisModule->inputChannels[c].showChannelConfigTrigger.process(thisModule->params[paramId].getValue())) {
#if OSCSV_DEBUG_EXPANDERS
                    RK_DEBUG("[ADV] btnClick: Input Ch %d, paramId = %d. Config Ix %d, Column Ix %d.", c, paramId, showConfigIndex, showConfigColumnIndex);
#endif
                    ...
                    else if (thisModule->expCurrentEditExpander != NULL)
                    {
                        ...
                        {
#if OSCSV_DEBUG_EXPANDERS
                            RK_DEBUG("[ADV] btnClick: Input Expander %d, Column Ix %d, Channel %d (Expander's Ch %d).", showConfigIndex, showConfigColumnIndex, c, chIx);
#endif
                        ...
                        {
                            RK_WARN("Error - Input Expander %d (Expander Ch %d) - %s", showConfigIndex, chIx, expanderNull.what());
                        }
                    }
                    isInput = true;
                    break;
                ...
                if (thisModule->outputChannels[c].showChannelConfigTrigger.process(thisModule->params[paramId].getValue())) {
#if OSCSV_DEBUG_EXPANDERS
                    RK_DEBUG("[ADV] btnClick: Output Ch %d, paramId = %d. Config Ix %d, Column Ix %d.", c, paramId, showConfigIndex, showConfigColumnIndex);
#endif
                    // Read any changes that may have happened. (we don't have room for a save button) on the previous screen
                    readChannelPathConfig(showConfigIndex, this->showConfigColumnIndex);
                    if (masterConfigLoaded)
                    ...
                    else if (thisModule->expCurrentEditExpander != NULL)
                    {
                        ...
                        {
#if OSCSV_DEBUG_EXPANDERS
                            RK_DEBUG("[ADV] btnClick: Output Expander %d, Column Ix %d, Channel %d (Expander's Ch %d).", showConfigIndex, showConfigColumnIndex, c, chIx);
#endif
                        ...
                        {
                            RK_WARN("Error - Output Expander %d (Expander Ch %d) - %s", showConfigIndex, chIx, expanderNull.what());
                        }
                    }
                    isInput = false;
                    break;
            ...
            if (thisModule->oscConnectTriggered)
            {
                ...
                if (oscConfigurationScreen->btnActionEnable)
                {
                    ...
                    if (!this->oscConfigurationScreen->isValidIpAddress())
                    {
                        ...
                        RK_DEBUG("IP Address is not valid.");
#endif
                    ...
                    else
                    {
                        // Try to connect
#if TROWA_DEBUG_MSGS >= TROWA_DEBUG_LVL_MED
                        RK_DEBUG("Save OSC Configuration clicked, save information for module.");
#endif
    ...
    if (module != NULL)
    {
        ...
        try
        {
            ...
            else if (index != 0)
            {
                ...
                {
#if OSCSV_DEBUG_EXPANDERS
                    RK_DEBUG("** Saving Expander %d - Column Ix %d. Expander has %d total channels.", index, colIx, exp->numberChannels);
#endif
...
void oscCVWidget::setChannelPathConfig() {
    if (module != NULL)
    {
        ...
        try
        {
            ...
            {
#if OSCSV_DEBUG_EXPANDERS
                RK_DEBUG("** READING Expander %d - Column Ix %d (Module ColIx is %d). Expander has %d total channels.", thisModule->expCurrentEditExpanderIx,
                    this->showConfigColumnIndex, thisModule->expCurrentEditPageCol,
...
void TSOscCVChannelConfigScreen::showControl(TSOSCCVChannel* channel, bool isInput)
{
    ...
    {
        RK_WARN("Error Configuring Channel: %s.", chEx.what());
    }
    return;
} // end showControl()
