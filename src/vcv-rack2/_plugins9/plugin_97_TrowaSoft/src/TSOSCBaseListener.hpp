...
public:
        ...
        const IpEndpointName& remoteEndpoint) override
    {
        ...
        try
        {
            ...
            RK_WARN("OSC Rx (Port %hu) Malformed Bundle: %s.", port, bEx.what());
        }
        catch (osc::MalformedMessageException& mEx) {
            RK_WARN("OSC Rx (Port %hu) Malformed Message: %s.", port, mEx.what());
        }
        catch (osc::MalformedPacketException& pEx) {
            RK_WARN("OSC Rx (Port %hu) Malformed Packet: %s.", port, pEx.what());
        }
        catch (std::exception& ex) {
            RK_WARN("OSC Rx (Port %hu) Error: %s.", port, ex.what());
        }
        return;
    }
protected:
