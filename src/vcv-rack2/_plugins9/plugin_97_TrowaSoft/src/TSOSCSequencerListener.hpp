...
public:
    ...
    virtual void ProcessPacket(const char* data, int size, const IpEndpointName& remoteEndpoint) override
    {
        ...
        try
        {
            ...
            RK_WARN("Sequencer OSC Rx Malformed Bundle: %s.", bEx.what());
        }
        catch (osc::MalformedMessageException& mEx) {
            RK_WARN("Sequencer OSC Rx Malformed Message: %s.", mEx.what());
        }
        catch (osc::MalformedPacketException& pEx) {
            RK_WARN("Sequencer OSC Rx Malformed Packet: %s.", pEx.what());
        }
        catch (std::exception& ex) {
            RK_WARN("Sequencer OSC Rx Error: %s.", ex.what());
        }
        return;
    }
protected:
