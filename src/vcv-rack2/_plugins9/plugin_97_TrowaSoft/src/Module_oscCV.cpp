...
oscCV::~oscCV()
{
    ...
    {
        RK_WARN("Error deleting inputChannels: %s", ex.what());
    }
    try
    {
    ...
    {
        RK_WARN("Error deleting outputChannels: %s", ex.what());
    }
    ...
    {
        RK_WARN("Error deleting pulseGens: %s", ex.what());
    }
    try
    {
    ...
    {
        RK_WARN("Error deleting inputTriggers: %s", ex.what());
    }
    return;
} // end destructor
//-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-
...
void oscCV::dataFromJson(json_t *rootJ) {
    ...
    if (autoReconnect)
    {
        ...
        {
            RK_WARN("oscCV::dataFromJson(): Error on auto-reconnect OSC %s :%d :%d.", this->currentOSCSettings.oscTxIpAddress.c_str(), this->currentOSCSettings.oscTxPort, this->currentOSCSettings.oscRxPort);
        }
        else
        {
            RK_INFO("oscCV::dataFromJson(): Successful auto-reconnection of OSC %s :%d :%d.", this->currentOSCSettings.oscTxIpAddress.c_str(), this->currentOSCSettings.oscTxPort, this->currentOSCSettings.oscRxPort);
        }
    }
    return;
} // end dataFromJson()
...
void oscCV::process(const ProcessArgs &args)
{
    ...
    if (expCurrentEditExpander)
    {
        ...
        {
            RK_WARN("Error expander must be deleted...\n%s", expansionEx.what());
    ...
    if (doCVPort2OSC) {
        ...
        {
            RK_WARN("Error with Expansion Module Input: %s.", expansionEx.what());
        }
    ...
    if (doOSC2CVPort)
    {
        ...
        while (rxMsgQueue.size() > 0)
        {
            ...
            {
                RK_WARN("Error accessing received message.\n%s", rxEx.what());
            }
            rxMsgMutex.lock();
            rxMsgQueue.pop();
            rxMsgMutex.unlock();
