...
void Erwin::dataFromJson(json_t *rootJ) {
    ...
    if(!gatesJ) {
        RK_DEBUG("Erwin: Invalid Input file");
        return;
    }
    ...
    struct ErwinSaveItem : MenuItem {
        ...
        static void pathSelected(json_t* rootJ, char* path) {
            if(path) {
                ...
                RK_DEBUG("Error: cannot export rewin scale file");
            }
            free(path);
        }
    };
...
#endif
                    RK_DEBUG("Error: Can't import file %s", path);
                    RK_DEBUG("Text: %s", error.text);
                    RK_DEBUG("Source: %s", error.source);
                }
                free(path);
            }
        }
