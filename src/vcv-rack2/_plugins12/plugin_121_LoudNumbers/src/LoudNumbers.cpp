...
struct LoudNumbers : Module
{
    ...
    void dataFromJson(json_t* rootJ) override {
        ...
        if (default_pathJ) {
            ...
            RK_INFO("LOADING PATH: %s", p.c_str());
            currentpath = p;
    ...
    void process(const ProcessArgs &args) override
    {
        ...
        if (!badcsv) {
            ...
            if (firstrun)
            {
                RK_INFO("data min: %f", datamin);
                RK_INFO("data max: %f", datamax);
                RK_INFO("data length: %i", datalength);
                firstrun = false;
            }
    ...
    void processCSV(std::string path)
    {
        ...
        try {
            ...
            badcsv = true;
            RK_WARN("ERROR: CSV file could not be read.");
        }
    }
};
