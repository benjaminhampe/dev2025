...
struct Delay : Module {
	...
	void process(const ProcessArgs& args) override {
		...
		// RK_DEBUG("freq %f index %f", freq, index);
		...
		if (outBuffer.empty()) {
			...
			// RK_DEBUG("index %f historyBuffer %lu consume %f ratio %lf", index, historyBuffer.size(), consume, ratio);
			...
			// RK_DEBUG("used %ld gen %ld", srcData.input_frames_used, srcData.output_frames_gen);
		}
