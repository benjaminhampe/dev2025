...
void Skins::setDefaultSkin(std::string skinKey) {
	...
	if (error.size() > 0) {
		RK_WARN("Bogaudio: setting default skin: %s\n", error.c_str());
	}
	else {
		_default = skinKey;
		RK_INFO("Bogaudio: skin information written to %s\n", path.c_str());
...
void Skins::loadSkins() {
	...
	if (!root) {
		RK_WARN("Bogaudio: JSON error loading skins data from %s: %s\n", path.c_str(), error.text);
		return;
	}

	json_t* skins = json_object_get(root, "skins");
	if (!skins) {
		RK_WARN("Bogaudio: no \"skins\" section found in %s\n", path.c_str());
	}
	else {
		json_t* d = json_object_get(skins, "default");
		if (!d) {
			RK_WARN("Bogaudio: \"skins\" section has no key \"default\" in %s\n", path.c_str());
		}
		else {
			std::string dk = json_string_value(d);
			if (!validKey(dk)) {
				RK_WARN("Bogaudio: \"skins\" \"default\" value \"%s\" is invalid in %s\n", dk.c_str(), path.c_str());
				RK_WARN("Bogaudio: available skins are:\n");
				for (auto s : _available) {
					RK_WARN("Bogaudio:   %s\n", s.key.c_str());
				}
			}
				...
				_default = dk;
				RK_INFO("Bogaudio: skin information loaded successfully from %s\n", path.c_str());
			}
		}
	}
...
void Skins::loadCssValues() {
	...
	json_t* root = json_load_file(path.c_str(), 0, &error);
	if (!root) {
		RK_WARN("Bogaudio: JSON error loading CSS values data from %s: %s\n", path.c_str(), error.text);
		return;
	}
