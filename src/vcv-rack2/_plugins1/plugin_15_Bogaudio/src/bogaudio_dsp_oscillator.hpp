...
#include "bogaudio_dsp_base.hpp"
#include "bogaudio_dsp_math.hpp"
#include "bogaudio_dsp_table.hpp"
...
struct Phasor : OscillatorGenerator {
    ...
    inline static phase_delta_t radiansToPhase(float radians) { return ((double)cyclePhase / twoPI) * radians; }
...
struct SquareOscillator : Phasor {
    ...
    phase_t _pulseWidth = (double)defaultPulseWidth * cyclePhase;
    phase_t _nextPulseWidth = (double)defaultPulseWidth * cyclePhase;
	bool positive = true;
...
struct TriangleOscillator : Phasor {
    const phase_t quarterCyclePhase = 0.25 * cyclePhase;
    const phase_t threeQuartersCyclePhase = 0.75 * cyclePhase;
