...
std::vector<IoNote>* interopPasteSequenceNotes(int maxSeqLen, int *seqLenPtr) {// caller must delete return vector when non null
    ...
    if (!interopClip) {
        RK_WARN("IOP error getting clipboard string");
        return nullptr;
    }
    ...
    if (!clipboardJ) {
        RK_WARN("IOP error json parsing clipboard");
        return nullptr;
    }
    ...
    if (!vcvrackSequenceJ) {
        RK_WARN("IOP error no vcvrack-sequence present in clipboard");
        return nullptr;
    }
    ...
    if (!jsonLength) {
        RK_WARN("IOP error vcvrack-seqence length missing");
        return nullptr;
    }
    *seqLenPtr = (int)std::ceil(json_number_value(jsonLength));
    if (*seqLenPtr < 1) {
        RK_WARN("IOP error vcvrack-sequence must have positive length");
        return nullptr;
    }
    if (*seqLenPtr > maxSeqLen) {
        *seqLenPtr = maxSeqLen;
        RK_WARN("IOP vcvrack-sequence truncated during paste");
    }

    // notes
    json_t* notesJ = json_object_get(vcvrackSequenceJ, "notes");
    if ( !notesJ || !json_is_array(notesJ) ) {
        RK_WARN("IOP error vcvrack-sequence notes array malformed or missing");
        return nullptr;
    }
    ...
    for (size_t i = 0; i < json_array_size(notesJ); i++) {
        ...
        if (!noteJ) {
            RK_WARN("IOP error missing note in notes array");
            return nullptr;
        }
        ...
        if (!typeJ || (std::string("note").compare(json_string_value(typeJ)) != 0)) {
            RK_WARN("IOP missing or unrecognized note type, skipping it");
            continue;
        }
        IoNote newNote;
        ...
        if (!startJ) {
            RK_WARN("IOP bad start time for note, note skipped");
            continue;
        }
        ...
        if (!lengthJ) {
            RK_WARN("IOP bad length for note, note skipped");
            continue;
        }
        ...
        if (!pitchJ) {
            RK_WARN("IOP bad pitch for note, note skipped");
            continue;
        }
    ...
    if (ioNotes->size() < 1) {
        RK_WARN("IOP error in vcvrack-sequence, no notes in notes array ");
        delete ioNotes;
        return nullptr;
    }
    return ioNotes;
