...
void NormalizedFloat12Paste(float* float12) {
    ...
    if (!float12Clip) {
        RK_WARN("Normalized float12 error getting clipboard string");
    }
    else {
        json_error_t error;
        json_t* clipboardJ = json_loads(float12Clip, 0, &error);
        if (!clipboardJ) {
            RK_WARN("Normalized float12 error json parsing clipboard");
        }
        else {
            DEFER({json_decref(clipboardJ);});
            json_t* normFloats12J = json_object_get(clipboardJ, "Impromptu normalized float12");
            if (!normFloats12J) {
                RK_WARN("Error no Impromptu normalized float12 values present in clipboard");
            }
            else {
