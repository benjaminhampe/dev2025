...
void savePresetOrShape(const std::string& path, Channel* channel, bool isPreset, Channel* channelDirtyCache) {
    RK_INFO((isPreset ? "Saving ShapeMaster channel preset %s" : "Saving ShapeMaster shape %s"), path.c_str());
...
struct PresetOrShapeItem : MenuItem {
    ...
    void onAction(const event::Action &e) override {
        RK_INFO((isPreset ? "Loading ShapeMaster channel preset %s" : "Loading ShapeMaster shape %s"), path.c_str());
