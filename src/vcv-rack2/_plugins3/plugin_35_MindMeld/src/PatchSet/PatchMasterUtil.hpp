...
struct TileConfig {
    ...
    void fromJson(json_t* pmTileJ) {
        ...
        if ( !configsJ || !json_is_array(configsJ) ) {
            RK_WARN("PatchMaster error patch-master-tile configs array malformed or missing");
            return;
        }
        for (int m = 0; m < std::min(4, (int)json_array_size(configsJ)); m++) {
            json_t* configJ = json_array_get(configsJ, m);
            if (!configJ) {
                RK_WARN("PatchMaster error missing config in configs array");
                return;
            }
            ...
            if (!rangeMaxJ) {
                RK_WARN("PatchMaster missing rangeMax in config, skipping it");
                continue;
            }
            ...
            if (!rangeMinJ) {
                RK_WARN("PatchMaster missing rangeMin in config, skipping it");
                continue;
            }
