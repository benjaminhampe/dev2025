...
struct MixMaster : Module {
    ...
    void swapPasteFromClipboard() {
        ...
        if (!inerchangeClip) {
            RK_WARN("MixMaster swap: error getting clipboard string");
            return;
        }
        ...
        if (!clipboardJ) {
            RK_WARN("MixMaster swap: error json parsing clipboard");
            return;
        }
        ...
        if (!mixerJ) {
            RK_WARN("MixMaster swap: error no mixmaster-swap present in clipboard");
            return;
        }
        ...
        if (!nTrkJ) {
            RK_WARN("MixMaster swap: error num tracks missing");
            return;
        }
        ...
        if (!nGrpJ) {
            RK_WARN("MixMaster swap: error num groups missing");
            return;
        }
        ...
        if (!dataToJsonJ) {
            RK_WARN("MixMaster swap: error dataToJson-data missing");
            return;
        }
    ...
    void jsonArrayToParamDirect(json_t* paramJ, int baseParam, int numParam) {// numParam is of .this
        if ( !paramJ || !json_is_array(paramJ) ) {
            RK_WARN("MixMaster swap: error param array malformed or missing");
            return;
        }
        for (int i = 0; i < std::min((int)json_array_size(paramJ), numParam) ; i++) {
            json_t* paramItemJ = json_array_get(paramJ, i);
            if (!paramItemJ) {
                RK_WARN("MixMaster swap: error missing param value in param array");
                return;
            }
