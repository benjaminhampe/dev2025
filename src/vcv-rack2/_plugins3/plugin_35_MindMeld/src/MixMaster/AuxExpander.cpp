...
struct AuxExpander : Module {
    ...
    void swapPasteFromClipboard() {
        ...
        if (!inerchangeClip) {
            RK_WARN("AuxSpander swap: error getting clipboard string");
            return;
        }
        ...
        if (!clipboardJ) {
            RK_WARN("AuxSpander swap: error json parsing clipboard");
            return;
        }
        ...
        if (!auxspanderJ) {
            RK_WARN("AuxSpander swap: error no auxspander-swap present in clipboard");
            return;
        }
        ...
        if ( !paramsJ || !json_is_array(paramsJ) ) {
            RK_WARN("AuxSpander swap: error params array malformed or missing");
            return;
        }
        for (size_t i = 0; i < json_array_size(paramsJ); i++) {
            json_t* paramJ = json_array_get(paramsJ, i);
            if (!paramJ) {
                RK_WARN("AuxSpander swap: error missing param in params array");
                return;
            }
        ...
        if (!dataToJsonJ) {
            RK_WARN("AuxSpander swap: error dataToJson-data missing");
            return;
        }
    ...
    void jsonArrayToParamDirect(json_t* paramJ, int baseParam, int numParam) {// numParam is of .this
        if ( !paramJ || !json_is_array(paramJ) ) {
            RK_WARN("AuxSpander swap: error param array malformed or missing");
            return;
        }
        for (int i = 0; i < std::min((int)json_array_size(paramJ), numParam) ; i++) {
            json_t* paramItemJ = json_array_get(paramJ, i);
            if (!paramItemJ) {
                RK_WARN("AuxSpander swap: error missing param value in param array");
                return;
            }
