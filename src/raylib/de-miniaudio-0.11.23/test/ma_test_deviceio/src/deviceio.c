...
ma_bool32 try_parse_backend(const char* arg, ma_backend* pBackends, ma_uint32 backendCap, ma_uint32* pBackendCount)
{
    ...
    assert(arg           != NULL);
    assert(pBackends     != NULL);
    assert(pBackendCount != NULL);
...
void on_data(ma_device* pDevice, void* pFramesOut, const void* pFramesIn, ma_uint32 frameCount)
{
    ...
    switch (pDevice->type)
    {
        ...
        case ma_device_type_duplex:
        {
            ...
            assert(pDevice->playback.format == pDevice->capture.format);
            assert(pDevice->playback.channels == pDevice->capture.channels);
            memcpy(pFramesOut, pFramesIn, ma_get_bytes_per_frame(pDevice->playback.format, pDevice->playback.channels) * frameCount);
