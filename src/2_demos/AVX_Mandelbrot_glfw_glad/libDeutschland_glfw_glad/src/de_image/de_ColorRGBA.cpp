...
// ===================================================================
uint32_t RGBA( glm::vec4 const & color )
{
   return RGBA( Float2Byte::float2byte( color.r ),
                Float2Byte::float2byte( color.g ),
                Float2Byte::float2byte( color.b ),
                Float2Byte::float2byte( color.a ) );
}
...
void randomize()
{
   ::srand( static_cast< uint32_t >( HighResolutionClock::GetTimeInMilliseconds() ) );
}
...
// ===================================================================
uint32_t randomColorRGBA( uint8_t const alpha )
{
...
//   a = dbClamp( a, 0, 255 );
   ...
   r = std::clamp( int(std::roundf( t * dr + float( r ) )), 0, 255 );
   g = std::clamp( int(std::roundf( t * dg + float( g ) )), 0, 255 );
   b = std::clamp( int(std::roundf( t * db + float( b ) )), 0, 255 );
   a = std::clamp( int(std::roundf( t * da + float( a ) )), 0, 255 );
   return RGBA( r, g, b, a );
}
