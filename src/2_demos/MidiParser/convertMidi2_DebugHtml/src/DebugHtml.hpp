...
{
   return RGBA( rand()%200, rand()%200, rand()%200 );
}
...
// =======================================================================
{
   ...
   // ---------------------------- print colored midiTokens ----------------------------------
      ...
      for ( size_t i = 0; i < m_tokens.size(); ++i )
      {
         ...
         int r = RGBA_R( token.m_fg );
         int g = RGBA_G( token.m_fg );
         int b = RGBA_B( token.m_fg );
      ...
      bool const ok = FileSystem::saveText( saveUri, htmlStr );
      perf.stop();
