...
Bresenham::traverseEllipse( int32_t a, int32_t b, SetPixel_t const & setPixel )
{
   ...
   int32_t F = int32_t(std::roundf( float( bb - aa * b ) + float( aa ) * 0.25f ));
   while ( ddF_x < ddF_y )
   {
      if ( F >= 0 )
      {
   ...
   // Bresenham Alghorithmus
   F = int32_t(std::roundf(
         float( bb ) * ( float( x ) + 0.5f ) * ( float( x ) + 0.5f ) +
                      float( aa*(y-1)*(y-1) - aa*bb ) ));
   while ( y > 0 )
   {
      if ( F <= 0 )
      {
...
//   }
   ...
   // Bresenham Alghorithmus
   int32_t F = dbRound32( float( b2-a2*b ) + float( a2 )*0.25f );
   while ( ddF_x < ddF_y )
   {
      if (F >= 0)
      {
   ...
   // Bresenham Alghorithmus
   F = dbRound32( float( b2 ) * ( float( x ) + 0.5f ) * ( float( x ) + 0.5f )
                     + float( a2*(y-1)*(y-1) - a2*b2 ) );
   while ( y > 0 )
   {
      if ( F <= 0 )
      {
   ...
   bool blend )
{
   ...
   float const A1=(f32)color_a.getAlpha();
   float const R1=(f32)color_a.getRed();
   float const G1=(f32)color_a.getGreen();
   float const B1=(f32)color_a.getBlue();
   float const dA=(f32)color_b.getAlpha()-A1;
   float const dR=(f32)color_b.getRed()-R1;
   float const dG=(f32)color_b.getGreen()-G1;
   float const dB=(f32)color_b.getBlue()-B1;
   ...
   bool blend )
{
   if (border==0)
   {
      ...
      float const n_inv = irr::core::reciprocal( (f32)(x2 - x1) * (f32)(y2 - y1) );

      for (int32_t y=y1; y<=y2; y++)
      {
         for (int32_t x=x1; x<=x2; x++)
         {
            float const f11 = (f32)(x2 - x) * (f32)(y2 - y) * n_inv; // bottomRight
            float const f21 = (f32)(x - x1) * (f32)(y2 - y) * n_inv; // bottomLeft
            float const f12 = (f32)(x2 - x) * (f32)(y - y1) * n_inv; // topRight
            float const f22 = (f32)(x - x1) * (f32)(y - y1) * n_inv; // topLeft

            float const fA = 	(f32)topLeft.getAlpha() * f11 +
                        (f32)topRight.getAlpha() * f21 +
                        (f32)bottomRight.getAlpha() * f22 +
                        (f32)bottomLeft.getAlpha() * f12;

            float const fR = 	(f32)topLeft.getRed() * f11 +
                        (f32)topRight.getRed() * f21 +
                        (f32)bottomRight.getRed() * f22 +
                        (f32)bottomLeft.getRed() * f12;

            float const fG = 	(f32)topLeft.getGreen() * f11 +
                        (f32)topRight.getGreen() * f21 +
                        (f32)bottomRight.getGreen() * f22 +
                        (f32)bottomLeft.getGreen() * f12;

            float const fB = 	(f32)topLeft.getBlue() * f11 +
...
//
   ...
   else if ( border > 1)
   {
      ...
      float const n_inv = irr::core::reciprocal( (f32)w * (f32)h );
      ...
      for (u32 y = 0; y < h; y++)
      {
         ...
         while (x < w)
         {
            ...
            if (colorNow == white)
            {
               float const f11 = (f32)(w-1-x) * (f32)(h-1-y) * n_inv; // topLeft
               float const f21 = (f32)(x) * (f32)(h-1-y) * n_inv; // topRight
               float const f12 = (f32)(w-1-x) * (f32)(y) * n_inv; // bottomLeft
               float const f22 = (f32)(x) * (f32)(y) * n_inv; // bottomRight
...
void Tool::drawRoundRect( IImage* dst, const core::recti& pos, int32_t rx, int32_t ry, const SColor& topLeft, const SColor& topRight, const SColor& bottomRight, const SColor& bottomLeft, int32_t border, bool blend)
{
   ...
   for (u32 y=0; y<size.Height; y++)
   {
      for (u32 x=0; x<size.Width; x++)
      {
         ...
         {
            float const f11 = (f32)(size.Width-1-x) * (f32)(size.Height-1 - y) * n_inv; // topLeft
            float const f21 = (f32)(x) * (f32)(size.Height-1 - y) * n_inv; // topRight
            float const f12 = (f32)(size.Width-1 - x) * (f32)(y) * n_inv; // bottomLeft
            float const f22 = (f32)(x) * (f32)(y) * n_inv; // bottomRight
...
//
//			float const fA = (f32)c1.getAlpha(), fdA = (f32)c2.getAlpha() - fA;
//			float const fR = (f32)c1.getRed(), fdR = (f32)c2.getRed() - fR;
//			float const fG = (f32)c1.getGreen(), fdG = (f32)c2.getGreen() - fG;
//			float const fB = (f32)c1.getBlue(), fdB = (f32)c2.getBlue() - fB;
//			const SColor white(255,255,255,255);
//			float const fw = 0.5f*(f32)tmp->getDimension().Width;
//			float const fh = 0.5f*(f32)tmp->getDimension().Height;
//			float const invRadius = irr::core::reciprocal( (f32)radius );
//
...
//					if (colorNow == white)
//					{
//						float const dx = (f32)x - fw;
//						float const dy = (f32)y - fh;
...
//				const core::dimension2du& Size = tmp->getDimension();
//
//				float const fw = 0.5f*(f32)Size.Width;
//				float const fh = 0.5f*(f32)Size.Height;
//				float const invRadius = irr::core::reciprocal( (f32)border );
