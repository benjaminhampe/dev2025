...
#define FRONT   "media/antons_opengl_book/negz.jpg"
#define BACK    "media/antons_opengl_book/posz.jpg"
#define TOP     "media/antons_opengl_book/posy.jpg"
#define BOTTOM  "media/antons_opengl_book/negy.jpg"
#define LEFT    "media/antons_opengl_book/negx.jpg"
#define RIGHT   "media/antons_opengl_book/posx.jpg"

#define MESH_FILE "media/antons_opengl_book/suzanne.obj"

/* choose pure reflection or pure refraction here. */
//#define MONKEY_VERT_FILE "media/antons_opengl_book/28_uniform_buffer_object_reflect.vs"
//#define MONKEY_FRAG_FILE "media/antons_opengl_book/28_uniform_buffer_object_reflect.fs"
#define MONKEY_VERT_FILE "media/antons_opengl_book/28_uniform_buffer_object_refract.vs"
#define MONKEY_FRAG_FILE "media/antons_opengl_book/28_uniform_buffer_object_refract.fs"

#define CUBE_VERT_FILE "media/antons_opengl_book/28_uniform_buffer_object_cube.vs"
#define CUBE_FRAG_FILE "media/antons_opengl_book/28_uniform_buffer_object_cube.fs"

typedef struct Cam_Vars {
	mat4 P; // 16 floats for my projection matrix
	mat4 V; // 16 floats for my view matrix
} Cam_Vars;
...
int main() {
	...
	/* NOTE: seems to report V as invalid -1 now. good! */

	// int monkey_V_location = glGetUniformLocation (monkey_sp, "V");
	// printf ("monkey V location = %i\n", monkey_V_location);
	...
	int cube_V_location = glGetUniformLocation( cube_sp, "cam_R" );
	...
	/*
	Bind the block to each of the shader programmes that will use it
	THIS IS UNNEEDED WITH JUST A SINGLE BLOCK BUT ALLOWS >1 BLOCKS */
	int block_id = 0;
...
#define ONE_DEG_IN_RAD ( 2.0 * M_PI ) / 360.0 // 0.017444444
	...
	Cam_Vars *cam_ubo_ptr = (Cam_Vars *)glMapBufferRange(
		GL_UNIFORM_BUFFER, 0, sizeof( Cam_Vars ),
		GL_MAP_WRITE_BIT | GL_MAP_INVALIDATE_BUFFER_BIT | GL_MAP_UNSYNCHRONIZED_BIT );

	cam_ubo_ptr->P = proj_mat;
	cam_ubo_ptr->V = view_mat;
	...
	while ( !glfwWindowShouldClose( g_window ) ) {
		...
		if ( cam_moved ) {
			...
			glBindBufferBase( GL_UNIFORM_BUFFER, block_id, cam_block_buffer );
			float *cam_ubo_ptr =
				(float *)glMapBufferRange( GL_UNIFORM_BUFFER, 0, sizeof( float ) * 32,
			...
			/*			glBindBufferBase (GL_UNIFORM_BUFFER, block_id,
				 cam_block_buffer);
															glBufferSubData (
																			GL_UNIFORM_BUFFER, sizeof (mat4), sizeof
				 (mat4), view_mat.m);*/
