...
| * This is just a case of activating texture slot 0 AND 1 and binding a diff  |
| texture into each one. then in the fragment shader we can have 2 samplers    |
...
#include "stb_image.h"
#include <GL/glew.h>		// include GLEW and new version of GL on g_windows
...
// keep track of g_window size for things like the viewport and the mouse cursor
int g_gl_width = 640;
int g_gl_height = 480;
...
int main() {
	...
	GLuint shader_programme = 
        ...
        "media/antons_opengl_book/14_multi_tex.vs", 
        "media/antons_opengl_book/14_multi_tex.fs" 
    );
...
#define ONE_DEG_IN_RAD ( 2.0 * M_PI ) / 360.0 // 0.017444444
	...
	load_texture( "media/antons_opengl_book/skulluvmap.png", &tex_a );
	glBindTexture( GL_TEXTURE_2D, tex_a );
	glActiveTexture( GL_TEXTURE1 );
	load_texture( "media/antons_opengl_book/ship.png", &tex_b );
