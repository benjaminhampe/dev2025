...
#define FIRST_PASS_VS   "media/antons_opengl_book/37_deferred_shading_first_pass.vs"
#define FIRST_PASS_FS   "media/antons_opengl_book/37_deferred_shading_first_pass.fs"
#define SECOND_PASS_VS  "media/antons_opengl_book/37_deferred_shading_second_pass.vs"
#define SECOND_PASS_FS  "media/antons_opengl_book/37_deferred_shading_second_pass.fs"
#define SPHERE_FILE     "media/antons_opengl_book/sphere.obj"
#define PLANE_FILE      "media/antons_opengl_book/plane.obj"
...
bool init_fb() {
	...
	glTexImage2D( GL_TEXTURE_2D, 0, GL_RGB32F, g_gl_width, g_gl_height, 0, GL_RGB,
	...
	glTexImage2D( GL_TEXTURE_2D, 0, GL_RGB16F, g_gl_width, g_gl_height, 0, GL_RGB,
								GL_UNSIGNED_BYTE, NULL );
	glTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST );
	glTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST );
	glTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE );
	...
	glTexImage2D( GL_TEXTURE_2D, 0, GL_DEPTH_COMPONENT32F, g_gl_width, g_gl_height, 0,
...
bool load_plane() {
	...
	float *normals = NULL;
	if ( !load_obj_file( PLANE_FILE, points, tex_coords, normals,
											 g_plane_point_count ) ) {
	...
	glBufferData( GL_ARRAY_BUFFER, g_plane_point_count * 3 * sizeof( GLfloat ),
...
bool load_sphere() {
	...
	if ( !load_obj_file( SPHERE_FILE, sphere_points, sphere_tex_coords,
											 sphere_normals, g_sphere_point_count ) ) {
	...
	glGenBuffers( 1, &points_vbo );
	glBindBuffer( GL_ARRAY_BUFFER, points_vbo );
	glBufferData( GL_ARRAY_BUFFER, g_sphere_point_count * 3 * sizeof( GLfloat ),
...
void draw_second_pass() {
	...
	for ( int i = 0; i < NUM_LIGHTS; i++ ) {
		/* world position */
		glUniform3f( g_second_pass_L_p_loc, g_L_p[i].v[0], g_L_p[i].v[1],
								 g_L_p[i].v[2] );
		/* diffuse colour */
		glUniform3f( g_second_pass_L_d_loc, g_L_d[i].v[0], g_L_d[i].v[1],
								 g_L_d[i].v[2] );
		/* specular colour */
		glUniform3f( g_second_pass_L_s_loc, g_L_s[i].v[0], g_L_s[i].v[1],
								 g_L_s[i].v[2] );
...
int main() {
	...
	g_second_pass_sp = create_programme_from_files( SECOND_PASS_VS, SECOND_PASS_FS );
	...
	for ( int i = 0; i < NUM_LIGHTS; i++ ) {
		g_L_M[i] =
			scale( identity_mat4(), vec3( light_radius, light_radius, light_radius ) );
		g_L_M[i] = translate( g_L_M[i], g_L_p[i] );
		/* cycle different colours for each of the lights */
		g_L_d[i] = vec3( (float)( ( redi + 1 ) / 3 ), (float)( ( greeni + 1 ) / 3 ),
										 (float)( ( bluei + 1 ) / 3 ) );
