...
uint32_t
GT_compileShader( const char* name, uint32_t type, const char* source)
{
    ...
    if (!ok)
    {
        ...
        switch(type)
        {
            case GL_VERTEX_SHADER: strType = "VS"; break;
            case GL_FRAGMENT_SHADER: strType = "FS"; break;
            case GL_GEOMETRY_SHADER: strType = "GS"; break;
            default: break;
        }

        DE_ERROR(strType,"-SHADER[",name,"]::COMPILATION_FAILED:\n", infoLog)
    }
...
uint32_t
GT_createShader( const char* name, const char* vsText, const char* fsText )
{
...
void TexManager::init()
{
    ...
    m_numTexUnits = uint32_t( glGetInteger( GL_MAX_TEXTURE_IMAGE_UNITS ) ); GL_VALIDATE;
    m_maxTex2DSize = uint32_t( glGetInteger( GL_MAX_TEXTURE_SIZE ) ); GL_VALIDATE;
