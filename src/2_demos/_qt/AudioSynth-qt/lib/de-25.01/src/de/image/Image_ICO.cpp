...
bool
ImageReaderICO::load( Image & img, const uint8_t* p, size_t n, const std::string& uri )
{
    Binary file(p,n,uri);
    if ( !file.is_open() )
    {
...
//      icoInfo.bits_per_pixel = file.readU16_lsb();
      ...
      bool isPNG = FileMagic::isPNG( file.data() + entry.offset );
...
//      }
   // Icon image encoded as a compressed PNG image.
      if ( isPNG )
      {
         ...
         std::string urii = StringUtil::join( uri, "_entry",i,"_", w, "x", h, "_", b, ".png" );

         Binary bin( file.data() + entry.offset, entry.size, urii);
      ...
      else
      {
         ...
         int pad = 0;
         std::string urii = StringUtil::join( uri, "_entry",i,"_", w, "x", h, "_", bpp, ".tga" );
...
//            {
            ...
            DE_DEBUG( "n(",n,") loops, bmp(",bmp.str(false),")")
