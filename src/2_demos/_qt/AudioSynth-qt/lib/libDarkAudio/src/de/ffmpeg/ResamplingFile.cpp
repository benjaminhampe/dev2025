...
std::string
ResamplingFileStream::getDetailName() const
{
...
// Owned and used only by the ResamplingFile, so its actually a ResamplingFileStream
ResamplingFileStream::ResamplingFileStream( AVFormatContext* avFormatCtx, int avStreamIndex )
...
ResamplingFileStream::~ResamplingFileStream()
{
...
bool
ResamplingFileStream::isSeekable() const { return m_IsSeekable; }

bool
ResamplingFileStream::seek( double pts )
{
   if ( m_IsEnabled )
   {
      m_Position = pts;
...
double
ResamplingFileStream::tell() const
{
   return m_Position;
}

//bool
//ResamplingFileStream::hasDuration() const { return m_HasDuration; }
//double
//ResamplingFileStream::getDuration() const { return m_Duration; }
//double
//ResamplingFileStream::getPosition() const { return m_Position; }

//bool
//ResamplingFileStream::setPosition( double pts )
//{
//   return seek( pts );
//}

// =====================================================================================
// class ResamplingFile
// =====================================================================================
...
void
ResamplingFile::setOutSampleRate( int dstRate )
{
   m_dstRate = dstRate;
}

bool
ResamplingFile::is_open() const
{
...
bool
ResamplingFile::open( std::string uri, bool debug )
{
   m_IsDebug = debug;
   ...
   {
      m_Streams.push_back( new ResamplingFileStream( m_avFormatCtx, i ) );
   }
   ...
   if ( m_IsDebug )
   {
      DE_DEBUG("ResamplingFile.IsSeekable = ", m_IsSeekable )
      DE_DEBUG("ResamplingFile.Duration = ", dbStrSeconds( m_Duration ) )
      timer.stop();
      DE_DEBUG("ResamplingFile.LoadTime = ", timer.ms(), " ms.")
   }
//   fillCacheUnguarded();
   return m_IsOpen;
}
...
void
ResamplingFile::close()
{
   //std::lock_guard< std::mutex > guard( m_Mutex );

   for ( size_t i = 0; i < m_Streams.size(); ++i )
   {
      ResamplingFileStream* p = m_Streams[ i ];
      if ( !p ) continue;
      delete p;
   }
   m_Streams.clear();
...
int32_t
ResamplingFile::readFrame( ResamplingFile::ON_GET_DATA const & onGetData )
{
   ...
   //
   ResamplingFileStream* stream = m_Streams[ m_avPacket->stream_index ];
      ...
      if (e >= 0)
      {
         if ( stream->isEnabled() )
         {
            ...
            if ( stream->isAudio() )
            {
               ...
               //Utils::extractAudio( m_avFrame, *payload->buf, de::audio::ST_F32 );
               Utils::extractAudioF32( m_avFrame, *payload->buf, m_dstRate, stream->m_swResampler );
...
bool
ResamplingFile::isSeekable() const
{
...
void
ResamplingFile::seek( double pts )
{
   DE_DEBUG("seek = ",pts)
   DE_DEBUG("m_IsSeekable = ",m_IsSeekable)
...
double
ResamplingFile::tell() const
{
   return m_Position;
}

bool
ResamplingFile::hasDuration() const
{
   return m_IsSeekable;
}

double
ResamplingFile::getDuration() const
{
   return m_Duration;
}

bool
ResamplingFile::isStream( int i ) const
{
   if ( i < 0 )
   {
      return false;
...
uint32_t
ResamplingFile::getStreamCount() const
{
...
ResamplingFile::getStream( int i )
{
   if ( !isStream( i ) ) { DE_ERROR("No stream i = ",i) return nullptr; }
   return m_Streams[ i ];
}

ResamplingFileStream const*
ResamplingFile::getStream( int i ) const
{
   if ( !isStream( i ) ) { DE_ERROR("No stream i = ",i) return nullptr; }
   return m_Streams[ i ];
}
...
void
ResamplingFile::enableStream( int i )
{
   ResamplingFileStream* stream = getStream( i );
   if ( !stream ) { DE_ERROR("No stream i = ",i) return; }
   stream->enable();
}

void
ResamplingFile::disableStream( int i )
{
   ResamplingFileStream* stream = getStream( i );
   if ( !stream ) { DE_ERROR("No stream i = ",i) return; }
   stream->disable();
...
double
ResamplingFile::getPosition() const
{
   std::lock_guard< std::mutex > guard( m_Mutex );
...
void
ResamplingFile::setPosition( double seconds )
{
   std::lock_guard< std::mutex > guard( m_Mutex );
...
uint32_t
ResamplingFile::getAudioStreamCount() const
{
   size_t n = 0;
...
ResamplingFileStream*
ResamplingFile::getVideoStream( int i ) const
{
...
ResamplingFileStream*
ResamplingFile::getAudioStream( int i ) const
{
...
uint32_t
ResamplingFile::getVideoStreamCount() const
{
   size_t n = 0;
...
uint64_t
ResamplingFile::fillAudioBuffer( audio::Buffer & dst, double pts )
{
   std::lock_guard< std::mutex > guard( m_Mutex );
...
bool
ResamplingFile::fillImageBuffer( Image & dst, double pts )
{
   std::lock_guard< std::mutex > guard( m_Mutex );
   fillCacheUnguarded();
...
void
ResamplingFile::fillCacheUnguarded()
{
   if ( !m_IsOpen )
   {
      DE_WARN("Not open")
