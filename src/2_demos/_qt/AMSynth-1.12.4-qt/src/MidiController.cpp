...
void
MidiController::sendNoteOff(u8 channel, u8 midiNote, u8 velocity)
{
   static const float scale = 1.f/127.f;
   if (!_handler)
   {
      printf("No midi handler\n"); fflush(stdout);
      return;
   }
   _handler->HandleMidiNoteOff( int(midiNote), float(velocity) * scale );
}

void
MidiController::sendNoteOn(u8 channel, u8 midiNote, u8 velocity)
{
   static const float scale = 1.f/127.f;
   if (!_handler)
   {
   ...
   _handler->HandleMidiNoteOn( int(midiNote), float(velocity) * scale );
}

void
MidiController::HandleShortMidiEvent( amsynth_short_midi_event_t event )
{
   switch (event.command)
   {
   case MIDI_STATUS_NOTE_OFF:
      sendNoteOff( event.channel, event.midiNote, event.velocity );
      break;
   case MIDI_STATUS_NOTE_ON:
      sendNoteOn( event.channel, event.midiNote, event.velocity );
      break;
   case MIDI_STATUS_NOTE_PRESSURE:
      break;
   case MIDI_STATUS_CONTROLLER:
      controller_change( event.midiNote, event.velocity );
      break;
