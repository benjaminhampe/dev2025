...
void Synthesizer::process( u32 nframes,
                           ...
                           u32 audio_stride )
{
   ...
   std::vector<amsynth_short_midi_event_t>::const_iterator itEvent = midi_in.begin();
   u32 frames_left_in_buffer = nframes;
   u32 frame_index = 0;

   while (frames_left_in_buffer)
   {
      while (itEvent != midi_in.end())
      {
         _midiController->HandleShortMidiEvent( *itEvent );
         ++itEvent;
      }

      u32 block_size_frames =
         std::min(frames_left_in_buffer, (unsigned)VoiceBoard::kMaxProcessBufferSize);
...
//      }
      ...
      _voiceAllocationUnit->Process(
                              audio_l + (frame_index * audio_stride),
                           ...
                           u32 audio_stride )
{
   ...
   while (frames_left_in_buffer)
   {
      ...
      u32 block_size_frames = std::min(frames_left_in_buffer, (unsigned)VoiceBoard::kMaxProcessBufferSize);
      if (event != midi_in.end() && event->offset_frames > frame_index)
      {
         unsigned frames_until_next_event = event->offset_frames - frame_index;
         block_size_frames = std::min(block_size_frames, frames_until_next_event);
...
{
   printf("Synthesizer.loadBank(%s)\n", filename); fflush(stdout);
   _presetController->loadPresets(filename);


   int presetNo = _presetController->getCurrPresetNumber();

   printf("Synthesizer.loadBank(%s) :: Current presetNo = %d\n", filename, presetNo);
   fflush(stdout);

   _presetController->selectPreset( presetNo );
}
