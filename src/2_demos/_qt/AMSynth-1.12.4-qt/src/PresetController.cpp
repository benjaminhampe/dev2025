...
//   }
   ...
   std::string uri = Filesystem::makeAbsolute( filename );
   ...
   for (char *end_ptr = line_ptr; end_ptr < buffer_end && *end_ptr; end_ptr++) {
      if (*end_ptr == '\n') {
         *end_ptr = '\0';
         ...
         {
            std::string presetName(line_ptr + sizeof(preset_prefix) - 1);

            //printf("Got preset name %s\n", presetName.c_str() );

            presets[++preset_index] = Preset( presetName );
         }
...
{
   printf("PresetController.loadPresets(%s)\n", filename); fflush(stdout);
   if (filename == nullptr)
   {
      filename = bank_file.c_str();

      printf("PresetController.loadPresets(%s) Got empty filename\n", filename); fflush(stdout);
   }
   ...
   {
      printf("file not modified since last load\n"); fflush(stdout);
      return 0; // file not modified since last load
   }
   if (!readBankFile(filename, presets))
   {
      printf("Failed readBankFile\n"); fflush(stdout);
      return -1;
   }


   currentBankNo = -1;
   std::vector<BankInfo> const & banks = getPresetBanks();
   printf("banks = %d\n", int(banks.size()) ); fflush(stdout);
...
static void scan_preset_bank( std::string const & uri, bool read_only)
{
   //std::string file_path = dir_path + std::string("/") + std::string(file_name);

   std::string bank_name = uri;
   ...
   if (!is_amsynth_file(uri.c_str()))
   {
      printf("scan_preset_bank :: Not an amsynth file %s\n", uri.c_str() ); fflush(stdout);
      return;
   }

   BankInfo bank_info;
   bank_info.name = bank_name;
   bank_info.file_path = uri;
   bank_info.read_only = read_only;
   readBankFile(uri.c_str(), bank_info.presets);
   s_banks.push_back(bank_info);
}

static void scan_preset_banks(const std::string dir_path, bool read_only)
{
   printf("scan_preset_banks(%s)\n", dir_path.c_str() ); fflush(stdout);

   std::vector<std::string> filenames;

   auto userFunc = [&]( std::string const & uri )
      {
         filenames.push_back( uri );
      };

   Filesystem::entryList( dir_path, false, true, false, userFunc );

   std::sort(filenames.begin(), filenames.end());

   printf("scan_preset_banks() :: Found %d preset banks in dir %s\n", int(filenames.size()), dir_path.c_str() ); fflush(stdout);

   for (auto &filename : filenames)
   {
      scan_preset_bank( filename, read_only);
   }

   printf("scan_preset_banks() :: Loaded %d preset banks\n", int(s_banks.size()) ); fflush(stdout);
}
...
{
   printf("PresetController::getPresetBanks() :: Scanning\n");

   if (s_banks.empty())
      scan_preset_banks();

   printf("PresetController::getPresetBanks() :: Got banks = %d\n", int(s_banks.size()) );
   fflush( stdout );
