...
{
   int w = pos.w() > 0 ? pos.w() : ref.w();
   int h = pos.h() > 0 ? pos.h() : ref.h();
   ...
   int x = pos.x();
   int y = pos.y();
   Rectf const & coords = ref.m_coords; // Is identity(0,0,1,1) if ref.tex is empty/nullptr.
   auto u1 = coords.u1();
   auto v1 = coords.v1();
   auto u2 = coords.u2();
   auto v2 = coords.v2();
            ...
            uint32_t colorC, uint32_t colorD, TexRef const & ref, int borderWidth )
{
   ...
   int x = pos.x();
   int y = pos.y();
   Rectf const & coords = ref.m_coords;
   auto const & u1 = coords.u1();
   auto const & v1 = coords.v1();
   auto const & u2 = coords.u2();
   auto const & v2 = coords.v2();
       ...
       uint32_t color, uint32_t tessCircle, uint32_t tessHull, glm::vec3 const & offset )
{
   ...
   for ( size_t j = 0; j < tessHull; ++j )
   {
      for ( size_t i = 0; i < tessCircle; ++i )
      {
         ...
         if ( a >= a_pos.size() )
         {
            DE_RUNTIME_ERROR( dbStrJoin("a = ",a,", n = ",a_pos.size()),"Got a >= a_pos.size()" )
         }
         if ( b >= a_pos.size() )
         {
            DE_RUNTIME_ERROR( dbStrJoin("b = ",b,", n = ",a_pos.size()),"Got b >= a_pos.size()" )
         }
         if ( c >= a_pos.size() )
         {
            DE_RUNTIME_ERROR( dbStrJoin("c = ",c,", n = ",a_pos.size()),"Got c >= a_pos.size()" )
         }
         if ( d >= a_pos.size() )
         {
            DE_RUNTIME_ERROR( dbStrJoin("d = ",d,", n = ",a_pos.size()),"Got d >= a_pos.size()" )
         }
