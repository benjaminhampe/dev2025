...
Window_WGL::~Window_WGL()
{
   ...
   //m_renderUpdateTimer.
   stopTimer();
   // Sleep(100); // This sleep seems to prevent segfault on program end.
   destroy();
}

void Window_WGL::timerEvent( uint32_t timerId )
{
   update();
}
...
void Window_WGL::update()
{
   if ( m_hWnd )
   {
      ...
      r_window.top = 0;
      r_window.left = 0;
      r_window.right = m_screenWidth;
      r_window.bottom = m_screenHeight;
      InvalidateRect( m_hWnd, &r_window, FALSE );
   }
}
...
void
Window_WGL::setEventReceiver( de::EventReceiver* receiver )
{
...
{
   if ( params.eventReceiver )
   {
      m_eventReceiver = params.eventReceiver;
   }
   ...
   HWND parent = nullptr;
   //HWND parent = reinterpret_cast<HWND>(params.parentWindow);
//   if ( parent )
//   {
//      m_windowedStyle |= WS_CHILD;
//      m_fullscreenStyle |= WS_CHILD;
//   }
...
//   if ( params.parentWindow != 0 )
//   {
//      m_hWnd = reinterpret_cast<HWND>(params.parentWindow);
//      SetWindowLongPtr( m_hWnd, GWL_USERDATA, reinterpret_cast< LONG_PTR >( this ) );
//      SetWindowLongPtr( m_hWnd, GWL_STYLE, LONG_PTR(m_windowStyle) );
//      SetWindowLongPtr( m_hWnd, GWL_WNDPROC, reinterpret_cast< LONG_PTR >( Window_WGL_WndProc ) );
//      GWL_ID
//   }
//   else
//   {
      ...
      m_hWnd = CreateWindowA( className.c_str(),
                              "3D Oscilloscope and Spectrograph Analyser",
                              m_windowStyle,
                              winX, winY, winW, winH,
                              parent,
                              nullptr,
...
//   {
//      HWND parent = reinterpret_cast<HWND>(params.parentWindow);
//      SetWindowLongPtr( m_hWnd, GWLP_HWNDPARENT, reinterpret_cast< LONG_PTR >( parent ) );
//   }

   DE_DEBUG("Hello Window")
...
// #ifdef WGL_ARB_pixel_format
   ...
   */
   ...
   if ( m_eventReceiver )
   {
      m_eventReceiver->windowCreated( this );
   }
...
//   }
   ...
   switch (message)
   {
      ...
      case WM_CLOSE: {
         ...
         {
            glwin->getEventReceiver()->windowAboutToBeDestroyed( glwin );
      ...
      case WM_DESTROY: {
         ...
         {
            glwin->getEventReceiver()->windowAboutToBeDestroyed( glwin );
            glwin->setEventReceiver( nullptr );
         }
         //DE_DEBUG("WM_DESTROY")
         //Sleep(200);

         //if ( glwin && glwin->m_params.parentWindow )
         //{
         //   DG_DEBUG("No PostQuitMessage(0)")
         //}
         //else
         //{
            PostQuitMessage(0);
      ...
      case WM_PAINT: {
         ...
         if ( glwin )
         {
            ...
            {
               PaintEvent event;
               event.w = glwin->m_screenWidth;
               event.h = glwin->m_screenHeight;
               glwin->getEventReceiver()->paintEvent( event );
            }
//            else
//            {
