#pragma once
#include "de_TMeshBuffer.h"
#include "de_FVF.h"
#include "de_Math3D_Quaternion.h"
...
// ===========================================================================
{
   ...
   virtual glm::vec3 const & getPos( uint32_t i ) const { return m_vertices[ i ].pos; }
   virtual glm::vec3 &       getPos( uint32_t i ) { return m_vertices[ i ].pos; }
   ...
   recalculateBoundingBox() override
   {
      ...
      {
         BoundingBox.reset( m_vertices[ 0 ].getPos() );

         for ( size_t i = 1; i < m_vertices.size(); ++i )
         {
            BoundingBox.addInternalPoint( m_vertices[ i ].getPos() );
         }
      }
   }
   ...
   virtual void rotate( glm::vec3 new_nrm, glm::vec3 old_nrm = glm::vec3(0.0f,0.0f,-1.0f) )
   {
      if ( old_nrm != new_nrm )
      {
         ...
         {
            auto v1 = m_vertices[ i ].getPos() - cc; // dir from center to pos
