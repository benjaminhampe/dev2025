#pragma once
#include "de_ShiftBuffer.h"

namespace de {

// does only swap pointers and does not move memory!
// push() always refills the 0-th row. If m_shiftBuffer collected enough
// samples to fill a new row.
// A ShiftBuffer is used to decouple different audio callback size and
// matrix column size, is only meaningful if m_dacFrames <= m_shiftBuffer.size().
//
