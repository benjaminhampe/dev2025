...
LinearColorGradient::lerpColor128( uint32_t A, uint32_t B, float t )
{
   ...
   //
   float const startR = RGBA_Rf( A ); float const deltaR = RGBA_Rf( B ) - startR;
   float const startG = RGBA_Gf( A ); float const deltaG = RGBA_Gf( B ) - startG;
   float const startB = RGBA_Bf( A ); float const deltaB = RGBA_Bf( B ) - startB;
   float const startA = RGBA_Af( A ); float const deltaA = RGBA_Af( B ) - startA;
   float const r = dbClampf( startR + t * deltaR, 0.0f, 1.0f );
   float const g = dbClampf( startG + t * deltaG, 0.0f, 1.0f );
   float const b = dbClampf( startB + t * deltaB, 0.0f, 1.0f );
   float const a = dbClampf( startA + t * deltaA, 0.0f, 1.0f );
...
LinearColorGradient::hasTransparentColor() const
{
   ...
   {
      if ( RGBA_A( colorStop.color ) < 255 ) return true;
   }
   return false;
}
...
LinearColorGradient::getColor128( float t ) const
{
   ...
   if ( m_Stops.size() < 2 )
   {
      return RGBAf( m_Stops.back().color );
   }
   ...
   else
   {
      ...
      {
         if ( dbEquals( t, m_Stops[ i ].stop ) )
         {
            return RGBAf( m_Stops[ i ].color );
         }
      ...
      if ( found < 0 )
      {
         return RGBAf( m_Stops[ m_Stops.size()-1 ].color );
      }
