...
struct JPGWriter
{
   ...
   callback_init_destination( j_compress_ptr ctx )
   {
      if ( !ctx )
      {
         std::cout << "JPG." << __FUNCTION__ << " :: [Error] Invalid j_compress_ptr.\n";
         return;
      }
      ...
      if ( !dst )
      {
         std::cout << "JPG." << __FUNCTION__ << " :: [Error] Invalid mem_dest_ptr.\n";
         return;
      }
   ...
   callback_empty_output_buffer( j_compress_ptr ctx )
   {
      ...
      if ( !dst->file )
      {
         std::cout << "JPG." << __FUNCTION__ << " :: [Error] Invalid FILE ptr.\n";
         return FALSE;
      }
      if ( !dst->buffer )
      {
         std::cout << "JPG." << __FUNCTION__ << " :: [Error] Invalid dst_buffer_ptr.\n";
         return FALSE;
      }
      ...
      {
         std::cout << "JPG." << __FUNCTION__ << " :: [Error] bufferBytes() != writtenBytes().\n";
   ...
   callback_term_destination( j_compress_ptr ctx )
   {
      ...
      {
         std::cout << "JPG." << __FUNCTION__ << " :: [Error] remainBytes() != writtenBytes().\n";
         ERREXIT( ctx, JERR_FILE_WRITE );
      }
   }
   ...
   setup( j_compress_ptr ctx, FILE* file )
   {
      if ( !ctx )
      {
         std::cout << "JPG." << __FUNCTION__ << " :: [Error] Invalid pointer to context.\n";
         return;
      }
      if ( !file )
      {
         std::cout << "JPG." << __FUNCTION__ << " :: [Error] Invalid pointer to FILE.\n";
         return;
      }
      ...
      if ( !ctx->dest )
      {
         std::cout << "JPG." << __FUNCTION__ << " :: [Error] Allocation failed to destination_mgr.\n";
         return;
      }
