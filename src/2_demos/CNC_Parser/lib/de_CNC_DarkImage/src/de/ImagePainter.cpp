...
ImagePainter::brighten( Image & img, float factor )
{
   for ( int32_t y = 0; y < img.getHeight(); ++y )
   {
      for ( int32_t x = 0; x < img.getWidth(); ++x )
      {
         ...
         int r = dbClampi( dbRound32( factor * float( RGBA_R( color ) ) ), 0, 255 );
         int g = dbClampi( dbRound32( factor * float( RGBA_G( color ) ) ), 0, 255 );
         int b = dbClampi( dbRound32( factor * float( RGBA_B( color ) ) ), 0, 255 );
...
ImagePainter::autoSaturate( Image & img )
{
   ...
   for ( int32_t y = 0; y < img.getHeight(); ++y )
   {
      ...
      {
         uint32_t color = img.getPixel( x,y );
         int r = dbClampi( (255 * (RGBA_Ri( color ) - r1)) / (r2-r1), 0, 255 );
         int g = dbClampi( (255 * (RGBA_Gi( color ) - g1)) / (g2-g1), 0, 255 );
         int b = dbClampi( (255 * (RGBA_Bi( color ) - b1)) / (b2-b1), 0, 255 );
         // int a = dbClampi( ( 255 * ( RGBA_A( color ) - sA )) / dA, 0, 255 );
         img.setPixel( x,y, RGBA( r,g,b,RGBA_A( color ) ) );
      }
   }
}
...
ImageRotate::rotate( Image const & img, float angleDegreesCCW, uint32_t colorKey )
{
   ...
   {
      // alpha
      float const a = ( 360.0f - angleDegreesCCW ) * Math::DEG2RAD;
      float const sa = sinf( a );
      float const ca = cosf( a );

      // gamma
      float const g = angleDegreesCCW * Math::DEG2RAD;
...
//   {
      ...
      iA = dbClampi( dbRound32( fa ), 0, 255 );
//   }
...
//   {
      ...
      iR = dbClampi( dbRound32( fr ), 0, 255 );
//   }
...
//   {
      ...
      iG = dbClampi( dbRound32( fg ), 0, 255 );
//   }
...
//   {
      ...
      iB = dbClampi( dbRound32( fb ), 0, 255 );
//   }
