...
// Clamp:
inline int32_t dbClamp( int32_t v, int32_t vmin, int32_t vmax ) { return std::clamp( v, vmin, vmax ); }
inline float dbClamp( float v, float vmin, float vmax ) { return std::clamp( v, vmin, vmax ); }
inline double dbClamp( double v, double vmin, double vmax ) { return std::clamp( v, vmin, vmax ); }
inline int32_t dbClampi( int32_t v, int32_t vmin, int32_t vmax ) { return std::clamp( v, vmin, vmax ); }
inline float dbClampf( float v, float vmin, float vmax ) { return std::clamp( v, vmin, vmax ); }
inline double dbClampd( double v, double vmin, double vmax ) { return std::clamp( v, vmin, vmax ); }
...
//}
//inline int32_t dbClampi( int32_t v, int32_t vmin, int32_t vmax ) { if ( vmax < vmin ) { std::swap( vmin, vmax ); } return std::min( std::max( v, vmin ), vmax ); }
//inline float dbClampf( float v, float vmin, float vmax ) { if ( vmax < vmin ) { std::swap( vmin, vmax ); } return std::min( std::max( v, vmin ), vmax ); }
//inline double dbClampd( double v, double vmin, double vmax ) { if ( vmax < vmin ) { std::swap( vmin, vmax ); } return std::min( std::max( v, vmin ), vmax ); }

// Reciprocal:
inline float dbReciprocal( float v ) { float i = 1.0f / v; if ( std::isinf( i ) || std::isnan( i ) ) { return 0.0f; } return i; }
inline double dbReciprocal( double v ) { double i = 1.0 / v; if ( std::isinf( i ) || std::isnan( i ) ) { return 0.0; } return i; }
// Equals:
...
inline bool dbEquals( glm::vec3 const & a, glm::vec3 const & b, float eps = 1.0e-6 )
{
   return dbEquals( a.x,b.x,eps ) && dbEquals( a.y,b.y,eps ) && dbEquals( a.z,b.z,eps );
}
inline bool dbEquals( glm::dvec3 const & a, glm::dvec3 const & b, double eps = 1e-6 )
{
   return dbEquals( a.x, b.x, eps ) && dbEquals( a.y, b.y, eps ) && dbEquals( a.z, b.z, eps );
}
// IsZero:
inline float dbIsZero( float v ) { return dbEquals( v, 0.0f ); }
inline double dbIsZero( double v ) { return dbEquals( v, 0.0, 1.0e-15 ); }
// Round:
inline float dbRound( float v ) { return std::round( v ); }
inline double dbRound( double v ) { return std::round( v ); }
// Round32:
inline int32_t dbRound32( float v ) { return std::round( v ); }
inline int32_t dbRound32( double v ) { return std::round( v ); }
// Round64:
inline int64_t dbRound64( float v ) { return std::round( v ); }
inline int64_t dbRound64( double v ) { return std::round( v ); }
// SafeModulo with negative numbers
inline int32_t dbModSafe( int32_t a, int32_t b ) { if ( a == 0 || b == 0 ) { return 0; } return std::abs( a ) % std::abs( b ); }
// Byte2float
inline float dbByte2float( uint8_t b_0_255 ) { return float( b_0_255 ) / 255.0f; }
// Float2byte
inline uint8_t dbFloat2byte( float f_0_1 ) { return uint8_t( dbClamp( int( f_0_1 * 255.0f ), 0, 255 ) ); }
