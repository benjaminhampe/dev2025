...
struct WallpaperShaderArt_F04
{
   IShaderManager* m_driver;
   IShader* m_shader;

   WallpaperShaderArt_F04()
      : m_driver(nullptr)
      , m_shader(nullptr)
   {
   ...
   void init( IShaderManager* shaderManager )
   {
      ...
      if ( !m_shader )
      {
         m_shader = m_driver->createShader( "shaderArt_F04",
            ...
            void main()
            {
               ...
               for ( float i = 0.0; i < 3.0; i++ )
               {
                  ...
                  vec3 color = paletteH( length(uv0) + i * 0.4 + u_timeInSeconds * 0.4 );
