...
static GLubyte* g_pixels; //[WIDTH * HEIGHT * BYTES_PER_PIXEL];
...
GLUSboolean init(GLUSvoid)
{
   ...
   glusFileLoadText("../../src/Example29_raytrace/shader/fullscreen.vert.glsl", &vertexSource);
   glusFileLoadText("../../src/Example29_raytrace/shader/texture.frag.glsl", &fragmentSource);
   ...
   if (!renderToPixelBuffer(g_pixels, WIDTH, HEIGHT))
   {
   ...
   glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, WIDTH, HEIGHT, 0, GL_RGB, GL_UNSIGNED_BYTE, g_pixels);
...
GLUSvoid reshape(GLUSint width, GLUSint height)
{
   ...
   free(g_directionBuffer);
   free(g_positionBuffer);
   free(g_pixels);
   g_directionBuffer = (GLfloat*)malloc(width * height * 3 * sizeof(GLfloat));
   g_positionBuffer = (GLfloat*)malloc(width * height * 4 * sizeof(GLfloat));
   g_pixels = (GLubyte*)malloc(width * height * BYTES_PER_PIXEL * sizeof(GLubyte));
 ...
 */
GLUSboolean render(GLUSfloat time)
{
...
int main(int argc, char* argv[])
{
   ...
   g_pixels = (GLubyte*)malloc(WIDTH * HEIGHT * BYTES_PER_PIXEL * sizeof(GLubyte));
    ...
    glusWindowSetUpdateFunc(render);

    glusWindowSetTerminateFunc(terminate);

   // To keep the program simple, no resize of the window.
    if (!glusWindowCreate("GLUS Example29_raytrace Window", WIDTH, HEIGHT, GLUS_FALSE, GLUS_FALSE, eglConfigAttributes, eglContextAttributes, 0))
    {
