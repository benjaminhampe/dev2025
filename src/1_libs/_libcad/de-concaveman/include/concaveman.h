...
public:
    typedef rtree<T, DIM, MAX_CHILDREN, DATA> TreeType;
    typedef const TreeType ConstTreeType;
    typedef TreeType *TreeTypePtr;
    typedef const TreeType *TreeTypeConstPtr;
    ...
    rtree(DataType data, const bounds_type &bounds):
    ...
    void insert(DataType data, const bounds_type &bounds) {
        if (m_is_leaf)
          throw std::runtime_error("Cannot insert into leaves");

        m_bounds = updated_bounds(bounds);
        if (m_children.size() < MAX_CHILDREN) {
            auto r = std::make_unique<TreeType>(data, bounds);
            m_children.push_back(std::move(r));
            return;
        }

        std::reference_wrapper<TreeType> best_child = *m_children.begin()->get();
    ...
    std::vector<std::reference_wrapper<ConstTreeType>> intersection(const bounds_type& bounds) const {
        std::vector<std::reference_wrapper<ConstTreeType>> res;
    ...
    void erase(DataType data, const bounds_type &bounds) {
        if (m_is_leaf)
    ...
    const std::list<std::unique_ptr<TreeType>>& children() const {
        return m_children;
    }
...
private:
    bool m_is_leaf;
    DataType m_data;
    std::list<std::unique_ptr<TreeType>> m_children;
    bounds_type m_bounds;
};


template<class T> struct Node {
    typedef Node<T> TreeType;
    typedef TreeType *TreeTypePtr;
...
public:
    typedef CircularElement<T> TreeType;
    typedef TreeType *ptr_type;
