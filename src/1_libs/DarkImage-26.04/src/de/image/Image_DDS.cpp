...
bool
ImageReaderDDS::load( Image & img, const uint8_t* p, size_t n, const std::string& uri )
{
    Binary file(p,n,uri);
...
#define	__BIG_ENDIAN__
   ...
   float32_t DDSBigFloat( float32_t src ) { return src; }
   ...
   float32_t DDSLittleFloat( float32_t src )
	{
   ...
   float32_t DDSLittleFloat( float32_t src ) { return src; }
   ...
   float32_t DDSBigFloat( float32_t src )
	{
		floatSwapUnion in,out;
		in.f = src;
		out.c[ 0 ] = in.c[ 3 ];
