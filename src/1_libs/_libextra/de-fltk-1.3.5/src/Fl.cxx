...
//           window, pushed(), grab(), modal());
  ...
  case FL_KEYBOARD:
    ...
    // changing the text and falling through to FL_SHORTCUT case:
    {
      ...
      if (!isalpha(int(*c))) return 0;
       *c = isupper(int(*c)) ? char(tolower(int(*c))) 
                             : char(toupper(int(*c)));
    }
...
bool Fl::option(Fl_Option opt)
{
  if (!options_read_) {
    ...
    { // first, read the system wide preferences
      ...
      options_[OPTION_ARROW_FOCUS] = uchar(tmp);
      ...
      options_[OPTION_VISIBLE_FOCUS] = uchar(tmp);
      opt_prefs.get("DNDText", tmp, 1);                         // default: on
      options_[OPTION_DND_TEXT] = uchar(tmp);
      opt_prefs.get("ShowTooltips", tmp, 1);                    // default: on
      options_[OPTION_SHOW_TOOLTIPS] = uchar(tmp);
      opt_prefs.get("FNFCUsesGTK", tmp, 1);                    // default: on
      options_[OPTION_FNFC_USES_GTK] = uchar(tmp);
    }
    { // next, check the user preferences
      ...
      if (tmp >= 0) options_[OPTION_ARROW_FOCUS] = uchar(tmp);
      ...
      if (tmp >= 0) options_[OPTION_VISIBLE_FOCUS] = uchar(tmp);
      opt_prefs.get("DNDText", tmp, -1);
      if (tmp >= 0) options_[OPTION_DND_TEXT] = uchar(tmp);
      opt_prefs.get("ShowTooltips", tmp, -1);
      if (tmp >= 0) options_[OPTION_SHOW_TOOLTIPS] = uchar(tmp);
      opt_prefs.get("FNFCUsesGTK", tmp, -1);
      if (tmp >= 0) options_[OPTION_FNFC_USES_GTK] = uchar(tmp);
    }
