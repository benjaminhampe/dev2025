...
const Fl_Menu_Item* Fl_Menu_Item::popup(
  int oX, int oY,
  const char* oTitle,
  const Fl_Menu_Item* oPicked,
  const Fl_Menu_* oButton
  ) const {
  static Fl_Menu_Item s_dummy; // static so it is all zeros
  s_dummy.text = oTitle;
  return pulldown(oX, oY, 0, 0, oPicked, oButton, oTitle ? &s_dummy : 0);
}
