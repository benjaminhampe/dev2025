...
void Fl::add_fd(int oFd, void (*cb)(FL_SOCKET, void*), void* v) 
{
  Fl::add_fd(oFd, FL_READ, cb, v);
}
	...
	*b = 0;
    ...
    { 
      ...
      if (lpBI)
      {
          ...
          { // direct use of the DIB data if it's RGB or RGBA
          ...
          { // the system will decode a complex DIB
...
static int ms2fltk(WPARAM vk, int extended) {
  ...
  if (!vklut[1]) { // init the table
    unsigned int i;
    for (i = 0; i < 256; i++) 
        vklut[i] = (unsigned short)tolower(i);
    for (i=VK_F1; i<=VK_F16; i++) 
        vklut[i] = (unsigned short)(i+(FL_F-(VK_F1-1)));
    for (i=VK_NUMPAD0; i<=VK_NUMPAD9; i++) 
        vklut[i] = (unsigned short)(i+(FL_KP+'0'-VK_NUMPAD0));
...
//    Fl::e_length = fl_unicode2utf(&u, 1, buffer);
	...
	buffer[0] = char(Fl::e_keysym-FL_KP);
	Fl::e_length = 1;
      } else {
...
Fl_X* Fl_X::make(Fl_Window* w) {
  ...
  if (w->parent()) {
    ...
    if (w->non_modal() && Fl_X::first && !fl_disable_transient_for) {
      ...
      Fl_Window* w2 = Fl_X::first->w; // TODO: Fl_Window* w?
      while (w2->parent()) w2 = w2->window();
      parent = fl_xid(w2);
      if (!w2->visible()) showit = 0;
...
#define DESKTOPHORZRES 118
        ...
        scaling = int(scaling * 100 + 0.5f)/100.f; // round to 2 digits after decimal point
      }
    }
    if (need_r) {
...
int Fl_Window::decorated_h()
{
  ...
  return h() + (int)(((float)bt)/scaling) + 2 * by;
}

void Fl_Paged_Device::print_window(Fl_Window *win, int oXoffset, int oYoffset)
{
  draw_decorated_window(win, oXoffset, oYoffset, this);
}

void Fl_Paged_Device::draw_decorated_window(Fl_Window *win, int oXoffset, int oYoffset, Fl_Surface_Device *toset)
{
  ...
  if (bt) {
    ...
    //fl_draw_image(top_image, oXoffset, oYoffset, ww, bt + by, 3);
    ...
    if (scaling > 1) {
      ...
      Fl_RGB_Image *tmp_img = (Fl_RGB_Image*)top_r->copy(ww, (int)((float)(bt + by)/scaling));
      Fl_Image::RGB_scaling(current);
      delete top_r;
      top_r = tmp_img;
    }
    top_r->draw(oXoffset, oYoffset);
    delete top_r;
    
    if (left_image) { fl_draw_image(left_image, oXoffset, oYoffset, bx, wh, 3); delete left_image; }
    if (right_image) { fl_draw_image(right_image, oXoffset + win->w() + bx, oYoffset, bx, wh, 3); delete right_image; }
    if (bottom_image) { fl_draw_image(bottom_image, oXoffset, oYoffset + win->h() + bt + by, ww, by, 3); delete bottom_image; }
  }
  // draw the window inner part
  this->print_widget(win, oXoffset + bx, oYoffset + (int)((float)(bt + by)/scaling));
}
