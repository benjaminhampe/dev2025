...
static int AnalyzeEachCHierarchy(unsigned subleaf, unsigned  numMappings)
{
   ...
   pEachCIDs= reinterpret_cast< uint32_t* >( _alloca( numMappings * sizeof(unsigned ) ) );
   if(pEachCIDs == NULL) return -1;
   memset(pEachCIDs, 0xff, numMappings* sizeof(unsigned ));
   pThreadIDsperEachC = reinterpret_cast< uint32_t* >( _alloca(    numMappings * sizeof(unsigned )) );
   ...
   pDetectedEachCIDs= reinterpret_cast< uint32_t* >( _alloca( numMappings * sizeof(unsigned ) ) );
   if(pDetectedEachCIDs == NULL) return -1;
   memset(pDetectedEachCIDs, 0xff, numMappings* sizeof(unsigned ));
   pDetectThreadIDsperEachC = reinterpret_cast< uint32_t* >( _alloca(    numMappings * maxCacheDetected * sizeof(unsigned )) );
