	...
	public:
		...
		*/
		virtual void append(const void* const vertices, u32 numVertices, const u16* const indices, u32 numIndices)
		{
            ...
            u32 const v0 = getVertexCount();
            u32 const i0 = getIndexCount();

            Vertices.reallocate(v0+numVertices);
            Vertices.set_used(v0);

            auto tVertices = reinterpret_cast<T const*>(vertices);
            for (u32 i = 0; i < numVertices; ++i)
			{
                Vertices.push_back(tVertices[i]);
                BoundingBox.addInternalPoint(tVertices[i].Pos);
			}


            Indices.reallocate(i0+numIndices);
            Indices.set_used(i0);
            for (u32 i = 0; i < numIndices; ++i)
			{
                Indices.push_back(indices[i]+v0);
			}
		}
