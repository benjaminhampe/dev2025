...
namespace
{
   inline u32 F2DW( f32 f )
   {
      u32 u = *reinterpret_cast< u32 const* >( &f );
      return u;
   }
}
...
bool
CD3D9Driver::initDriver(
   HWND hwnd,
   bool pureSoftware )
{
   if (!pID3D)
   {
      D3DLibrary = LoadLibrary( TEXT("d3d9.dll") );
      ...
      const core::matrix4& mat)
{
   ...
   switch(state)
   {
   case ETS_VIEW: pID3DDevice->SetTransform(D3DTS_VIEW, (D3DMATRIX*)((void*)mat.pointer())); break;
   case ETS_WORLD: pID3DDevice->SetTransform(D3DTS_WORLD, (D3DMATRIX*)((void*)mat.pointer())); break;
   case ETS_PROJECTION: pID3DDevice->SetTransform( D3DTS_PROJECTION, (D3DMATRIX*)((void*)mat.pointer())); break;
   case ETS_COUNT: return;
   default:
      s32 ds = s32(state) - s32(ETS_TEXTURE_0);
      if (ds >= 0 && ds < s32(MATERIAL_MAX_TEXTURES))
      {
         if (mat.isIdentity())
      ...
      E_INDEX_TYPE iType, bool is3D)
{
   ...
   case scene::EPT_LINE_LOOP:
      ...
      else
      {
         ...
         u16 tmpIndices[] = { u16(primitiveCount - 1), u16(0) };
      ...
      bool useAlphaChannelOfTexture)
{
   ...
   // core::dimension2du renderTargetSize = getCurrentRenderTargetSize();
...
s32 CD3D9Driver::addDynamicLight(const SLight& dl)
{
   ...
   switch (dl.Type)
   {
   //case ELT_POINT: light.Type = D3DLIGHT_POINT; break;
   case ELT_SPOT: light.Type = D3DLIGHT_SPOT; break;
   case ELT_DIRECTIONAL: light.Type = D3DLIGHT_DIRECTIONAL; break;
   default: break;
   }

   light.Position = *reinterpret_cast< D3DVECTOR const* >( &dl.Position );
   light.Direction = *reinterpret_cast< D3DVECTOR const* >( &dl.Direction );

   light.Range = core::min_(dl.Radius, MaxLightDistance);
   light.Falloff = dl.Falloff;

   light.Diffuse = *reinterpret_cast< D3DCOLORVALUE const* >( &dl.DiffuseColor );
   light.Specular = *reinterpret_cast< D3DCOLORVALUE const* >( &dl.SpecularColor );
   light.Ambient = *reinterpret_cast< D3DCOLORVALUE const* >( &dl.AmbientColor );
...
D3DFORMAT CD3D9Driver::getD3DFormatFromColorFormat(ECOLOR_FORMAT format) const
{
   switch(format)
   {
      case ECF_A1R5G5B5: return D3DFMT_A1R5G5B5;
      case ECF_R5G6B5: return D3DFMT_R5G6B5;
      case ECF_R8G8B8: return D3DFMT_R8G8B8;
      case ECF_A8R8G8B8: return D3DFMT_A8R8G8B8;
      // Floating Point formats. Thanks to Patryk "Nadro" Nadrowski.
      case ECF_R16F: return D3DFMT_R16F;
      case ECF_G16R16F: return D3DFMT_G16R16F;
      case ECF_A16B16G16R16F: return D3DFMT_A16B16G16R16F;
      case ECF_R32F: return D3DFMT_R32F;
      case ECF_G32R32F: return D3DFMT_G32R32F;
      case ECF_A32B32G32R32F: return D3DFMT_A32B32G32R32F;
