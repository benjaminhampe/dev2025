   ...
   case WM_SIZE:
      {
         ...
         if (dev)
         {
            dev->OnResized();
            irr::SEvent event;
            event.EventType = irr::EET_RESIZE_EVENT;
            event.ResizeEvent.width = LOWORD( lParam );
            event.ResizeEvent.height = HIWORD( lParam );
            dev->postEventFromUser( event );
         }
      }
      //return 0;
   ...
   ExternalWindow(false), Win32CursorControl(0), JoyControl(0)
{
   ...
   if (!CreationParams.WindowId && CreationParams.DriverType != video::EDT_NULL)
   {
      ...
      wcex.hIcon = nullptr; // (HICON)LoadImage(hInstance, __TEXT("irrlicht.ico"), IMAGE_ICON, 0,0, LR_LOADFROMFILE);
...
//! sets the icon of the window by a resource id contained in a .rc resource file.
void CIrrDeviceWin32::setWindowIcon(int resourceID )
{
   auto ico = LoadIcon( GetModuleHandle( nullptr ), MAKEINTRESOURCE(resourceID) );
   if (!ico) return;
   SetClassLongPtr( HWnd, GCLP_HICON,   reinterpret_cast<LONG_PTR>( ico ) );
   SetClassLongPtr( HWnd, GCLP_HICONSM, reinterpret_cast<LONG_PTR>( ico ) );
}
