...
void
CGeometryCreator::addCube( SMeshBuffer & o
   , core::vector3df const & siz
   , core::vector3df const & pos
   , video::SColor const & colorNX
   , video::SColor const & colorNY
   , video::SColor const & colorNZ
   , video::SColor const & colorPX
   , video::SColor const & colorPY
   , video::SColor const & colorPZ
   , bool flip
   , core::vector2df const & texScale ) const
{
   typedef core::vector2df V2;
   typedef core::vector3df V3;
   typedef video::S3DVertex S3D;

   auto addQuad = [] ( SMeshBuffer & o, S3D const & a, S3D const & b,
         S3D const & c, S3D const & d, bool flip = false )
   {
      size_t const v0 = o.Vertices.size();
      size_t const i0 = o.Indices.size();
      o.Vertices.reallocate( v0 + 6 );
      o.Vertices.set_used( v0 );
      o.Indices.reallocate( i0 + 6 );
      o.Indices.set_used( i0 );
      if ( flip )
      {
      ...
      o.Indices.push_back( v0 );
      o.Indices.push_back( v0 + 1 );
      o.Indices.push_back( v0 + 2 );
      o.Indices.push_back( v0 + 3 );
      o.Indices.push_back( v0 + 4 );
      o.Indices.push_back( v0 + 5 );
   };
   //
   //        F-------G                 tex atlas infos         +----+----+---------+
   //       /|      /|                                         |    |    |         |
   ...
   float dy = siz.Y * 0.5f;
   float dz = siz.Z * 0.5f;
   auto a = pos + V3( -dx, -dy, -dz );
   auto b = pos + V3( -dx,  dy, -dz );
   auto c = pos + V3(  dx,  dy, -dz );
   auto d = pos + V3(  dx, -dy, -dz );
   auto e = pos + V3( -dx, -dy,  dz );
   auto f = pos + V3( -dx,  dy,  dz );
   auto g = pos + V3(  dx,  dy,  dz );
   auto h = pos + V3(  dx, -dy,  dz );

   auto const tA = texScale*V2(0,1);
   auto const tB = texScale*V2(0,0);
   auto const tC = texScale*V2(1,0);
   auto const tD = texScale*V2(1,1);

   // Front quad ABCD
   {  n = V3(0,0,-1);
      S3D A( a, n, colorNZ, tA );
      S3D B( b, n, colorNZ, tB );
      S3D C( c, n, colorNZ, tC );
      S3D D( d, n, colorNZ, tD );
      addQuad( o,A,B,C,D, flip );
   }
   // Back quad HGFE
   {  n = V3(0,0,1);
      S3D H( h, n, colorPZ, tA );
      S3D G( g, n, colorPZ, tB );
      S3D F( f, n, colorPZ, tC );
      S3D E( e, n, colorPZ, tD );
      addQuad( o,H,G,F,E, flip );
   }
   // Left quad EFBA
   {  n = V3(-1,0,0);
      S3D E( e, n, colorNX, tA );
      S3D F( f, n, colorNX, tB );
      S3D B( b, n, colorNX, tC );
      S3D A( a, n, colorNX, tD );
      addQuad( o,E,F,B,A, flip );
   }
   // Right quad DCGH
   {  n = V3(1,0,0);
      S3D D( d, n, colorPX, tA );
      S3D C( c, n, colorPX, tB );
      S3D G( g, n, colorPX, tC );
      S3D H( h, n, colorPX, tD );
      addQuad( o,D,C,G,H, flip );
   }
   // Top quad BFGC
   {  n = V3(0,1,0);
      S3D B( b, n, colorPY, tA );
      S3D F( f, n, colorPY, tB );
      S3D G( g, n, colorPY, tC );
      S3D C( c, n, colorPY, tD );
      addQuad( o, B,F,G,C, flip );
   }
   // Bottom quad EADH
   {  n = V3(0,-1,0);
      S3D E( e, n, colorNY, tA );
      S3D A( a, n, colorNY, tB );
      S3D D( d, n, colorNY, tC );
      S3D H( h, n, colorNY, tD );
      addQuad( o, E,A,D,H, flip );
   }
}
