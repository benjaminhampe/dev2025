...
IFileList* CFileSystem::createFileList()
{
	...
	if (FileSystemType == FILESYSTEM_NATIVE)
	{
        ...
        hFile = _tfindfirst64( _T("*"), &c_file );
        if (hFile != -1)
		{
			do
			{
                bool const bIsDirectory = (_A_SUBDIR & c_file.attrib) != 0;
                r->addItem(Path + c_file.name, 0, static_cast<uint64_t>(c_file.size), bIsDirectory, 0);
			}
