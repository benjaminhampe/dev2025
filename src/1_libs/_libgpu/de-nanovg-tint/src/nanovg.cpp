...
static float nvg__signf(float a)
{
    ...
    return float(u >> 31) * 2.0f - 1.0f; // -> -1.0f or +1.0f
}
static float nvg__absf(float a) { return fabsf(a); }
static float nvg__minf(float a, float b) { return fminf(a,b); }
static float nvg__maxf(float a, float b) { return fmaxf(a,b); }
static float nvg__clampf(float a, float mn, float mx) { return fminf(fmaxf(a,mn),mx); }
