 ...
 */
int
__db_tas_mutex_init(ENV *env, db_mutex_t mutex, u_int32_t flags)
	// ENV *env;
	// db_mutex_t mutex;
	// u_int32_t flags;
{
 ...
 */
inline static int
__db_tas_mutex_lock_int(ENV *env, db_mutex_t mutex, db_timeout_t timeout, int nowait)
	// ENV *env;
	// db_mutex_t mutex;
 ...
 */
int
__db_tas_mutex_lock(ENV *env, db_mutex_t mutex, db_timeout_t timeout)
	// ENV *env;
	// db_mutex_t mutex;
	// db_timeout_t timeout;
{
 ...
 */
int
__db_tas_mutex_trylock(ENV *env, db_mutex_t mutex)
	// ENV *env;
	// db_mutex_t mutex;
{
 ...
 */
static inline int
__db_tas_mutex_readlock_int(ENV *env, db_mutex_t mutex, int nowait)
	// ENV *env;
	// db_mutex_t mutex;
	// int nowait;
{
...
loop:	/* Attempt to acquire the resource for N spins. */
	for (nspins =
	    mtxregion->stat.st_mutex_tas_spins; nspins > 0; --nspins) {
		lock = db_atomic_read(&mutexp->sharecount);
	 ...
	 */
	if (nowait) {
		if (db_atomic_read(&mutexp->sharecount) != MUTEX_SHARE_ISEXCLUSIVE)
			goto loop;
		return (DB_LOCK_NOTGRANTED);
	}
 ...
 */
int
__db_tas_mutex_readlock(ENV *env, db_mutex_t mutex)
	// ENV *env;
	// db_mutex_t mutex;
{
 ...
 */
int
__db_tas_mutex_tryreadlock(ENV *env, db_mutex_t mutex)
	// ENV *env;
	// db_mutex_t mutex;
{
 ...
 */
int
__db_tas_mutex_unlock(ENV *env, db_mutex_t mutex)
 //    ENV *env;
	// db_mutex_t mutex;
{
	DB_ENV *dbenv;
...
#ifdef HAVE_SHARED_LATCHES
	if (F_ISSET(mutexp, DB_MUTEX_SHARED)) {
		sharecount = db_atomic_read(&mutexp->sharecount);
 ...
 */
int
__db_tas_mutex_destroy(ENV *env, db_mutex_t mutex)
	// ENV *env;
	// db_mutex_t mutex;
{
	DB_MUTEX *mutexp;
