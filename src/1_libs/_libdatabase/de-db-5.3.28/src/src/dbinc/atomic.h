 ...
 */
#if defined(DB_WIN32)
typedef DWORD	db_atomic_value_t;
#else
typedef int32_t	 db_atomic_value_t;
#endif
...
#else
typedef struct {
	volatile db_atomic_value_t value;
} db_atomic_t;
#endif
 ...
 */
#define	db_atomic_read(p)		((p)->value)
 ...
 */
#define	db_atomic_inc(env, p)	(++(p)->value)
#define	db_atomic_dec(env, p)	(--(p)->value)
#define	db_atomic_compare_exchange(env, p, oldval, newval)		\
	(DB_ASSERT(env, db_atomic_read(p) == (oldval)),		\
	db_atomic_init(p, (newval)), 1)
#else
#define db_atomic_inc(env, p)	__atomic_inc(env, p)
#define db_atomic_dec(env, p)	__atomic_dec(env, p)
#endif
#endif
