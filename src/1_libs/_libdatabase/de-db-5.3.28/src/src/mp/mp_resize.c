...
static int __memp_merge_buckets __P((DB_MPOOL *, u_int32_t, u_int32_t, u_int32_t));
 ...
 */
int
__memp_get_bucket(ENV *env, MPOOLFILE *mfp, db_pgno_t pgno, REGINFO **infopp, DB_MPOOL_HASH **hpp, u_int32_t *bucketp)
	// ENV *env;
	// MPOOLFILE *mfp;
	// db_pgno_t pgno;
	// REGINFO **infopp;
...
static int
__memp_merge_buckets(DB_MPOOL *dbmp, u_int32_t new_nbuckets, u_int32_t old_bucket, u_int32_t new_bucket)
	// DB_MPOOL *dbmp;
...
retry:	MUTEX_LOCK(env, old_hp->mtx_hash);
	SH_TAILQ_FOREACH(bhp, &old_hp->hash_bucket, hq, __bh) {
		...
		if (bucket == new_bucket && db_atomic_read(&bhp->ref) != 0) {
...
static int
__memp_add_bucket(DB_MPOOL *dbmp)
//	DB_MPOOL *dbmp;
{
	ENV *env;
	MPOOL *mp;
...
static int
__memp_add_region(DB_MPOOL *dbmp)
	// DB_MPOOL *dbmp;
{
	ENV *env;
	MPOOL *mp;
...
static int
__memp_remove_bucket(DB_MPOOL *dbmp)
	// DB_MPOOL *dbmp;
{
	ENV *env;
	MPOOL *mp;
...
static int
__memp_remove_region(DB_MPOOL *dbmp)
	// DB_MPOOL *dbmp;
{
	DB_MPOOL_HASH *hp;
	ENV *env;
...
static int
__memp_map_regions(DB_MPOOL *dbmp)
	// DB_MPOOL *dbmp;
{
	ENV *env;
	MPOOL *mp;
 ...
 */
int
__memp_resize(DB_MPOOL *dbmp, u_int32_t gbytes, u_int32_t bytes)
	// DB_MPOOL *dbmp;
 ...
 */
int
__memp_get_cache_max(DB_ENV *dbenv, u_int32_t *max_gbytesp, u_int32_t *max_bytesp)
	// DB_ENV *dbenv;
 ...
 */
int
__memp_set_cache_max(DB_ENV *dbenv, u_int32_t max_gbytes, u_int32_t max_bytes)
	// DB_ENV *dbenv;
