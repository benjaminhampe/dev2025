 ...
 */
int
__memp_fput_pp(DB_MPOOLFILE *dbmfp, void *pgaddr, DB_CACHE_PRIORITY priority, u_int32_t flags)
	// DB_MPOOLFILE *dbmfp;
	// void *pgaddr;
 ...
 */
int
__memp_fput(DB_MPOOLFILE *dbmfp, DB_THREAD_INFO *ip, void *pgaddr, DB_CACHE_PRIORITY priority)
	// DB_MPOOLFILE *dbmfp;
	// DB_THREAD_INFO *ip;
	// void *pgaddr;
	// DB_CACHE_PRIORITY priority;
...
unpin:
	 ...
	 */
	DB_ASSERT(env, atomic_read(&bhp->ref) != 0);
	if (atomic_dec(env, &bhp->ref) > 1 || (db_atomic_read(&bhp->ref) == 1 &&
 ...
 */
static int
__memp_reset_lru(ENV *env, REGINFO *infop)
	// ENV *env;
	// REGINFO *infop;
{
	BH *bhp, *tbhp;
 ...
 */
int
__memp_unpin_buffers(ENV *env, DB_THREAD_INFO *ip)
	// ENV *env;
	// DB_THREAD_INFO *ip;
{
	BH *bhp;
