 ...
 */
int
__memp_stat_pp(DB_ENV *dbenv, DB_MPOOL_STAT **gspp, DB_MPOOL_FSTAT ***fspp, u_int32_t flags)
	// DB_ENV *dbenv;
	// DB_MPOOL_STAT **gspp;
	// DB_MPOOL_FSTAT ***fspp;
	// u_int32_t flags;
 ...
 */
static int
__memp_stat(ENV *env, DB_MPOOL_STAT **gspp, DB_MPOOL_FSTAT ***fspp, u_int32_t flags)
	// ENV *env;
	// DB_MPOOL_STAT **gspp;
	// DB_MPOOL_FSTAT ***fspp;
	// u_int32_t flags;
...
static int
__memp_file_stats(ENV *env, MPOOLFILE *mfp, void *argp, u_int32_t *countp, u_int32_t flags)
	// ENV *env;
	// MPOOLFILE *mfp;
	// void *argp;
	// u_int32_t *countp;
...
static int
__memp_count_files(ENV *env, MPOOLFILE *mfp, void *argp, u_int32_t *countp, u_int32_t flags)
	// ENV *env;
	// MPOOLFILE *mfp;
	// void *argp;
	// u_int32_t *countp;
 ...
 */
static int
__memp_get_files(ENV *env, MPOOLFILE *mfp, void *argp, u_int32_t *countp, u_int32_t flags)
	// ENV *env;
	// MPOOLFILE *mfp;
	// void *argp;
	// u_int32_t *countp;
 ...
 */
int
__memp_stat_print_pp(DB_ENV *dbenv, u_int32_t flags)
	// DB_ENV *dbenv;
	// u_int32_t flags;
{
	DB_THREAD_INFO *ip;
 ...
 */
int
__memp_stat_print(ENV *env, u_int32_t flags)
	// ENV *env;
	// u_int32_t flags;
{
 ...
 */
static int
__memp_print_stats(ENV *env, u_int32_t flags)
	// ENV *env;
	// u_int32_t flags;
{
 ...
 */
static int
__memp_print_all(ENV *env, u_int32_t flags)
	// ENV *env;
	// u_int32_t flags;
{
	static const FN cfn[] = {
...
static int
__memp_print_files(ENV *env, MPOOLFILE *mfp, void *argp, u_int32_t *countp, u_int32_t flags)
	// ENV *env;
	// MPOOLFILE *mfp;
	// void *argp;
	// u_int32_t *countp;
 ...
 */
static int
__memp_print_hash(ENV *env, DB_MPOOL *dbmp, REGINFO *reginfo, roff_t *fmap, u_int32_t flags)
	// ENV *env;
	// DB_MPOOL *dbmp;
	// REGINFO *reginfo;
	// roff_t *fmap;
	...
	for (hp = R_ADDR(reginfo, c_mp->htab),
		...
		if ((bhp = SH_TAILQ_FIRST(&hp->hash_bucket, __bh)) != NULL) {
			__db_msgadd(env, &mb,
				...
				(u_long)db_atomic_read(&hp->hash_page_dirty));
			if (hp->hash_frozen != 0)
 ...
 */
static void
__memp_print_bh(ENV *env, DB_MPOOL *dbmp, const char *prefix, BH *bhp, roff_t *fmap)
	// ENV *env;
	// DB_MPOOL *dbmp;
	// const char *prefix;
	// BH *bhp;
	// roff_t *fmap;
{
	...
	__db_msgadd(env, &mb, "%2lu, %lu/%lu", (u_long)db_atomic_read(&bhp->ref),
 ...
 */
static void
__memp_stat_wait(ENV *env, REGINFO *reginfo, MPOOL *mp, DB_MPOOL_STAT *mstat, u_int32_t flags)
	// ENV *env;
	// REGINFO *reginfo;
	// MPOOL *mp;
	// DB_MPOOL_STAT *mstat;
 ...
 */
void
__memp_stat_hash(REGINFO *reginfo, MPOOL *mp, u_int32_t *dirtyp)
	// REGINFO *reginfo;
	// MPOOL *mp;
	// u_int32_t *dirtyp;
{
	...
	for (i = 0, dirty = 0; i < mp->htab_buckets; i++, hp++)
		dirty += (u_int32_t)db_atomic_read(&hp->hash_page_dirty);
	*dirtyp = dirty;
}
