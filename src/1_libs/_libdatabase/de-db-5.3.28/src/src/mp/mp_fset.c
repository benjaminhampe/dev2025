 ...
 */
int
__memp_dirty(DB_MPOOLFILE *dbmfp, void *addrp, DB_THREAD_INFO *ip, DB_TXN *txn, DB_CACHE_PRIORITY priority, u_int32_t flags)
	// DB_MPOOLFILE *dbmfp;
	// void *addrp;
	// DB_THREAD_INFO *ip;
	// DB_TXN *txn;
	...
	// u_int32_t flags;
{
	...
	env = dbmfp->env;
	dbmp = env->mp_handle;
	mvcc = db_atomic_read(&dbmfp->mfp->multiversion);
 ...
 */
int
__memp_shared(DB_MPOOLFILE *dbmfp, void *pgaddr)
	// DB_MPOOLFILE *dbmfp;
	// void *pgaddr;
{
	BH *bhp;
