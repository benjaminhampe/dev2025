...
// Precompute sine and cosine LUTs
void generate_twiddle_luts(std::vector<float>& sin_lut, std::vector<float>& cos_lut, int size) {
    sin_lut.resize(size);
    cos_lut.resize(size);
...
// In-place radix-2 FFT using AVX2 and LUTs
void fft_avx2_lut(std::vector<complexf>& data, const std::vector<float>& sin_lut, const std::vector<float>& cos_lut) {
...
void ifft_avx2_fma(std::vector<complexf>& data, const std::vector<float>& sin_lut, const std::vector<float>& cos_lut) {
    int N = static_cast<int>(data.size());
    int logN = static_cast<int>(std::log2(N));
    ...
    for (int s = 1; s <= logN; ++s) {
        ...
        for (int k = 0; k < N; k += m) {
            for (int j = 0; j < m2; j += 8) {
                ...
                // FMA3 complex multiply: t = B * W
                __m256 t_re = _mm256_fmsub_ps(b_re, tw_re, _mm256_mul_ps(b_im, tw_im));
                __m256 t_im = _mm256_fmadd_ps(b_re, tw_im, _mm256_mul_ps(b_im, tw_re));
