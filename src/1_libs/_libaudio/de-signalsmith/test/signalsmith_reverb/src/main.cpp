...
public:
    ExampleRunner(std::string inputFile, std::string outputPrefix)
        : inputWav(inputFile), outputPrefix(outputPrefix)
    {
        if (!inputWav.result) {
...
int main(int argc, const char **argv)
{
    ...
    if (argc < 3)
    {
        std::cout << "Missing 2 args: <input.wav> <output-prefix->\n";
        // src = "media/signalsmith/piano-space.wav";
        // dst = "signalsmith_reverb_piano_space_";
        src = "media/signalsmith/sines.wav";
        dst = "signalsmith_reverb_sines_";
    }
    else
    {
        src = argv[1];
    ...
    std::cout << "Using argv[1] = " << src << "\n";
    std::cout << "Using argv[2] = " << dst << "\n";
    std::cout << std::flush;

    ExampleRunner runner(src,dst);
    runner.runExamples();
}
