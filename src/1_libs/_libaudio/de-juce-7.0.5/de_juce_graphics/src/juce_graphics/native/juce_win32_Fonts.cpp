...
namespace FontEnumerators
{
    ...
    static int CALLBACK fontEnum1 (ENUMLOGFONTEXW* lpelfe, NEWTEXTMETRICEXW*, int type, LPARAM lParam)
    {
        if (lpelfe != nullptr && (type & RASTER_FONTTYPE) == 0)
        {
            ...
            EnumFontFamiliesExW(dc, &lf, (FONTENUMPROCW) &fontEnum2, lParam, 0);
            DeleteDC (dc);
        }
...
public:
    ...
    float getStringWidth (const String& text)
    {
        ...
        if (GetGlyphIndicesW(dc, utf16, (int) numChars, reinterpret_cast<WORD*> (results.getData()),
...
private:
    ...
    void loadFont()
    {
        ...
        auto standardSizedFont = CreateFontIndirectW(&lf);
