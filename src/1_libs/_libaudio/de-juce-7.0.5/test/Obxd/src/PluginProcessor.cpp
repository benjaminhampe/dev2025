   ...
   , programs()
{
   ...
   jassert(this->getBus(false,0));
   jassert(this->getBus(false,0)->getNumberOfChannels() >= 2);
}
...
*/
   ...
   while (samplePos < numSamples)
   {
      ...
      synth.processSample(L+samplePos,R+samplePos);
...
{
   #if TODO_FIX
   auto e = getXmlFromBinary(data,sizeInBytes);
