...
/* internal */
struct JUCE_API FFTInstance
{
    virtual ~FFTInstance() = default;
...
struct FFTEngine
{
    FFTEngine (int priorityToUse) : enginePriority (priorityToUse)
    {
    ...
    virtual FFTInstance* create (int order) const = 0;

    static Array<FFTEngine*>& getEngines()
    {
        static Array<FFTEngine*> engines;
        return engines;
    }
...
//==============================================================================
//FFTInstance* createBestFFTEngineForPlatform(int order);

inline FFTInstance* createBestFFTEngineForPlatform(int order)
{
    for (auto* engine : FFTEngine::getEngines())
...
// //==============================================================================
// #ifndef DOXYGEN
// template <typename> struct FFTEngineImpl;
// #endif
...
public:
    ...
    */
    FFT (int order)
        : engine(createBestFFTEngineForPlatform(order))
        , size(1 << order)
    {
    }
...
private:
    //==============================================================================
    std::unique_ptr<FFTInstance> engine;
    int size;
