...
struct FFTEngineImpl  : public FFTEngine
{
    FFTEngineImpl() : FFTEngine (InstanceToUse::priority)
    {}
    FFTInstance* create (int order) const override
...
/*
void FFT::performRealOnlyForwardTransform (float* inputOutputData,
                                     bool onlyCalculateNonNegativeFrequencies) const noexcept
{
    if (engine != nullptr)
        engine->performRealOnlyForwardTransform (inputOutputData, onlyCalculateNonNegativeFrequencies);
}
*/
...
//==============================================================================
//==============================================================================
struct FFTFallback : public FFTInstance
{
...
FFTEngineImpl<FFTFallback> fftFallback;

//==============================================================================
//==============================================================================
#if (JUCE_MAC || JUCE_IOS) && JUCE_USE_VDSP_FRAMEWORK
struct AppleFFT  : public FFTInstance
{
...
FFTEngineImpl<AppleFFT> appleFFT;
#endif
...
#endif
*/
struct FFTWImpl  : public FFTInstance
{
   #if JUCE_DSP_USE_STATIC_FFTW
...
*/
    ...
    static FFTWImpl* create (int order)
    {
        return new FFTWImpl (static_cast<size_t> (order));
    }

    FFTWImpl (size_t orderToUse)
    /*
        : fftwLibrary (std::move (libraryToUse))
        , fftw (symbols)
*/
        : order (static_cast<size_t> (orderToUse))
    {
        ScopedLock lock (getFFTWPlanLock());

        auto n = (1 << order);
        HeapBlock<Complex<float>> in(n);
        HeapBlock<Complex<float>> out(n);

        c2cForward = fftwf_plan_dft_1d(n, reinterpret_cast<fftwf_complex*>(in.getData()),
                                         reinterpret_cast<fftwf_complex*>(out.getData()),
                                         -1, unaligned | estimate);

        c2cInverse = fftwf_plan_dft_1d(n, reinterpret_cast<fftwf_complex*>(in.getData()),
                                         reinterpret_cast<fftwf_complex*>(out.getData()),
                                         +1, unaligned | estimate);

        r2c = fftwf_plan_dft_r2c_1d(n, reinterpret_cast<float*>(in.getData()),
                                       reinterpret_cast<fftwf_complex*>(in.getData()),
                                       unaligned | estimate);
        c2r = fftwf_plan_dft_c2r_1d(n, reinterpret_cast<fftwf_complex*>(in.getData()),
                                       reinterpret_cast<float*>(in.getData()),
                                       unaligned | estimate);
    }
    ...
    ~FFTWImpl() override
    {
        ...
        fftwf_destroy_plan(c2cForward);
        fftwf_destroy_plan(c2cInverse);
    ...
    void perform (const Complex<float>* input, Complex<float>* output, bool inverse) const noexcept override
    {
        ...
        {
            auto n = (1u << order);
            fftwf_execute_dft(c2cInverse,
                              const_cast<fftwf_complex*>(reinterpret_cast<fftwf_complex const*>(input)),
                              reinterpret_cast<fftwf_complex*>(output));
            FloatVectorOperations::multiply ((float*) output, 1.0f / static_cast<float> (n), (int) n << 1);
        }
        else
        {
            fftwf_execute_dft(c2cForward,
                              const_cast<fftwf_complex*>(reinterpret_cast<fftwf_complex const*>(input)),
                              reinterpret_cast<fftwf_complex*>(output));
        }
    }
    ...
    void performRealOnlyForwardTransform (float* inputOutputData, bool ignoreNegativeFreqs) const noexcept override
    {
        ...
        fftwf_execute_dft_r2c(r2c, inputOutputData, reinterpret_cast<fftwf_complex*>(out));
    ...
    void performRealOnlyInverseTransform (float* inputOutputData) const noexcept override
    {
        ...
        fftwf_execute_dft_c2r(c2r, reinterpret_cast<fftwf_complex*>(inputOutputData), inputOutputData);
    ...
    fftwf_plan c2cForward, c2cInverse, r2c, c2r;
};

FFTEngineImpl<FFTWImpl> fftwEngine;
#endif

//==============================================================================
//==============================================================================
#if JUCE_DSP_USE_INTEL_MKL
struct IntelFFT  : public FFTInstance
{
...
FFTEngineImpl<IntelFFT> fftwEngine;
#endif
...
class IntelPerformancePrimitivesFFT : public FFTInstance
{
public:
...
FFTEngineImpl<IntelPerformancePrimitivesFFT> intelPerformancePrimitivesFFT;
#endif
