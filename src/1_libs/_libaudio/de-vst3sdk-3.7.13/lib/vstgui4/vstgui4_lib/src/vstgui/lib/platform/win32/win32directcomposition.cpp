...
struct SurfaceRedrawArea : IPlatformTimerCallback
{
...
#ifdef BENNI_MINGW_WITH_IDCOMPOSITIONVISUAL3
	void restart ()
	{
    ...
    void restart ()
    {
        COM::Ptr<IDCompositionVisual2> vis3;
        auto hr = surface->visual->QueryInterface (__uuidof(IDCompositionVisual2),
                                                   reinterpret_cast<void**> (vis3.adoptPtr ()));
        if (SUCCEEDED (hr) && vis3 && animation)
        {
            animation->Reset ();
...
//------------------------------------------------------------------------
ID2D1Device* Factory::getDevice () const
{
