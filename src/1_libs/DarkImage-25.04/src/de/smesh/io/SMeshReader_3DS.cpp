...
struct SMeshLoad3DSFile
{
   ...
   bool
   load( gpu::VideoDriver* driver, SMesh & out, Binary & file, LoadOptions const & opt )
   {
      Mesh = &out;
      m_Driver = driver;
      ...
      if ( ok )
      {
         ...
         if ( opt.m_logLevel > 0 )
         {
            ...
            {
               DE_DEBUG("Limb[",i,"] ", Mesh->getMeshBuffer(i).str() )
            }
         }
      }
...
SMeshLoad3DSFile::readMaterialChunk( de::Binary & file, ChunkData* parent)
{
   ...
   while( parent->read < parent->header.length )
   {
      ...
      case C3DS_WIRE:
         ...
         break;
      case C3DS_TWO_SIDE:
         CurrentMaterial.Material.state.culling = gpu::Culling::disabled();
         break;
      case C3DS_SHADING:
         {
            int16_t flags;
   ...
   CurrentMaterial.Material.getTexture( 0 ).transform = glm::mat2( tx, ty, sx, sy );
   CurrentMaterial.Material.getTexture( 1 ).transform = glm::mat2( tx2, ty2, sx2, sy2 );
...
void SMeshLoad3DSFile::loadMaterials( de::Binary & file )
{
   ...
   for (uint32_t i=0; i<Materials.size(); ++i)
   {
      ...
      if ( matData.Filename[0].size() )
      {
         ...
         std::string uri = de::FileSystem::fileDir(modelUri) + "/" + matData.Filename[0];
         //DE_DEBUG("TexUri = ", uri )
         if ( !dbExistFile(uri) )
         {
            dbStrLowerCase( uri );
         ...
         if ( uri.size() )
         {
            gpu::TexRef ref = m_Driver->getTexture( uri );
            if (!ref.tex)
            {
      ...
      if (matData.Filename[1].size())
      {
         ...
         std::string uri = de::FileSystem::fileDir(modelUri) + "/" + matData.Filename[1];
      ...
      if (matData.Filename[2].size())
      {
         ...
         std::string uri = de::FileSystem::fileDir(modelUri) + "/" + matData.Filename[2];
      ...
      if (matData.Filename[3].size())
      {
         ...
         std::string uri = de::FileSystem::fileDir(modelUri) + "/" + matData.Filename[3];
      ...
      if (matData.Filename[4].size())
      {
         ...
         std::string uri = de::FileSystem::fileDir(modelUri) + "/" + matData.Filename[4];
...
bool
SMeshReader3DS::load(gpu::VideoDriver* driver,
     SMesh & m, std::string const & uri, const uint8_t* p, size_t n,
     LoadOptions const & opt )
{
    SMeshLoad3DSFile meshFile;
    de::Binary srcFile(p,n,uri);
    return meshFile.load( driver, m, srcFile, opt );
}
