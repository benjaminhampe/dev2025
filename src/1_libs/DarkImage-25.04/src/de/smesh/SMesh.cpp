...
void SMaterial::setTexture( int stage, gpu::TexRef const & ref )
{
...
SMaterial::writeXML( tinyxml2::XMLDocument & doc, tinyxml2::XMLElement* matNode ) const
{
   ...
   blendNode->SetAttribute("eq", gpu::Blend::getEquationString(state.blend.equation).c_str() );
   blendNode->SetAttribute("src_rgb", gpu::Blend::getFunctionString(state.blend.src_rgb).c_str() );
   blendNode->SetAttribute("src_a", gpu::Blend::getFunctionString(state.blend.src_a).c_str() );
   blendNode->SetAttribute("dst_rgb", gpu::Blend::getFunctionString(state.blend.dst_rgb).c_str() );
   blendNode->SetAttribute("dst_a", gpu::Blend::getFunctionString(state.blend.dst_a).c_str() );
   ...
   depthNode->SetAttribute("fn", gpu::Depth::getString( state.depth.getFunc() ).c_str() );
   ...
   stencilNode->SetAttribute("sfail", gpu::Stencil::getActionString( state.stencil.sfail ).c_str() );
   stencilNode->SetAttribute("zfail", gpu::Stencil::getActionString( state.stencil.zfail ).c_str() );
   stencilNode->SetAttribute("zpass", gpu::Stencil::getActionString( state.stencil.zpass ).c_str() );
   stencilNode->SetAttribute("func", gpu::Stencil::getFuncString( state.stencil.testFunc ).c_str() );
...
SMeshBuffer::str( bool withMaterial ) const
{
    ...
    o << "p:" << gpu::PrimitiveType::getShortString( primitiveType ) << "|";
    o << "v:" << getVertexCount() << "|";
    o << "i:" << getIndexCount() << "|";
...
// static
uint32_t
SMeshBufferTool::computeEdgeCount( const SMeshBuffer& m)
{
    return 0;
}
...
// static
uint32_t
SMeshBufferTool::computeFaceCount( const SMeshBuffer& m)
{
    if (m.primitiveType == gpu::PrimitiveType::Triangles)
    {
        if (m.indices.empty())
        {
            return m.vertices.size() / 3;
        }
        else
        {
...
{
    if ( mesh.getPrimitiveType() != gpu::PrimitiveType::Triangles )
    {
...
{
    if ( b.getPrimitiveType() != gpu::PrimitiveType::Triangles ) return {};
...
{
    if ( o.getPrimitiveType() != gpu::PrimitiveType::Triangles )
    {
...
{
    if ( o.primitiveType == gpu::PrimitiveType::Triangles )
    {
        if ( o.indices.size() > 0 )
        {
            for ( size_t i = 0; i < o.indices.size()/3; ++i )
...
{
    if ( src.getPrimitiveType() == gpu::PrimitiveType::Triangles )
    {
...
{
    SMeshBuffer o( gpu::PrimitiveType::Lines );
...
gpu::TexRefs SMesh::getUniqueTextures() const
{
   std::vector< gpu::TexRef > textures;

   for (const auto& buffer : buffers)
   {
      for (gpu::TexRef const & ref : buffer.getMaterial().getTextures() )
      {
         if (!ref.tex) continue;

         auto found = std::find_if( textures.begin(), textures.end(),
            [&]( gpu::TexRef const & cached ) { return cached.tex == ref.tex; } );
...
{
   std::map< gpu::PrimitiveType, uint32_t > primMap;
   ...
   {
      if ( k > 0 ) o << "+";
      o << gpu::PrimitiveType::getString( it->first );
      k++;
      it++;
   }
...
std::string
SMesh::str( bool printDetails ) const
{
...
bool
SMesh::isBufferIndex( int i ) const
{
...
void
SMesh::setCulling( gpu::Culling cull )
{
...
void
SMesh::setDepth( gpu::Depth depth )
{
   for ( SMeshBuffer & p : buffers ) { p.setDepth( depth ); }
}

void
SMesh::setStencil( gpu::Stencil stencil )
{
   for ( SMeshBuffer & p : buffers ) { p.setStencil( stencil ); }
}

void
SMesh::setBlend( gpu::Blend blend )
{
...
void
SMesh::setTexture( int stage, gpu::TexRef ref )
{
...
SMeshTool::removeDoubleMaterials( SMesh & mesh )
{
    ...
    std::vector< gpu::TexRef > deball;
    std::vector< gpu::TexRef > sorted;
    for ( auto const & buffer : mesh.buffers )
    {
        std::vector< gpu::TexRef > unsorted = buffer.getMaterial().getTextures();

        for ( auto const & ref : unsorted )
        {
            auto it = std::find_if( sorted.begin(), sorted.end(),
                                   [&]( gpu::TexRef const & cached )
                                   {
...
SMeshTool::removeEmptyMeshBuffer( SMesh & mesh )
{
    ...
    auto shouldPersist = []( SMeshBuffer const & buf )
    {
        ...
        if (buf.vertices.size() < 2 && buf.primitiveType == gpu::PrimitiveType::Lines) return false;
        if (buf.vertices.size() < 3 && buf.primitiveType == gpu::PrimitiveType::Triangles) return false;
        return true;
    };
