...
void SMaterialRenderer::draw3D( const SMeshBuffer& m )
{
    setMaterial( m.material );
    m.draw();
}

void SMaterialRenderer::draw3D( const SMesh& mesh )
{
...
SMaterialRenderer::createShaderName( SMaterial const & material ) const
{
    std::ostringstream o;
    o << "s3d";
    //if ( material.hasAmbientMap() ) o << "+Ta";
    if ( material.CloudTransparent ) o << "+ct";
    if ( material.hasDiffuseMap() ) o << "+Td";
    if ( material.hasBumpMap() ) o << "+Tb";
    if ( material.hasNormalMap() ) o << "+Tn";
    //if ( material.Lighting > 0 ) o << "+illum" << material.Lighting;
    o << "+illum" << material.Lighting;
    if ( material.FogEnable ) o << "+fog";
    return o.str();
}
...
SMaterialRenderer::getShader( SMaterial const & material )
{
    ...
    if ( !m_driver ) { DE_ERROR("No driver.") return shader; }

    //uint32_t id = createShaderId( material );
    const std::string shaderName = createShaderName( material );
...
SMaterialRenderer::getShader( SMaterial const & material )
{
    // if ( !m_Driver ) { DE_ERROR("No driver.") return 0; }
    ...
    auto it = m_ShaderLut.find( shaderName );
    if ( it != m_ShaderLut.end() )
    {
        shaderId = (*it).second;
    }
    else
    {
        // Create by expensive name
        //auto name = createShaderName( material );
        //shaderId = m_Driver->getShader( name );
        //if ( !shader )
        //{
            shaderId = GT_createShader( shaderName.c_str(),
                                        createVS( material ).c_str(),
                                        createFS( material ).c_str() );
        //}
        ...
        m_ShaderLut[ shaderName ] = shaderId; // Store new and valid shader
    }

    if ( !shaderId )
    {
        DE_ERROR("No shader with name ", shaderName, ", id = ", shaderId)
    }
...
SMaterialRenderer::getShader( SMaterial const & material )
{
    ...
    //uint32_t id = createShaderId( material );
    const std::string name = createShaderName( material );
    ...
    else
    {
        ...
        m_shaders[ name ] = shader; // Store new and valid shader
    }

    if ( !shader )
    {
        DE_ERROR("No shader ",name);
        return nullptr;
    }
...
bool
SMaterialRenderer::setMaterial( const SMaterial& mtl )
{
    if ( !m_driver ) { DE_ERROR("No driver.") return false; }

    // DE_BENNI(mtl.toString())

    // [UseProgram]
    gpu::Shader* shader = getShader( mtl );
    if ( !shader )
    {
        DE_ERROR("No shader in use for material(",mtl.toString(),")")
        return false;
    }

    // [MVP] Common to all shaders
    glm::dmat4 u_viewProjMat = glm::dmat4( 1.0 );
    glm::vec3 u_cameraPos = glm::vec3( 0.0f );

    auto camera = m_driver->getCamera();
    if (camera)
    {
        camera->update();
        u_viewProjMat = camera->getViewProjectionMatrix();
        u_cameraPos = camera->getPos();
    }

    shader->setMat4f( "u_projViewMat", u_viewProjMat );
    shader->setMat4f( "u_modelMat", m_driver->getModelMatrix() );
    //shader->setVec3f( "u_cameraPos", u_cameraPos );
    ...
    if ( mtl.Lighting > 0 )
    {
        ...
        if ( mtl.Lighting == 3 )
        {
            for ( size_t i = 0; i < m_Lights.size(); ++i )
            {
                ...
                glm::vec4 clipRaw = glm::vec4( u_viewProjMat * glm::dvec4( glm::dvec3( m_Lights[i].pos ), 1.0 ) );
    ...
    // [DiffuseMap] <RGB> color + <A> opacity
    const gpu::TexRef& diffuseMap = mtl.getDiffuseMap();
    if (diffuseMap.tex)
    {
        m_driver->useTexture( diffuseMap.tex, texStage );
        shader->setInt( "u_diffuseMap", texStage );
        shader->setVec4f( "u_diffuseMapTransform", diffuseMap.coords );
        shader->setVec2f( "u_diffuseMapRepeat", diffuseMap.repeat );
        texStage++;
    }

    // [TexBump] <RGB> normalMap + <A> bumpMap
    const gpu::TexRef& bumpMap = mtl.getBumpMap();
    if (bumpMap.tex)
    {
        m_driver->useTexture( bumpMap.tex, texStage );
        shader->setInt( "u_bumpMap", texStage );
        shader->setVec1f( "u_bumpMapScale", mtl.BumpScale );
        shader->setVec4f( "u_bumpMapTransform", bumpMap.coords );
        shader->setVec2f( "u_bumpMapRepeat", bumpMap.repeat );
    }

    // [State] Cull, Depth, Stencil, Blend, etc...
    gpu::State state = mtl.state;
    if ( mtl.CloudTransparent )
    {
        state.blend = gpu::Blend::alphaBlend();
    }
...
void
SMaterialRenderer::unsetMaterial( const SMaterial& mtl )
{
    (void)mtl;
...
SMaterialRenderer::setMaterial( SMaterial const & material )
{
   ...
   // [UseProgram]
   const uint32_t shaderId = getShader( material );
   if ( !shaderId )
   {
...
SMaterialRenderer::createVS( SMaterial const & material ) const
{
    ...
    if ( material.hasBumpMap() )
    {
        ...
        uniform float u_bumpMapScale;
        uniform vec4 u_bumpMapTransform;
        uniform vec2 u_bumpMapRepeat;
        )";
    }
    ...
    {
        vs << R"(
        vec2 bumpMapCoords = u_texBumpTransform.zw * fract( u_bumpMapRepeat * a_tex ) + u_texBumpTransform.xy;
        float h = texture( u_bumpMap, scaleTb * bumpMapCoords ).a;
        vec3 pos = a_pos + (a_normal * h * u_bumpMapScale);
        )";
    }
    else
    {
    ...
    vs << R"(
        gl_Position = u_projViewMat * u_modelMat * vec4( pos, 1.0 );
    )";
...
SMaterialRenderer::createFS( SMaterial const & material ) const
{
   ...
   if ( material.hasDiffuseMap() )
   {
      fs << R"(
        ...
        uniform vec4 u_diffuseMapTransform;
        uniform vec2 u_diffuseMapRepeat;
      )";
   }
...
//   }
   ...
   if ( material.hasDiffuseMap() )
   {
      fs << R"(
         ...
         vec2 diffuseMapCoords = u_diffuseMapTransform.zw * fract( u_diffuseMapRepeat * v_tex ) + u_diffuseMapTransform.xy;
         vec4 diffuseMapColor = texture( u_diffuseMap, diffuseMapCoords );
      )";
      if ( material.CloudTransparent )
      {
         fs << R"(
         float r = diffuseMapColor.r;
         diffuseColor *= vec4(r, r, r, r);
         )";
      }
      else
      {
   ...
   if ( material.hasDiffuseMap()
       //&& ( material.CloudTransparent || material.getDiffuseMap().getTexture()->hasTransparency() )
       )
   {
      fs << R"(
