...
// ===========================================================================
{
   ...
   bool GouraudShading;
   gpu::TexRef DiffuseMap;
   gpu::TexRef BumpMap;
   float BumpScale;
   gpu::TexRef NormalMap;
   float NormalScale;
   gpu::TexRef SpecularMap;
   gpu::TexRef DetailMap;
   std::string Name;
   ...
   gpu::TexRef const & getTexture( int stage ) const;
   gpu::TexRef & getTexture( int stage );
   void setTexture( int stage, gpu::TexRef const & ref );

   std::vector< gpu::TexRef > getTextures() const
   {
      std::vector< gpu::TexRef > textures;
   ...
   void setCulling( gpu::Culling const & cull ) { state.culling = cull; }
   void setDepth( gpu::Depth const & depth ) { state.depth = depth; }
   void setStencil( gpu::Stencil const & stencil ) { state.stencil = stencil; }
   void setBlend( gpu::Blend const & blend ) { state.blend = blend; }
   void setBumpScale( float scale ) { BumpScale = scale; }
   void setDiffuseMap( gpu::TexRef ref ) { DiffuseMap = ref; }
   void setBumpMap( gpu::TexRef ref ) { BumpMap = ref; }
   void setNormalMap( gpu::TexRef ref ) { NormalMap = ref; }
   void setSpecularMap( gpu::TexRef ref ) { SpecularMap = ref; }
/*
...
*/
   ...
   gpu::TexRef const & getDiffuseMap() const { return DiffuseMap; }
   gpu::TexRef const & getBumpMap() const { return BumpMap; }
   gpu::TexRef const & getNormalMap() const { return NormalMap; }
   gpu::TexRef const & getSpecularMap() const { return SpecularMap; }
   gpu::TexRef const & getDetailMap() const { return DetailMap; }
   gpu::TexRef & getDiffuseMap() { return DiffuseMap; }
   gpu::TexRef & getBumpMap() { return BumpMap; }
   gpu::TexRef & getNormalMap() { return NormalMap; }
   gpu::TexRef & getSpecularMap() { return SpecularMap; }
   gpu::TexRef & getDetailMap() { return DetailMap; }
//   bool
...
// =======================================================================
{
    ...
    gpu::PrimitiveType primitiveType;   /// @brief GLenum PrimitiveType;
    ...
    typedef std::vector< TVertex > TVertices;   /// @brief Vertices
    TVertices vertices;   /// @brief Vertices
    typedef std::vector< uint32_t > TIndices;   /// @brief Indices
    TIndices indices;   /// @brief Indices
    std::string name;   /// @brief Able to find specific MeshBuffer in Mesh by (category/material) name. ( e.g. "Lines", "Triangles" )

    /// @brief Constructor
    explicit SMeshBuffer( gpu::PrimitiveType primType = gpu::PrimitiveType::Triangles )
    ...
    TVertices const &   getVertexVector() const { return vertices; }
    TVertices &         getVertexVector()        { return vertices; }
    S3DVertex const &   getVertex( size_t i ) const { return vertices[ i ]; }
    S3DVertex &         getVertex( size_t i )       { return vertices[ i ]; }
    TIndices const &    getIndexVector() const  { return indices; }
    TIndices &          getIndexVector()        { return indices; }
    ...
    size_t              computeByteConsumption() const;   // [MemorySizeInBytes]
    ...
    void                setCulling( gpu::Culling cull ) { getMaterial().setCulling( cull ); }
    void                setDepth( gpu::Depth depth ) { getMaterial().setDepth( depth ); }
    void                setStencil( gpu::Stencil blend ) { getMaterial().setStencil( blend ); }
    void                setBlend( gpu::Blend blend ) { getMaterial().setBlend( blend ); }
    ...
    void                setTexture( int stage, gpu::Texture* tex ) { getMaterial().setTexture( stage, gpu::TexRef( tex ) ); }
    void                setTexture( int stage, gpu::TexRef const & ref ) { getMaterial().setTexture( stage, ref ); }
    ...
    gpu::PrimitiveType  getPrimitiveType() const { return primitiveType; }
    void                setPrimitiveType( gpu::PrimitiveType type ) { primitiveType = type; }
   ...
   glm::vec3 const &    getPos( uint32_t i ) const { return vertices[ i ].pos; }
    ...
    void addLineTriangle( S3DVertex const & a, S3DVertex const & b, S3DVertex const & c )
    {
      auto v = getVertexCount();
      addVertex( a );
      addVertex( b );
    ...
    void addLineQuad( S3DVertex const & a, S3DVertex const & b, S3DVertex const & c, S3DVertex const & d )
    {
      auto v = getVertexCount();
      addVertex( a );
      addVertex( b );
    ...
    void addVertexQuad( S3DVertex const & a, S3DVertex const & b, S3DVertex const & c, S3DVertex const & d, bool flip = false )
    {
      if ( flip )
      {
         addVertex( a );
    ...
    void addIndexedTriangle( bool flip = false )
    {
   ...
   // Quad ABCD: (ccw) ACB + ADC, (cw) ABC + ACD
    void addIndexedQuad( uint32_t a, uint32_t b, uint32_t c, uint32_t d, bool flip = false )
    {
    ...
    void addIndexedQuad( bool flip = false )
    {
      uint32_t const v = getVertexCount();
      if ( v < 4 ) return;
    ...
    {
       if ( getPrimitiveType() != gpu::PrimitiveType::Triangles ) return {};
    ...
    {
       // [Triangles]
       if ( getPrimitiveType() == gpu::PrimitiveType::Triangles )
       {
           if ( getVertexCount() < 3 )
           {
               return; // no triangles
       ...
       // [TriangleStrip]
       else if ( getPrimitiveType() == gpu::PrimitiveType::TriangleStrip )
       {
           if ( getVertexCount() < 3 )
           {
       ...
       // [BenniQuads]
       else if ( getPrimitiveType() == gpu::PrimitiveType::Quads )
       {
           if ( getVertexCount() < 4 )
           {
               return; // no quads
...
// ===========================================================================
{
    static uint32_t computeEdgeCount( const SMeshBuffer& m);
    static uint32_t computeFaceCount( const SMeshBuffer& m);
    static bool onlyUniqueVertices( const SMeshBuffer& in, SMeshBuffer& out );
    static void transformPoints( std::vector< glm::vec3 > & points, glm::dmat4 const & modelMat );
    static void rotatePoints( std::vector< glm::vec3 > & points, float a, float b, float c );
    static void rotateTexCoords90_onlyQuads( SMeshBuffer & mesh );
    static SMeshBuffer tesselate( SMeshBuffer const & src );
    static void fuse( SMeshBuffer & dst, SMeshBuffer const & src, glm::vec3 const & offset = glm::vec3() );
    static void fuse( SMeshBuffer & dst, std::vector< SMeshBuffer > const & meshes );
    static uint32_t countVertices( std::vector< SMeshBuffer > const & liste );
    static uint32_t countIndices( std::vector< SMeshBuffer > const & liste );
    static std::vector< Triangle3f > createCollisionTriangles( SMeshBuffer const & b );
    static void computeBoundingBox( SMeshBuffer & o );
    static Box3f computeBoundingBox( std::vector< S3DVertex > const & vertices );
    static void computeNormals( SMeshBuffer & o );
    static void computeNormals( std::vector< S3DVertex > & vertices, std::vector< uint32_t > const & indices );
    static void bumpVertices( SMeshBuffer & o, Image* bumpMap, float bumpScale = 1.0f );
    static void flipX( SMeshBuffer & o );
    static void flipY( SMeshBuffer & o );
    static void flipZ( SMeshBuffer & o );
    static void flipXY( SMeshBuffer & o );
    static void flipYZ( SMeshBuffer & o );
    static void flipXZ( SMeshBuffer & o );
    static void flipNormals( SMeshBuffer & o );
    static void flipVertexPosYZ( SMeshBuffer & o );
    static void flipWinding( SMeshBuffer & o );
    static void translateVertices( SMeshBuffer & o, glm::vec3 const & offset );
    static void translateVertices( SMeshBuffer & o, glm::vec3 const & offset, uint32_t vStart );
    static void translateVertices( SMeshBuffer & o, float x, float y, float z );
    static void transformVertices( SMeshBuffer & o, glm::dmat4 const & modelMat = glm::dmat4(1.0) );
    static void randomRadialDisplace( SMeshBuffer & o, float h );
    static void rotateVertices( SMeshBuffer & o, float a, float b, float c );
    static void scaleVertices( SMeshBuffer & o, float scale = 1.01f );
    static void scaleVertices( SMeshBuffer & o, float sx, float sy, float sz );
    static void translateNormals( SMeshBuffer & o, glm::vec3 const & offset );
    static void translateNormals( SMeshBuffer & o, float nx, float ny, float nz );
    static void setNormals( SMeshBuffer & o, glm::vec3 const & normal );
    static void setNormals( SMeshBuffer & o, float nx, float ny, float nz );
    static void setNormalZ( SMeshBuffer & o, float nz );
    static void translateTexture( SMeshBuffer & o, float x, float y, bool bClamp = false );
    static void translateTexture( SMeshBuffer & o, const glm::vec2& offset, bool bClamp = false );
    static void translateTextureU( SMeshBuffer & o, const float offset, const bool bClamp = false );
    static void translateTextureV( SMeshBuffer & o, const float offset, const bool bClamp = false );
    static void scaleTexture( SMeshBuffer & o, float u, float v );
    static void scaleTexture( SMeshBuffer & o, const glm::vec2& scale );
    static void rotateTexture( SMeshBuffer & o, const glm::mat2& m );
    static void transformTexCoords( SMeshBuffer & o, Recti const & r_atlas, int atlas_w, int atlas_h );
    static void colorVertices( SMeshBuffer & o, uint32_t color );
    static void centerVertices( SMeshBuffer & o );
    static void addVertex( SMeshBuffer & o, S3DVertex const & vertex );
    static void addVertex( SMeshBuffer & o, glm::vec3 const & pos, glm::vec3 const & nrm, uint32_t color, glm::vec2 const & tex );
    static void addVertex( SMeshBuffer & o, glm::vec3 const & pos, uint32_t color );
    static void addLine( SMeshBuffer & o, S3DVertex const & a, S3DVertex const & b );
    static void addTriangle( SMeshBuffer & o, S3DVertex const & a, S3DVertex const & b, S3DVertex const & c );
    static void addLine( SMeshBuffer & o, glm::vec3 const & a, glm::vec3 const & b, uint32_t color = 0xFFFFFFFF );
    static void addLine( SMeshBuffer & o, glm::vec3 const & a, glm::vec3 const & b, uint32_t colorA, uint32_t colorB );
    static void addLine( SMeshBuffer & o, float x1, float y1, float x2, float y2, float z = 0.0f, uint32_t color = 0xFFFFFFFF );
    static void addLine( SMeshBuffer & o, float x1, float y1, float x2, float y2, float z, uint32_t colorA, uint32_t colorB );
    static void addLineTriangle( SMeshBuffer & o, glm::vec3 const & a, glm::vec3 const & b, glm::vec3 const & c, uint32_t color = 0xFFFFFFFF );
    static void addLineTriangle( SMeshBuffer & o, glm::vec3 const & a, glm::vec3 const & b, glm::vec3 const & c, uint32_t colorA, uint32_t colorB, uint32_t colorC );
    static void addLineQuad( SMeshBuffer & o, glm::vec3 const & a, glm::vec3 const & b, glm::vec3 const & c, glm::vec3 const & d, uint32_t color = 0xFFFFFFFF );
    static void addLineQuad( SMeshBuffer & o, glm::vec3 const & a, glm::vec3 const & b, glm::vec3 const & c, glm::vec3 const & d,
                uint32_t colorA, uint32_t colorB, uint32_t colorC = 0xFFFFFFFF, uint32_t colorD = 0xFFFFFFFF );
    static void addLineBox( SMeshBuffer & o, Box3f const & box, uint32_t color );
    // =======================================================================
    // DebugGeometry:
    // =======================================================================
    static void addLineNormals( SMeshBuffer & o, SMeshBuffer const & src, float n_len, uint32_t color );
    static void addWireframe( SMeshBuffer & o, SMeshBuffer const & src, float outScale, uint32_t color );
    static SMeshBuffer createWireframe( SMeshBuffer const & src, float scale = 1.0f, uint32_t color = 0 );
    static SMeshBuffer createScaled( SMeshBuffer const & src, float scale = 1.01f );
    static SMeshBuffer createTranslated( SMeshBuffer const & src, glm::vec3 pos );
    static SMeshBuffer createLineBox( Box3f const & box, uint32_t color );
    static SMeshBuffer createLineNormals( SMeshBuffer const & src, float normalLen, uint32_t color );
};
...
// ===========================================================================
{
    typedef std::vector< SMeshBuffer > SMeshBuffers;

    SMeshBuffers buffers;   //! Geometry buffers for this mesh
    //glm::dmat4 ModelMat;
...
public:
    ...
    static SharedPtr
    shared( std::string const & name = "" )
    {
       return std::make_shared< SMesh >();
    }

    static SharedPtr
    shared( SMeshBuffer const & buf )
    {
    ...
    std::string str( bool printDetails = false ) const;

    SMeshBuffers const & getMeshBuffers() const { return buffers; }
    SMeshBuffers & getMeshBuffers() { return buffers; }
    ...
    gpu::TexRefs getUniqueTextures() const;
    ...
    bool isBufferIndex( int i ) const;
    ...
    void setCulling( gpu::Culling cull );
    void setCulling( bool enable );
    void setDepth( gpu::Depth depth );
    void setStencil( gpu::Stencil stencil );
    void setBlend( gpu::Blend blend );
    //void setTexture( int stage, ITexture* tex );
    void setTexture( int stage, gpu::TexRef ref );
    ...
    // bool intersectRayWithBoundingBox( Ray3< T > const & ray, MeshIntersectionResult< T > & result,
    //                                glm::tmat4x4<T> const & trs ) const;
    bool intersectRayWithBoundingBox( Ray3< T > const & ray,
                                     MeshIntersectionResult< T > & result ) const;
    bool intersectRay( Ray3< T > const & ray,
                      MeshIntersectionResult< T > & result ) const;
    // bool intersectRay( Ray3< T > const & ray,
