...
namespace memory {
    ...
    template <typename T>
    using TAlignedVector = std::vector<T, TAlignedVectorAllocator<T, 64>>;

    typedef TAlignedVector<float>       AlignedFloatVector;
    typedef TAlignedVector<uint8_t>     AlignedByteVector;
    //typedef TVector<int16_t>     ShortVector;
    //typedef TVector<uint32_t>    ImageVector;
    ...
    struct TAlignedShiftVector
    {
        ...
        void shiftLeft( uint64_t shifts, bool debug = false )
        {
            ...
            for ( uint64_t i = shifts; i < m_write->size(); ++i )
            {
    ...
    typedef TAlignedShiftVector<float>       AlignedFloatShiftVector;
    typedef TAlignedShiftVector<uint8_t>     AlignedByteShiftVector;
    ...
    // =======================================================================
    {
        ...
        uint32_t m_colCount;         // Count matrix cols = m_shiftBuffer.size()
        uint32_t m_rowCount;        // Count matrix rows
        uint32_t m_pushCount;        // Debug collection of new rows of samples
        uint32_t m_dummy;        // Count m_shiftBuffer fill status to control shift tigger.
        ...
        void push( T const* src, uint32_t srcFrames )
        {
            ...
            float* dst = m_view.front();
            memcpy( dst, src, srcFrames * sizeof( float ));
        }

    };

    typedef TAlignedShiftMatrix<float>       AlignedFloatShiftMatrix;
    typedef TAlignedShiftMatrix<uint8_t>     AlignedByteShiftMatrix;

} // end namespace memory.
} // end namespace de.


typedef de::memory::AlignedFloatVector DE_AlignedFloatVector;
typedef de::memory::AlignedFloatShiftVector DE_AlignedFloatShiftVector;
typedef de::memory::AlignedFloatShiftMatrix DE_AlignedFloatShiftMatrix;
