...
// ===========================================================================
{
   ...
   void init( gpu::VideoDriver* driver ) { m_driver = driver; }
...
protected:
   ...
   gpu::Shader* getShader( SMaterial const & material ); // Could be public

public:
   gpu::VideoDriver* m_driver;
   //std::map< uint32_t, IShader* > m_ShaderLut; // get Shader by faster id, not a name.
   std::unordered_map< std::string, gpu::Shader* > m_shaders; // get Shader by faster id, not a name.
   float m_ScreenGamma;
