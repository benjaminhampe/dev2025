...
inline void setCol(glm::mat4& M, int col, const glm::vec3& v)
{
    M[col][0] = v.x;
    M[col][1] = v.y;
    M[col][2] = v.z;
...
glm::mat4 buildLookAtLH(const glm::vec3& pos,
                        const glm::vec3& target,
                        const glm::vec3& worldUp = glm::vec3(0,1,0))
{
    glm::vec3 forward = glm::normalize(target - pos);            // +Z in LH
    glm::vec3 right   = glm::normalize(glm::cross(worldUp, forward)); // LH: right = up × forward
    glm::vec3 up      = glm::cross(forward, right);              // recompute orthonormal up
