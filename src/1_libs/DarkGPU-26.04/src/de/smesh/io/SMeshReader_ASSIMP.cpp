...
bool
SMeshReaderASSIMP::load( gpu::VideoDriver* driver, SMesh & out, std::string const & uri,
                        const uint8_t* p, size_t n, LoadOptions const & options )
{
    AssimpFile file( uri );
    if ( !file.is_open() )
    {
    ...
    std::string meshDir = de::FileSystem::fileDir( uri );
   ...
   for ( uint32_t i = 0; i < nMeshBuffer; ++i )
   {
      ...
      // ============================
      SMeshBuffer limb( gpu::PrimitiveType::Triangles );
      limb.name = limbName;
      ...
      for ( uint32_t faceIndex = 0; faceIndex < faceCount; ++faceIndex )
      {
         ...
         {
            limb.setPrimitiveType( gpu::PrimitiveType::Points );
         ...
         {
            limb.setPrimitiveType( gpu::PrimitiveType::Lines );
            limb.getMaterial().setLighting( 0 );
            uint32_t indexA = face.mIndices[ 0 ];
         ...
         {
            limb.setPrimitiveType( gpu::PrimitiveType::Triangles );
            uint32_t indexA = face.mIndices[ 0 ];
            uint32_t indexB = face.mIndices[ 1 ];
...
//         }
         else // if ( face.mNumIndices == 4 )
         {
            ...
            limb.setPrimitiveType( gpu::PrimitiveType::TriangleStrip );
