#include <de/smesh/io/SMeshReader_ASSIMP_Tools.h>
...
AssimpTexture::AssimpTexture()
   : m_uri()
   , m_texType( aiTextureType_NONE ) // diffuse tex
...
//static
bool
AssimpTools::parseMaterial( aiScene const * const assimpScene,
                            uint32_t matIndex,
                            SMaterial & out,
                            ...
                            std::string baseDir )
{
   ...
   {
       DE_ERROR("Invalid index(",matIndex,") of materials(",matCount,")")
      return false;
   }
   ...
   if ( !assimpMaterial )
   {
       DE_ERROR("Invalid pointer to aiMaterial")
      return false;
   }
   ...
   {
      if ( ivalue < 1 )
         out.state.culling = gpu::Culling::enabled();
      else {
         out.state.culling = gpu::Culling::disabled();
      }
   }
                  ...
                  i < uint32_t( aiTextureType_UNKNOWN ); ++i )
   {
      ...
      for ( uint32_t k = 0; k < typeCount; ++k )
      {
         ...
         uri = de::FileSystem::fileName( uri );
         uri = baseDir + "/" + uri; // Make absolute ( relative to mesh )

         if ( !dbExistFile( uri ) )
         {
            DE_WARN( "Texture file not found, try *.jpg ( uri ) = ", uri )
            uri = de::FileSystem::fileDir( uri ) + "/" + de::FileSystem::fileBase( uri ) + ".jpg";
         }

         if ( !dbExistFile( uri ) )
         {
            DE_WARN( "Texture file not found, try *.png ( uri ) = ", uri )
            uri = de::FileSystem::fileDir( uri ) + "/" + de::FileSystem::fileBase( uri ) + ".png";
         }
         ...
         if ( !dbExistFile( uri ) )
         {
            ...
            uri = de::FileSystem::fileDir( uri ) + "/" + de::FileSystem::fileBase( uri ) + ".bmp";
         }
...
//               }
   ...
   if ( textures.size() > 0 && driver )
   {
      ...
      {
         std::string const uri = textures[ k ].m_uri;
         gpu::TexRef const ref = driver->getTexture( uri );
         if ( ref.tex )
         {
