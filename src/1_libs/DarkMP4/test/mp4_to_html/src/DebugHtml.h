...
// =======================================================================
struct DebugHtml : public de::file::mp4::IParserListener
// =======================================================================
{
    ...
    void
    onParserStart( uint8_t const* beg, uint8_t const* end, std::string const & uri ) override
    {
        m_fileName = uri;
        m_startAddress = beg;
        m_endAddress = end;
    ...
    void
    onParserEnd() override
    {
        DE_OK("Reached end of MP4 file")
    }
    ...
    void onAtom(uint64_t parent,
       ...
       const uint8_t* dataEnd ) override
    {
        auto fileOffset = uint64_t( atomBeg - m_startAddress );
        DebugHtmlToken token;
        token.m_address = fileOffset;
        token.m_fg = randomHtmlColor();
        token.m_bg = 0xFFFFFFFF;
        token.m_comment = utf8_atomName;

        token.m_parsed.assign( atomBeg, atomEnd );
    ...
    save( std::string saveUri ) const
    {
        ...
        {
            css += " background-color:rgb(255,150,150); ";
        }
        ...
        "<tr style=\"" << css << "\">"
        "<td><pre>" << i << "</pre></td>"
        "<td><pre>(0x" << dbHex( uint32_t(token.m_address) ) << ")</pre></td>"
        "<td><pre>" << dbHex( orig_beg, orig_end, 4 ) << "</pre></td>"
        "<td><pre>" << dbHex( pars_beg, pars_end, 4 ) << "</pre></td>"
        ...
        bool const ok = de::FileSystem::saveStr( saveUri, htmlStr );
        perf.stop();
