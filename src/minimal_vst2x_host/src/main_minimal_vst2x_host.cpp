...
public:
    ...
    size_t  getSampleRate()     const { return 48000; }
...
private:
    bool init(const wchar_t* vstModulePath, HWND hWndParent) {
        ...
        if(!vstEntryProc) {
            std::cout << "No VSTPluginMain!" << std::endl;
            vstEntryProc = reinterpret_cast<VstEntryProc*>(GetProcAddress(hModule, "main"));
        }
        else
        {
         std::cout << "Has VSTPluginMain." << std::endl;
        }
        ASSERT_THROW(vstEntryProc, "VST's entry point not found");

        aEffect = vstEntryProc(hostCallback_static);
        ASSERT_THROW(aEffect, "No VST plugin pointer");
        ASSERT_THROW(aEffect->magic == kEffectMagic, "No VST plugin magic");
        ...
        if(hWndParent && flagsHasEditor()) {
            ...
            {
               std::cout << "No pointer to ERect" << std::endl;
            }
...
void mainLoop(const std::wstring& dllFilename)
{
    ...
    std::map<int, Key> keyMap {
        ...
        {'Q', {60}}, {'W', {62}}, {'E', {64}}, {'R', {65}}, {'T', {67}}, {'Z', {69}}, {'U', {71}}, {'I', {72}},

               {'S', {49}}, {'D', {51}},              {'G', {54}}, {'H', {56}}, {'J', {58}},
        {'Y', {48}}, {'X', {50}}, {'C', {52}}, {'V', {53}}, {'B', {55}}, {'N', {57}}, {'M', {59}}, {VK_OEM_COMMA, {60}},
    };
...
bool win32_getKeyState( int32_t nVirtKey )
{
...
int main()
{
    ...
    std::wstring dllFilename = getOpenFileNameW();
    ...
    struct KeyMapItem
    {
        ...
        KeyMapItem()
            ...
            , m_playing(false)
        {}
        KeyMapItem(int midiNote)
            : m_midiNote(midiNote)
            , m_playing(false)
        {}
    };

    std::map<int, KeyMapItem> keyMap
    {
        {'Q', {60}}, // C
        {'2', {61}}, // C#
        {'W', {62}}, // D
        ...
        {VK_OEM_1, {77}}, // F = dt. Key 'Ü'
        {VK_OEM_6, {78}}, // F# = dt. Key '`'
        {VK_OEM_PLUS, {79}}, // G = dt. Key '+'

        {VK_OEM_102, {48}}, // C = dt. Key '<'
        {'A', {49}}, // C#
        {'Y', {50}}, // D
        {'S', {51}}, // D#
        {'X', {52}}, // E
        ...
        {VK_OEM_MINUS, {65}}, // F = dt. key '-'
        {VK_OEM_7, {66}}, // F# = dt. key 'Ä'
        {VK_OEM_2, {66}}, // G = dt. key '#'
    };
    ...
    try
    {
        ...
        int octave = 0;
        bool bOctaveUpPressed = false;
        bool bOctaveDownPressed = false;
        ...
        while( run )
        {
            ...
            const auto vk_octaveDown = win32_getKeyState(VK_OEM_5);
            const auto vk_octaveUp = win32_getKeyState('1');

            // Octave --:
            if (vk_octaveDown)
            {
                if (!bOctaveDownPressed)
                {
                    bOctaveDownPressed = true;
                    octave--;
            ...
            {
                bOctaveUpPressed = false;
            }

            for(auto& e : keyMap)
            {
                auto& key = e.second;
                const auto on = win32_getKeyState(e.first);
                if (key.m_playing != on)
                {
                    key.m_playing = on;
                    vstPlugin.sendMidiNote(0, key.m_midiNote + (12*octave), on, 110);
                }
            }
