...
struct DemoText
{
    DemoText(const std::string& fontFilename, std::string_view message) :
        font(std::make_unique<sf::Font>(mediaDir() +"/"+ fontFilename)),
    ...
    void updateGlyphBox(int offset)
    {
        ...
        {
            // Vertical space
            glyphBox.setPosition({-localBounds.size.x / 2.0f, localBounds.position.y + shapedGlyph.position.y - boxHeight});
