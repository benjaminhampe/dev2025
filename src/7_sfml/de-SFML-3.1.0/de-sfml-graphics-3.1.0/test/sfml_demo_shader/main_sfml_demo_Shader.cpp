...
{
    sf::Texture texture;
    if (!texture.loadFromFile(mediaDir()+"/background.jpg"))
        return std::nullopt;

    sf::Shader shader;
    if (!shader.loadFromFile(mediaDir()+"/pixelate.frag", sf::Shader::Type::Fragment))
        return std::nullopt;
...
{
    sf::Shader shader;
    if (!shader.loadFromFile(mediaDir()+"/wave.vert",
                             mediaDir()+"/blur.frag"))
        return std::nullopt;
...
{
    sf::Shader shader;
    if (!shader.loadFromFile(mediaDir()+"/storm.vert",
                             mediaDir()+"/blink.frag"))
        return std::nullopt;
...
std::optional<Edge> tryLoadEdge()
{
    ...
    if (!backgroundTexture.loadFromFile(mediaDir()+"/sfml.png"))
        return std::nullopt;
    ...
    if (!entityTexture.loadFromFile(mediaDir()+"/devices.png"))
        return std::nullopt;
    ...
    // Load the shader
    sf::Shader shader;
    if (!shader.loadFromFile(mediaDir()+"/edge.frag", sf::Shader::Type::Fragment))
        return std::nullopt;
...
std::optional<Geometry> tryLoadGeometry()
{
    ...
    if (!logoTexture.loadFromFile(mediaDir()+"/logo.png"))
        return std::nullopt;
    ...
    // Load the shader
    sf::Shader shader;
    if (!shader.loadFromFile(mediaDir()+"/billboard.vert",
                             mediaDir()+"/billboard.geom",
                             mediaDir()+"/billboard.frag"))
        return std::nullopt;
...
int main()
{
    ...
    const sf::Font font(mediaDir()+"/tuffy.ttf");
    ...
    const sf::Texture textBackgroundTexture(mediaDir()+"/text-background.png");
