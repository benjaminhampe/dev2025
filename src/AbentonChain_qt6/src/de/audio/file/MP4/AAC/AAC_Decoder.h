...
public:
                      ...
                      uint64_t sample_count)
    {
        ...
        fileInfo.sampleRate   = static_cast<uint32_t>(sampleRate);
        fileInfo.channelCount = static_cast<uint32_t>(channels);
        fileInfo.sampleType   = FileInfo::ST_F32;
        fileInfo.frameCount   = totalPcmSamples; // PCM-Samples (interleaved)
        fileInfo.duration     = double(totalPcmSamples) *
                                1'000'000'000.0 /
                                double(sampleRate);
