...
bool Parser::parse( std::string const & uri )
{
    ...
    {
        DE_DEBUG("Needed ", perf.ms(), " ms, parsed all expectBytes(",expectBytes,"), loadUri = ", uri )
    }
    else
    {
        if ( expectBytes > resultBytes )
        {
            DE_ERROR("Needed ", perf.ms(), " ms, Got fewer resultBytes(",resultBytes,") than expectBytes(",expectBytes,"), loadUri = ", uri )
        }
        else
        {
            DE_WARN("Needed ", perf.ms(), " ms, Got more resultBytes(",resultBytes,") than expectBytes(",expectBytes,"), loadUri = ", uri )
        }
    }
    return true;
}
...
Parser::parseFileHeader( uint8_t const* const beg, uint8_t const* const end )
{
   ...
   // DEBUG
   std::string s = dbStr( "MIDI file header fileType(", m_fileType, "), trackCount(", m_trackCount,"), " );
   ...
   if ( m_resolution & 0x8000 ) // is bit 15 set in time resolution?
   {
      ...
      // DEBUG
      s += dbStr( "ERROR clock(SMPTE,", m_resolution,",[ticks per frame])" );
   }
   else
   {
      // DEBUG
      ...
      {
         // MORE DEBUG
         s += dbStr( ", WARN no ticksPerQuarterNote, fallback to 96." );
...
Parser::parseTrackHeader( uint8_t const* const beg, uint8_t const* const end, uint32_t & dataSize )
{
   ...
   auto s = dbStr( "NewTrack(",m_trackIndex+1,"/",m_trackCount,") header, dataSize = ", m_trackDataSize );
...
Parser::parseTrackEnd( uint8_t const* const beg, uint8_t const* const end )
{
   ...
   auto s = dbStr( "TrackEnd(",m_trackIndex+1,"/",m_trackCount,")" );
                           ...
                           uint8_t metatype )
{
   ...
   else if (metatype == 0x58)
   {
      ...
      s = dbStr( "dt(",dt,") FF 58 04 - TimeSignature(",top,"/",bottom,"), "
