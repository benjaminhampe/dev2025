#include "MixerItemWidget.h"
#include <App.h>

MixerItemWidget::MixerItemWidget(QWidget* parent )
   : QWidget( parent )
{
   setObjectName( "MixerItemWidget" );
...
void MixerItemWidget::applySkin()
{
...
void MixerItemWidget::updateLayout()
{
    const int w = width();
    const int h = height();
    if (w < 1) return;
...
void MixerItemWidget::resizeEvent(QResizeEvent* e)
{
...
void MixerItemWidget::paintEvent( QPaintEvent* event )
{
    const int w = width();
    const int h = height();
    if (w < 1) return;
...
void
MixerItemWidget::hideEvent( QHideEvent* event )
{
   QWidget::hideEvent( event );
}

void
MixerItemWidget::showEvent( QShowEvent* event )
{
   //DE_DEBUG("")
...
void
MixerItemWidget::mouseMoveEvent( QMouseEvent* event )
{
   m_mouseX = event->x();
   m_mouseY = event->y();
...
void
MixerItemWidget::wheelEvent( QWheelEvent* event )
{
...
void
MixerItemWidget::mousePressEvent( QMouseEvent* event )
{
//   m_X = event->x();
//   m_Y = event->y();
//   m_Wheel = 0.0f;
...
void
MixerItemWidget::mouseReleaseEvent( QMouseEvent* event )
{
   m_mouseX = event->x();
   m_mouseY = event->y();
...
void
MixerItemWidget::keyPressEvent( QKeyEvent* event )
{
...
void
MixerItemWidget::keyReleaseEvent( QKeyEvent* event )
{
