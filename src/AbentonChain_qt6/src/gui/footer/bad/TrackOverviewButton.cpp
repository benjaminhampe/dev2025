#include "TrackOverviewButton.h"
#include <QPainter>

// =================================================================
TrackOverviewButton::TrackOverviewButton(QWidget* parent)
// =================================================================
    : QPushButton(parent)
{
    ...
    // connect(this, &QWidget::customContextMenuRequested,
            // this, &TrackOverviewButton::showContextMenu);

    // connect(this, &QPushButton::toggled,
    //         this, &TrackOverviewButton::onToggled);
...
void TrackOverviewButton::setKeyAssign( bool enabled )
{
    m_bEnabledKeyAssign = enabled;
    update();
}

void TrackOverviewButton::setMidiAssign( bool enabled )
{
    m_bEnabledMidiAssign = enabled;
    update();
}

void TrackOverviewButton::setPixmaps(QPixmap pixActive, QPixmap pixDeactive)
{
    m_active = pixActive;
...
void TrackOverviewButton::paintEvent(QPaintEvent* event)
{
    const int w = width();
    const int h = height();
    if (w<2) return;
...
/*
void TrackOverviewButton::showContextMenu(const QPoint &pos)
{
    QMenu menu;
