...
void ArraCentral::updateLayout()
{
    ...
    int x = m;
    int y = m;
    m_overview->setGeometry(x,y,dx,h1);
    y += h1;
    m_tracks->setGeometry(x,y,dx,h2);
    y += h2;
    m_mixer->setGeometry(x,y,dx,h3);
    update();
}

void ArraCentral::resizeEvent(QResizeEvent* e)
{
...
void ArraCentral::paintEvent( QPaintEvent* event )
{
    const int w = width();
    const int h = height();
    if (w < 1) return;
...
void
ArraCentral::hideEvent( QHideEvent* event )
{
   QWidget::hideEvent( event );
}

void
ArraCentral::showEvent( QShowEvent* event )
{
   //DE_DEBUG("")
...
void
ArraCentral::mouseMoveEvent( QMouseEvent* event )
{
   m_mouseX = event->x();
   m_mouseY = event->y();
...
void
ArraCentral::wheelEvent( QWheelEvent* event )
{
...
void
ArraCentral::mousePressEvent( QMouseEvent* event )
{
//   m_X = event->x();
//   m_Y = event->y();
//   m_Wheel = 0.0f;
...
void
ArraCentral::mouseReleaseEvent( QMouseEvent* event )
{
   m_mouseX = event->x();
   m_mouseY = event->y();
...
void
ArraCentral::keyPressEvent( QKeyEvent* event )
{
...
void
ArraCentral::keyReleaseEvent( QKeyEvent* event )
{
