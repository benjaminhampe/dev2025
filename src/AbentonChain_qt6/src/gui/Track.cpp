    ...
    : QWidget(parent)
{
    ...
    // connect(m_scrollTimer, &QTimer::timeout, this, &Body::autoScroll);
}
...
void Body::updateLayout()
{
    ...
    const int widgetSpacing = (g_pSkin->zoom *
                               g_pSkin->trkChnWidgetSpacing) / 100;
    const int dropIndicatorWidth = (g_pSkin->zoom *
                                    g_pSkin->trkChnDropIndicatorWidth) / 100;
...
// Zeichnen
// ------------------------------------------------------------
void Body::paintEvent(QPaintEvent* e)
{
    // const int ml = contentsMargins().left();
    const int mt = contentsMargins().top();
...
void Body::removePlugin(PluginWidget* w)
{
...
// Drag&Drop
// ------------------------------------------------------------
void Body::dragEnterEvent(QDragEnterEvent* e)
{
...
int Body::computeDropIndex(const QPoint &pos)
{
    for (int i = 0; i < m_widgets.size(); ++i)
    {
        auto w = m_widgets[ i ];
...
void Body::dragMoveEvent(QDragMoveEvent* e)
{
...
void Body::dragLeaveEvent(QDragLeaveEvent*)
{
...
void Body::dropEvent(QDropEvent* e)
{
    // qDebug() << "dropEvent()";
...
int Body::computeDropX(int index)
{
    const int n = static_cast<int>(m_widgets.size());
...
// Auto-Scroll
// ------------------------------------------------------------
void Body::startAutoScrollIfNeeded(const QPoint &pos)
{
/*
    int margin = 40;
...
int Body::computeWidgetIndex(const QPoint &pos)
{
    for (int i = 0; i < m_widgets.size(); ++i)
    {
        auto w = m_widgets[ i ];
...
void Body::mouseReleaseEvent(QMouseEvent* e)
{
...
void Body::mousePressEvent(QMouseEvent* e)
{
    if (e->button() == Qt::LeftButton)
    {
...
void Body::mouseMoveEvent(QMouseEvent* e)
{
    if (m_isLeftPressed)
    {
...
void Body::swapWidgets(int drag, int drop)
{
    if (drag == drop)
    {
        return;
...
// Speicherung / Laden
// ------------------------------------------------------------
void Body::saveState(const QString &path)
{
    QJsonArray arr;
...
void Body::loadState(const QString &path)
{
    QFile f(path);
