...
/*
    ...
    pixOff = createFromText(0,48,"1-PianoRoll", m_textColor,m_panelColor);
    pixOn = createFromText(0,48,"1-PianoRoll", m_textColor,m_panelColor);
...
void
PianoRoll::mousePressEvent( QMouseEvent* event )
{
//   m_X = event->x();
//   m_Y = event->y();
//   m_Wheel = 0.0f;
...
int64_t
PianoRoll::pixel2time( int64_t px ) const
{
   return (px - m_keyWidth) * m_nanosecondsPerPixel;
}

int64_t
PianoRoll::time2pixel( int64_t timeInNanoSec ) const
{
   return m_keyWidth + (timeInNanoSec * m_pixelsPerNanosecond );
}

void
PianoRoll::mouseReleaseEvent( QMouseEvent* event )
{
   m_mouseX = event->x();
   m_mouseY = event->y();
