...
inline void db2D(uint64_t id, uint32_t w, uint32_t& x, uint32_t& y)
{
    y = id / w;
...
void GL_Mesh16::upload( bool bForceUpload )
{
    ...
    if ( bNeedUpload )
    {
        ...
        if ( VBO )
        {
            ...
            const size_t vertexSize = sizeof(GL_Mesh16_Vertex);
...
void GL_Mesh16::createWavMatrix01(GL_Mesh16 & m,
                                  glm::vec3 const & d,
                                  DE_AlignedFloatShiftMatrix const & table,
                                  DE_AlignedFloatVector& rowBuffer )
{
    ...
    for (size_t i = 0; i < rowBuffer.size(); ++i)
    {
        ...
        const float y = dy * s;
        const float z = 0.0f;
        m.Vertices.push_back( GL_Mesh16_Vertex( x, 0, z, 0 ) );
        m.Vertices.push_back( GL_Mesh16_Vertex( x, y, z, t ) );
    }
...
// size3d.y = 500.0f;
// size3d.z = 300.0f;
void GL_Mesh16::createFftMatrix(GL_Mesh16 & m, glm::vec3 const & d,
                                DE_AlignedFloatShiftMatrix const & table,
                                DE_AlignedFloatVector& rowBuffer )
{
    const float dBmin = dbAudioLoopback_dBmin(); // -90;
    const float dBmax = dbAudioLoopback_dBmax(); // 70;
    ...
    float sampleRate_over_fftSize = 48000.0f / (2.0f*cols);
    ...
    else if ( scaleXmode == 1 ) // X-axis is scaled logarithmicly.
    {
        ...
        for ( size_t row = 0; row < rows; row++ )
        {
            for ( size_t col = 0; col < cols; col++ )
            {
                float dB = table.getPixel( col, row );  // The row data
                float t = (dB - dBmin) * dBrangeInv;
                float k = log10f( float(col+1) ) / log10f( float(cols) ); //  - 1.5f -1 = shift by 10^-1
                float x = d.x * k;
                float y = dy * t;
                float z = dz * row;
                m.Vertices.push_back( GL_Mesh16_Vertex(x,y,z,t) );
...
void GL_Mesh16_Shader::setMaterial( const GL_Mesh16_Material & material )
{
    if (!m_driver)
    {
        DE_ERROR("No shader")
    ...
    glm::mat4 u_mvp = viewProjMat * material.modelMat;
...
void GL_Mesh16_Shader::draw( const GL_Mesh16 & mesh )
{
