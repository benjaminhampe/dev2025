...
BVH::Partition_output
BVH::surface_area_heuristic(const BVH::Node parent_node, std::vector<unsigned int>& triangle_indices, const std::vector<Triangle>& triangles, const bool& split_buckets)
{
    BVH::Partition_output output;
    ...
    for (size_t axis = 0; axis < 3; axis++)
    {
        ...
        {
            return triangles.at(a).centroid[axis]
                 < triangles.at(b).centroid[axis];
		});
...
BVH::BVH_data BVH::construct(std::string path, const Heuristic heuristic) {
    ...
    while (!queue.empty()) {
        ...
        BVH.at(current_node_idx).child1_idx = BVH_len; // index if the first child in the BVH_index_array
        BVH.push_back(Lnode);
        ...
        BVH.at(current_node_idx).child2_idx = BVH_len + 1; // index if the first child in the BVH_index_array
        BVH.push_back(Rnode);
